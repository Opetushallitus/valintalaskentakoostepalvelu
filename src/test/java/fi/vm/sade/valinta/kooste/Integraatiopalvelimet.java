...
public class Integraatiopalvelimet {
  ...
  private static void mockToReturnValue(String method, String p, String r) {
    mockServer.when(request().withMethod(method).withPath(p))
        .respond(response().withStatusCode(200)
            .withHeaders(new org.mockserver.model.Header("Content-Type", "application/json; charset=utf-8"))
            .withBody(r));
  }

  private static void mockToReturnValueWithCookie(String method, String p, String r, String cookieName,
      String cookieValue) {
    mockServer.when(request().withMethod(method).withPath(p))
        .respond(response().withStatusCode(200)
            .withHeaders(new org.mockserver.model.Header("Set-Cookie", cookieName + "=" + cookieValue),
  ...
  private static void mockToReturnValueWithParams(String method, String p, String r, List<Parameter> parameters) {
    mockServer.when(request().withMethod(method).withPath(p).withQueryStringParameters(parameters))
        .respond(response().withStatusCode(200)
            .withHeaders(new org.mockserver.model.Header("Content-Type", "application/json; charset=utf-8"))
            .withBody(r));
  }
  ...
  private static void mockToReturnValueAndCheckBody(String method, String p, String r, String regex) {
    mockServer.when(request().withMethod(method).withPath(p).withBody(new RegexBody(regex)))
        .respond(response().withStatusCode(200)
            .withHeaders(new org.mockserver.model.Header("Content-Type", "application/json; charset=utf-8"))
            .withBody(r));
  }
  ...
  public static void mockToNoContent(String method, String p) {
    mockServer.when(request().withMethod(method).withPath(p)).respond(response().withStatusCode(204));
  }

  public static void mockToNotFound(String method, String p) {
    mockServer.when(request().withMethod(method).withPath(p)).respond(response().withStatusCode(404));
  }

  public static void mockToInternalServerError(String method, String p) {
    mockServer.when(request().withMethod(method).withPath(p)).respond(response().withStatusCode(500));
  }

  public static void mockToAccept(String method, String p) {
    mockServer.when(request().withMethod(method).withPath(p)).respond(response().withStatusCode(200));
  }

  public static void mockToReturnJsonWithParams(String method, String p, Object r, Map<String, String> parameters) {
    String s;
    mockToReturnValueWithParams(method, p, s = gson().toJson(r), parameters.keySet().stream()
        .map(name -> new Parameter(name, parameters.get(name))).collect(Collectors.toList()));
    LOG.debug(s);
  }
  ...
  public static void mockToReturnJsonWithCookie(String method, String p, Object r, String cookieName,
      String cookieValue) {
    String s;
  ...
  private static JsonSerializer<Date> dateJsonSerializer = (Date src, Type typeOfSrc,
      JsonSerializationContext context) -> src == null ? null : new JsonPrimitive(src.getTime());

  private static Gson gson = DateDeserializer.gsonBuilder().registerTypeAdapter(Date.class, dateJsonSerializer)
      .create();
