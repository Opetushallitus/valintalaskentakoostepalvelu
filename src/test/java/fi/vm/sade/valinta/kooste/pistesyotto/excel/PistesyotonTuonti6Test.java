...
public class PistesyotonTuonti6Test extends PistesyotonTuontiTestBase {
  ...
  public void testaaOutput() throws Exception {
    List<ValintakoeOsallistuminenDTO> osallistumistiedot =
        lueOsallistumisTiedot("6/List_ValintakoeOsallistuminenDTO.json");
    ...
    List<ApplicationAdditionalDataDTO> pistetiedot =
        luePistetiedot("6/List_ApplicationAdditionalDataDTO.json");
    ...
    ApplicationAdditionalDataDTO alenderinPistetiedot =
        pistetiedot.stream().filter(h -> h.getLastName().equals("Alander")).findFirst().get();
    // System.out.println(new GsonBuilder().setPrettyPrinting().create().toJson(dada));
    assertEquals(
        alenderinPistetiedot.getAdditionalData().get("SOTE1_kaikkiosiot-OSALLISTUMINEN"),
        "EI_OSALLISTUNUT");
    assertEquals(
        alenderinPistetiedot.getAdditionalData().get("SOTEKOE_VK_RYHMA1-OSALLISTUMINEN"),
        "EI_OSALLISTUNUT");
  }
}
