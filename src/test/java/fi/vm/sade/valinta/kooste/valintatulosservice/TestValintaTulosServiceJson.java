...
public class TestValintaTulosServiceJson {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(TestValintaTulosServiceJson.class);
  ...
  public void testaaMuutoshistoriaJson() throws JsonSyntaxException, IOException {
    ...
    List<Muutoshistoria> muutoshistoriat = GSON.fromJson(
        IOUtils.toString(new ClassPathResource("valintatulosservice/muutoshistoria.json").getInputStream()),
    ...
    final Predicate<Map.Entry<String, Date>> isVastaanotto = entry -> asList("VASTAANOTTANUT_SITOVASTI",
        "VASTAANOTTANUT", "EHDOLLISESTI_VASTAANOTTANUT").contains(entry.getKey());
    ...
    Optional<Map.Entry<String, Date>> newestVastaanottoFieldStatus = muutoshistoriat.stream()
        ...
        .sorted(Comparator.<Map.Entry<String, Date>, Date>comparing(Map.Entry::getValue).reversed())
        .findFirst();
  ...
  public void testaaJson() throws JsonSyntaxException, IOException {
    Gson GSON = new Gson();
    Collection<ValintaTulosServiceDto> valintaTulosServiceDto = GSON.fromJson(IOUtils.toString(
        new ClassPathResource("valintatulosservice/valintatuloservice_1.2.246.562.29.173465377510.json")
            .getInputStream()),
        new TypeToken<ArrayList<ValintaTulosServiceDto>>() {
        }.getType());
    valintaTulosServiceDto = valintaTulosServiceDto.stream()
