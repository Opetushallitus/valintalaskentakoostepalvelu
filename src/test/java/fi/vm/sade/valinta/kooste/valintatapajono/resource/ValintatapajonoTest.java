...
public class ValintatapajonoTest extends ValintatapajonoTestTools {
  static final Logger LOG = LoggerFactory.getLogger(ValintatapajonoTest.class);
  ...
  public void testaaExcel() throws Exception {
    ...
    try {
      ValintatapajonoExcel valintatapajonoExcel = new ValintatapajonoExcel("1.2.246.562.29.173465377510",
          "1.2.246.562.20.85029108298", "14229501603804360431186491391519", "", "", valinnanvaiheet,
      ...
      valintatapajonoExcel.getExcel().tuoXlsx(
          new ClassPathResource("/valintatapajono/valintatapajono_yksi_hakija.xlsx").getInputStream());
    } catch (Throwable t) {
    ...
    assertRivi(new ValintatapajonoRivi("1.2.246.562.11.00000000181", "1", "Ilman Laskentaa",
        ValintatapajonoExcel.HYVAKSYTTAVISSA, "55.45", null, null, null), rivit.get(0));
  }

  @Test
  public void testaaValintatapajononTuontia() throws Exception {
    String valintatapajonoOid = "14229501603804360431186491391519";
    ValintatapajonoRivi rivi = new ValintatapajonoRivi("1.2.246.562.11.00000000181", "1", "Ilman laskentaa",
        "HYVAKSYTTAVISSA", "", "", "", "");
    ...
    ValinnanvaiheDTO generoitu_valinnanvaihe = ValintatapajonoTuontiConverter.konvertoi("hakuOid",
        "1.2.246.562.20.85029108298", valintatapajonoOid, valintaperusteet,
        hakemukset.stream().map(HakuappHakemusWrapper::new).collect(Collectors.toList()), valinnanvaihe,
        rivit.getRivit());
  ...
  public void testaaValintatapajononTuontiaPisteitaJaMaarittelematonTila() throws Exception {
    String valintatapajonoOid = "14229501603804360431186491391519";
    ValintatapajonoRivi rivi = new ValintatapajonoRivi("1.2.246.562.11.00000000181", "", "Ilman laskentaa",
        "MAARITTELEMATON", "20", "", "", "");
    ValintatapajonoRivit rivit = new ValintatapajonoRivit(Arrays.asList(rivi));
    LOG.error("{}", new GsonBuilder().setPrettyPrinting().create().toJson(rivit));
    List<ValinnanVaiheJonoillaDTO> valintaperusteet = GSON.fromJson(
  ...
  public void testaaValintatapajononTuonninVirhe() throws Exception {
    ...
    try {
      ...
      assertEquals("Samassa valintatapajonossa ei voida käyttää sekä jonosijoja että kokonaispisteitä.",
          re.getMessage());
