...
public class HaunValintakoelaskentaKoosteReititysTest {
	...
	@Bean
	public LaskeValintakoeosallistumisetHakemukselleKomponentti getLaskeValintakoeosallistumisetHakemukselleKomponentti() {
		return new LaskeValintakoeosallistumisetHakemukselleKomponentti();
	}

	@Bean
	public HakukohteenValintaperusteetProxyCachingImpl getHakukohteenValintaperusteetProxyCachingImpl(
			ValintaperusteService vs) {
		return new HakukohteenValintaperusteetProxyCachingImpl(vs, 100);
	}
	...
	public void test() throws JSONException {
		...
		when(
				applicationResourceMock.findApplications(anyString(),
						anyList(), anyString(), anyString(), eq(HAKU_OID),
						anyString(), anyInt(), anyInt())).thenReturn(
				new Gson()
						.fromJson(HAKEMUKSET_RESPONSE_JSON, HakemusList.class));
		when(applicationResourceMock.getApplicationByOid(eq(HAKEMUS1_OID)))
				.thenReturn(
						new Gson().fromJson(HAKEMUS1_RESPONSE_JSON,
								Hakemus.class));
		when(applicationResourceMock.getApplicationByOid(eq(HAKEMUS2_OID)))
				.thenReturn(
		...
		verify(valintalaskentaServiceMock, times(2)).valintakokeet(
				ac.capture(), anyList());
	...
	private final static String HAKEMUKSET_RESPONSE_JSON = "{"
			+ "totalCount: 2," + "results: [" + "{" + "oid: \"" + HAKEMUS1_OID
			+ "\"," + "state: \"ACTIVE\"," + "firstNames: \"VXCVX XccVrVr\","
			+ "lastName: \"pFUBjjes\"," + "ssn: \"300582-2022\","
			+ "personOid: \"1.2.246.562.24.37911437777\"" + "}," + "{"
			+ "oid: \"" + HAKEMUS2_OID + "\"," + "state: \"ACTIVE\","
			+ "firstNames: \"ZYayxck cozVZk\"," + "lastName: \"mqmVgddU\","
			+ "ssn: \"131194-1412\","
