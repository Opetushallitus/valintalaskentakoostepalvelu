...
public class KoskiAsyncResourceImplTest {
  ...
  private final HttpClient koosteHttpClient = new HttpClient(httpClient, null,
      DateDeserializer.gsonBuilder().create());
  private final UrlConfiguration urlConfiguration = mock(UrlConfiguration.class);
  private final KoskiAsyncResourceImpl koskiAsyncResource = new KoskiAsyncResourceImpl(urlConfiguration,
      koosteHttpClient, username, password, maxOppijatPostSize);
  ...
  public void findKoskiOppijat() throws Exception {
    ...
    assertThat(oppija.getOpiskeluoikeudet().toString(), containsString(
        "\"oppilaitosnumero\":{\"koodiarvo\":\"07522\",\"nimi\":{\"fi\":\"Hatsalan klassillinen koulu\"}"));
    assertThat(oppija.getOpiskeluoikeudet().toString(),
