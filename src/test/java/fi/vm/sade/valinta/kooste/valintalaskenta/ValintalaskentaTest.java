...
public class ValintalaskentaTest {
  ...
  private final SuoritusrekisteriAsyncResource suoritusrekisteriAsyncResource = mock(
      SuoritusrekisteriAsyncResource.class);
  private final ValintalaskentaAsyncResource valintalaskentaAsyncResource = mock(ValintalaskentaAsyncResource.class);
  private final ValintaperusteetAsyncResource valintaperusteetAsyncResource = mock(
      ValintaperusteetAsyncResource.class);
  ...
  private final OhjausparametritAsyncResource ohjausparametritAsyncResource = mock(
      OhjausparametritAsyncResource.class);
  ...
  private final LaskentaActorSystem laskentaActorSystem = new LaskentaActorSystem(seurantaAsyncResource,
      new LaskentaStarter(ohjausparametritAsyncResource, valintaperusteetAsyncResource, seurantaAsyncResource,
          tarjontaAsyncResource),
      new LaskentaActorFactory(5, valintalaskentaAsyncResource, applicationAsyncResource, ataruAsyncResource,
          valintaperusteetAsyncResource, seurantaAsyncResource, suoritusrekisteriAsyncResource,
  ...
  private final Haku hakuDTO = new Haku(hakuOid, new HashMap<>(), new HashSet<>(), null, null, null, null, null,
      null);
  private final Haku ataruHakuDTO = new Haku(ataruHakuOid, new HashMap<>(), new HashSet<>(), "ataru-lomake-avain",
  ...
  private final List<HakukohdeJaOrganisaatio> hakukohdeJaOrganisaatios = Arrays.asList(
      new HakukohdeJaOrganisaatio(hakukohde1Oid, "o1"), new HakukohdeJaOrganisaatio(hakukohde2Oid, "o2"),
  ...
  private final AuditSession auditSession = new AuditSession("1.2.3.4.5",
      Collections.singletonList("APP_VALINTA_EVERYTHING_CRUD"), "Firefox", "127.0.0.1");
  ...
  public void setUpTestData() {
    ...
    PisteetWithLastModified pisteet = new PisteetWithLastModified(Optional.empty(),
        Collections.singletonList(new Valintapisteet(hakemus.getOid(), hakemus.getPersonOid(), "Frank",
            "Tester", Collections.emptyList())));

    PisteetWithLastModified ataruPisteet = new PisteetWithLastModified(Optional.empty(),
        Collections.singletonList(new Valintapisteet(ataruHakemus.getHakemusOid(), ataruHakemus.getPersonOid(),
            "Zl2A5", "TAUsuL4BQc", Collections.emptyList())));
    ...
    when(valintapisteAsyncResource.getValintapisteetWithHakemusOidsAsFuture(anyList(), any(AuditSession.class)))
        .thenReturn(CompletableFuture.completedFuture(pisteet));
    when(valintapisteAsyncResource.getValintapisteet(eq(hakuOid), eq(hakukohde1Oid), any(AuditSession.class)))
        .thenReturn(CompletableFuture.completedFuture(pisteet));
    when(valintapisteAsyncResource.getValintapisteet(eq(hakuOid), eq(hakukohde2Oid), any(AuditSession.class)))
        .thenReturn(CompletableFuture.completedFuture(pisteet));
    when(valintapisteAsyncResource.getValintapisteet(eq(hakuOid), eq(hakukohde3Oid), any(AuditSession.class)))
        .thenReturn(CompletableFuture.completedFuture(pisteet));
    when(valintapisteAsyncResource.getValintapisteet(eq(ataruHakuOid), eq(ataruHakukohdeOid),
        any(AuditSession.class))).thenReturn(CompletableFuture.completedFuture(ataruPisteet));
    when(valintapisteAsyncResource.getValintapisteet(eq(ataruHakuOid), eq(ataruHakukohdeOid2),
        any(AuditSession.class))).thenReturn(CompletableFuture.completedFuture(ataruPisteet));
    ...
    when(suoritusrekisteriAsyncResource.getSuorituksetByOppijas(List.of("personOid"), ataruHakuOid))
        .thenReturn(CompletableFuture.completedFuture(Collections.singletonList(anonOppijaFromSure)));
    when(suoritusrekisteriAsyncResource.getSuorituksetByOppijas(List.of("personOid"), hakuOid))
        .thenReturn(CompletableFuture.completedFuture(Collections.singletonList(anonOppijaFromSure)));
    when(suoritusrekisteriAsyncResource.getSuorituksetByOppijas(Collections.emptyList(), ataruHakuOid))
        .thenReturn(CompletableFuture.completedFuture(Collections.emptyList()));
  ...
  public void onnistuneestaValintalaskennastaPidetaanKirjaaSeurantapalveluun() throws InterruptedException {
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde1Oid)).thenReturn(
        CompletableFuture.completedFuture(Collections.singletonList(new HakuappHakemusWrapper(hakemus))));
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde2Oid)).thenReturn(
        CompletableFuture.completedFuture(Collections.singletonList(new HakuappHakemusWrapper(hakemus))));
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde3Oid)).thenReturn(
        CompletableFuture.completedFuture(Collections.singletonList(new HakuappHakemusWrapper(hakemus))));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, hakukohde1Oid, HakukohdeTila.VALMIS, Optional.empty()))
        .thenReturn(Observable.just(Response.noContent().build()));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, hakukohde2Oid, HakukohdeTila.VALMIS, Optional.empty()))
        .thenReturn(Observable.just(Response.noContent().build()));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, hakukohde3Oid, HakukohdeTila.VALMIS, Optional.empty()))
        .thenReturn(Observable.just(Response.noContent().build()));
    ...
    when(koskiService.haeKoskiOppijat(any(String.class), any(), any(), any(SuoritustiedotDTO.class),
        any(Date.class))).thenReturn(CompletableFuture.completedFuture(Collections.emptyMap()));

    LaskentaStartParams laskentaJaHaku = new LaskentaStartParams(auditSession, uuid, hakuOid, false, null, null,
        hakukohdeJaOrganisaatios, LaskentaTyyppi.HAKUKOHDE);
    ...
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, hakukohde1Oid, HakukohdeTila.VALMIS,
        Optional.empty());
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, hakukohde2Oid, HakukohdeTila.VALMIS,
        Optional.empty());
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, hakukohde3Oid, HakukohdeTila.VALMIS,
        Optional.empty());
  ...
  public void valintaLaskentaHakeeKoskestaTiedot() throws InterruptedException {
    ...
    when(koskiService.haeKoskiOppijat(any(String.class), any(), any(), any(SuoritustiedotDTO.class),
        any(Date.class))).thenReturn(CompletableFuture.completedFuture(Collections.emptyMap()));
    when(valintalaskentaAsyncResource.laskeKaikki(any(LaskeDTO.class), any(SuoritustiedotDTO.class)))
    ...
    verify(koskiService, times(3)).haeKoskiOppijat(any(String.class), any(), any(), any(SuoritustiedotDTO.class),
        any(Date.class));
  ...
  public void onnistuneestaValintalaskennastaAtaruHaullePidetaanKirjaaSeurantapalveluun()
    ...
    when(ataruAsyncResource.getApplicationsByHakukohde(ataruHakukohdeOid, false))
        .thenReturn(CompletableFuture.completedFuture(Collections.singletonList(
            MockAtaruAsyncResource.getAtaruHakemusWrapper("1.2.246.562.11.00000000000000000063"))));
    when(ataruAsyncResource.getApplicationsByHakukohde(ataruHakukohdeOid2, false))
        .thenReturn(CompletableFuture.completedFuture(Collections.singletonList(
            MockAtaruAsyncResource.getAtaruHakemusWrapper("1.2.246.562.11.00000000000000000063"))));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, ataruHakukohdeOid, HakukohdeTila.VALMIS,
        Optional.empty())).thenReturn(Observable.just(Response.noContent().build()));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, ataruHakukohdeOid2, HakukohdeTila.VALMIS,
        Optional.empty())).thenReturn(Observable.just(Response.noContent().build()));
    when(seurantaAsyncResource.merkkaaLaskennanTila(uuid, LaskentaTila.VALMIS, Optional.empty()))
        .thenReturn(Observable.just(Response.noContent().build()));
    when(seurantaAsyncResource.otaSeuraavaLaskentaTyonAlle()).thenReturn(Observable.just(Optional.empty()));
    ...
    LaskentaStartParams laskentaJaHaku = new LaskentaStartParams(auditSession, uuid, ataruHakuOid, false, null,
        null, ataruHakukohdeJaOrganisaatios, LaskentaTyyppi.HAKUKOHDE);
    ...
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, ataruHakukohdeOid, HakukohdeTila.VALMIS,
        Optional.empty());
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, ataruHakukohdeOid2, HakukohdeTila.VALMIS,
        Optional.empty());
    verify(seurantaAsyncResource).merkkaaLaskennanTila(uuid, LaskentaTila.VALMIS, Optional.empty());
    Mockito.verifyNoMoreInteractions(seurantaAsyncResource);
  }
  ...
  public void onnistuneestaValintaryhmalaskennastaPidetaanKirjaaSeurantapalveluun() throws InterruptedException {
    ...
    LaskentaStartParams laskentaJaHaku = new LaskentaStartParams(auditSession, uuid, hakuOid, false, vaiheenNumero,
        null, hakukohdeJaOrganisaatios, LaskentaTyyppi.VALINTARYHMA);

    when(valintalaskentaAsyncResource.laskeJaSijoittele(eq(uuid), anyList(), any(SuoritustiedotDTO.class)))
  ...
  public void puuttuvaJonokriteeriAiheuttaaLaskennanEpaonnistumisen() throws InterruptedException {
    ...
    LaskentaStartParams laskentaJaHaku = new LaskentaStartParams(auditSession, uuid, hakuOid, false, vaiheenNumero,
        null, Collections.singletonList(new HakukohdeJaOrganisaatio(hakukohde1Oid, "o1")),
        LaskentaTyyppi.HAKUKOHDE);
    laskentaActorSystem.suoritaValintalaskentaKerralla(hakuDTO, null, laskentaJaHaku);
    Thread.sleep(500);

    verify(seurantaAsyncResource).merkkaaHakukohteenTila(eq(uuid), eq(hakukohde1Oid), eq(HakukohdeTila.KESKEYTETTY),
  ...
  private ValintaperusteetDTO valintaperusteetWithValintatapajonoUsingValintalaskenta(
    ...
    if (hasJonokriteeri) {
      valintatapajono
          .setJarjestyskriteerit(Collections.singletonList(new ValintaperusteetJarjestyskriteeriDTO()));
    }
  ...
  public void valintalaskentaEiKaytossaAiheuttaaLaskennanEpaonnistumisen() throws InterruptedException {
    ...
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(eq(uuid), eq(hakukohde1Oid), eq(HakukohdeTila.KESKEYTETTY),
        getIlmoitusDtoOptional(String.format(
  ...
  public void epaonnistuneetLaskennatKirjataanSeurantapalveluun() throws InterruptedException {
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde1Oid))
        .thenReturn(CompletableFuture.failedFuture(new RuntimeException(
            getClass().getSimpleName() + " : Ei saatu haettua hakemuksia kohteelle " + hakukohde1Oid)));
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde2Oid)).thenReturn(
        CompletableFuture.completedFuture(Collections.singletonList(new HakuappHakemusWrapper(hakemus))));
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde3Oid)).thenReturn(
        CompletableFuture.completedFuture(Collections.singletonList(new HakuappHakemusWrapper(hakemus))));
    ...
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, hakukohde3Oid, HakukohdeTila.VALMIS, Optional.empty()))
        .thenReturn(Observable.just(Response.ok().build()));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, hakukohde3Oid, HakukohdeTila.KESKEYTETTY,
        Optional.empty())).thenReturn(Observable.just(Response.ok().build()));
    when(koskiService.haeKoskiOppijat(any(String.class), any(), any(), any(SuoritustiedotDTO.class),
        any(Date.class))).thenReturn(CompletableFuture.completedFuture(Collections.emptyMap()));
    ...
    verify(valintapisteAsyncResource, times(2)).getValintapisteetWithHakemusOidsAsFuture(eq(hakemusOids),
  ...
  public void epaonnistuneetAtaruLaskennatKirjataanSeurantapalveluun() throws InterruptedException {
    ...
    when(ataruAsyncResource.getApplicationsByHakukohde(ataruHakukohdeOid, false))
        .thenReturn(CompletableFuture.failedFuture(new RuntimeException(
            getClass().getSimpleName() + " : Ei saatu haettua hakemuksia kohteelle " + ataruHakukohdeOid)));
    when(ataruAsyncResource.getApplicationsByHakukohde(ataruHakukohdeOid2, false))
        .thenReturn(CompletableFuture.completedFuture(
            Collections.singletonList(MockAtaruAsyncResource.getAtaruHakemusWrapper(ataruHakemusOid))));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, ataruHakukohdeOid2, HakukohdeTila.VALMIS,
        Optional.empty())).thenReturn(Observable.just(Response.ok().build()));
    when(seurantaAsyncResource.merkkaaHakukohteenTila(uuid, ataruHakukohdeOid, HakukohdeTila.KESKEYTETTY,
        Optional.empty())).thenReturn(Observable.just(Response.ok().build()));
    when(koskiService.haeKoskiOppijat(any(String.class), any(), any(), any(SuoritustiedotDTO.class),
        any(Date.class))).thenReturn(CompletableFuture.completedFuture(Collections.emptyMap()));
    ...
    verify(ataruAsyncResource).getApplicationsByHakukohde(ataruHakukohdeOid2, false);
    verify(valintapisteAsyncResource, times(1)).getValintapisteetWithHakemusOidsAsFuture(eq(ataruHakemusOids),
        any(AuditSession.class));
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(uuid, ataruHakukohdeOid2, HakukohdeTila.VALMIS,
        Optional.empty());
    verify(seurantaAsyncResource).merkkaaHakukohteenTila(eq(uuid), eq(ataruHakukohdeOid),
  ...
  private Optional<IlmoitusDto> getIlmoitusDtoOptional(String odotettuOtsikonSisalto) {
    return argThat(new ArgumentMatcher<>() {
      ...
      public String toString() {
        return IlmoitusDto.class.getSimpleName() + ", jossa " + odotettuIlmoitustyyppi + " ja otsikossa \""
  ...
  public void poikkeukseenEpaonnistuneitaLaskentojaEiKirjataSeurantapalveluun() throws InterruptedException {
    when(applicationAsyncResource.getApplicationsByOid(hakuOid, hakukohde1Oid)).thenThrow(new RuntimeException(
        getClass().getSimpleName() + " : Ei saatu taaskaan haettua hakemuksia kohteelle " + hakukohde1Oid));

    laskentaActorSystem.suoritaValintalaskentaKerralla(hakuDTO, null, new LaskentaStartParams(auditSession, uuid,
        hakuOid, false, null, null, hakukohdeJaOrganisaatios, LaskentaTyyppi.HAKUKOHDE));
    Thread.sleep(500);
