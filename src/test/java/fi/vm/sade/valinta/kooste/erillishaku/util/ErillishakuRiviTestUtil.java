...
public class ErillishakuRiviTestUtil {
  ...
  public static ErillishakuRivi laillinenRivi() {
    return new ErillishakuRiviBuilder().sukunimi(SUKUNIMI).etunimi(ETUNIMI).henkilotunnus(HETU).sahkoposti(EMPTY)
        .syntymaAika(SYNTYMAAIKA).sukupuoli(SUKUPUOLI).personOid(EMPTY).aidinkieli(AIDINKIELI)
        ...
        .ilmoittautumisTila(IlmoittautumisTila.EI_ILMOITTAUTUNUT.toString()).julkaistaankoTiedot(true)
        ...
        .kotikunta("HELSINKI").toisenAsteenSuoritus(true).toisenAsteenSuoritusmaa("FIN")
        .maksuvelvollisuus(Maksuvelvollisuus.NOT_REQUIRED).build();
  }

  public static ErillishakuRivi viallinenRiviPuuttuvillaTunnisteilla() {
    return new ErillishakuRiviBuilder().sukunimi(SUKUNIMI).etunimi(ETUNIMI).henkilotunnus(EMPTY).sahkoposti(EMPTY)
        .syntymaAika(EMPTY).sukupuoli(Sukupuoli.EI_SUKUPUOLTA).personOid(EMPTY).aidinkieli(EMPTY)
        .hakemuksenTila(HakemuksenTila.HYVAKSYTTY.toString()).ehdollisestiHyvaksyttavissa(false)
        .vastaanottoTila(ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI.toString())
        .ilmoittautumisTila(IlmoittautumisTila.EI_ILMOITTAUTUNUT.toString()).julkaistaankoTiedot(true)
        .poistetaankoRivi(false).asiointikieli("FI").puhelinnumero("040123456789").osoite("Esimerkkitie 2")
  ...
  public static String viallinenJsonRiviPuuttuvillaTunnisteilla() {
    ...
    return "{\"rivit\":["
        + new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").create().toJson(laillinenRivi()) + ","
