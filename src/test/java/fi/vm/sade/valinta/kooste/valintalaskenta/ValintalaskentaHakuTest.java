...
import fi.vm.sade.valinta.kooste.HakemusCacheInvalidatorMock;
...
@Import({ ValintalaskentaRouteImpl.class, SuoritaLaskentaKomponentti.class,
		HakemusCacheInvalidatorMock.class })
...
public class ValintalaskentaHakuTest {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ValintalaskentaHakuTest.class);
	...
	public void testaaPalvelutToimiiReitti() {
		...
		Mockito.when(
				haeHakukohteetTarjonnaltaKomponentti
						.haeHakukohteetTarjonnalta(Mockito.anyString()))
		//
				.thenReturn(Arrays.asList(h));

		Mockito.when(
				haeHakukohteenHakemuksetKomponentti
						.haeHakukohteenHakemukset(Mockito.anyString()))
		//
		...
		Mockito.verify(valintalaskentaService, Mockito.atLeastOnce()).laske(
	...
	private void reset() {
		Mockito.reset(haeHakukohteetTarjonnaltaKomponentti,
				haeHakukohteenHakemuksetKomponentti, haeHakemusKomponentti,
	...
	public void testaaTarjontaEpaonnistuuSatunnaisestiReitti() {
		...
		//
				.thenThrow(
						new RuntimeException(
								"412: Autentikaatio tai vastaava meni rikki!"));
		valintalaskentaRoute.aktivoiValintalaskenta("h0");
		...
		Mockito.verify(haeHakemusKomponentti, Mockito.atLeastOnce())
				.haeHakemus(Mockito.anyString());

		LOG.info(
				"Tarjonnan epäonnistumisvirhe {}",
				Arrays.toString(valintalaskentaValvomo.getUusimmatProsessit()
						.iterator().next().getExceptions().toArray()));
	}
	...
	public void testaaValintalaskentaEpaonnistuuSatunnaisestiReitti() {
		...
		Mockito.when(
				valintalaskentaService.laske(
						Mockito.anyListOf(HakemusTyyppi.class),
						Mockito.anyListOf(ValintaperusteetTyyppi.class)))
				.thenThrow(new RuntimeException("Valintalaskenta epäonnistui!"));
