...
public class KirjeetUtilTest {
  ...
  public void testShouldSortByPriorityAscending() {
    List<Integer> priot = flatten(jono().map(setPriority(5)).map(setTila(HakemuksenTila.HYVAKSYTTY)),
  ...
  public void testShouldSortByTila() {
    List<HakemuksenTila> tilas = flatten(jono().map(setPriority(4)).map(setTila(HakemuksenTila.HYLATTY)),
        jono().map(setPriority(5)).map(setTila(HakemuksenTila.HYVAKSYTTY)),
        jono().map(setPriority(6)).map(setTila(HakemuksenTila.VARALLA)),
        jono().map(setPriority(6)).map(setTila(HakemuksenTila.HYVAKSYTTY)),
        jono().map(setPriority(7)).map(setTila(HakemuksenTila.HYLATTY))).sorted(KirjeetUtil.sortByTila())
            .map(tilat).collect(Collectors.toList());

    Assert.assertTrue(tilas.equals(ImmutableList.of(HakemuksenTila.HYVAKSYTTY, HakemuksenTila.HYVAKSYTTY,
        HakemuksenTila.VARALLA, HakemuksenTila.HYLATTY, HakemuksenTila.HYLATTY)));
  }

  @Test
  public void testShouldSortByVarasijaNumero() {
    List<HakutoiveenValintatapajonoDTO> jonos = flatten(
        jono().map(setPriority(5)).map(setTila(HakemuksenTila.HYVAKSYTTY)),
        ...
        jono().map(setPriority(7)).map(setTila(HakemuksenTila.HYVAKSYTTY))).sorted(KirjeetUtil.sortByTila())
            .collect(Collectors.toList());
    List<HakemuksenTila> tilas = jonos.stream().map(tilat).collect(Collectors.toList());

    Assert.assertTrue(tilas.equals(ImmutableList.of(HakemuksenTila.HYVAKSYTTY, HakemuksenTila.HYVAKSYTTY,
        HakemuksenTila.VARALLA, HakemuksenTila.VARALLA)));
  ...
  private final Function<HakutoiveenValintatapajonoDTO, Integer> varasijanumerot = (jono) -> Optional
      .ofNullable(jono.getVarasijanNumero()).orElse(-1);
  private final Function<HakutoiveenValintatapajonoDTO, HakemuksenTila> tilat = (jono) -> jono.getTila();
  private final Function<HakutoiveenValintatapajonoDTO, Integer> prioriteetit = (jono) -> jono
  ...
  private Function<HakutoiveenValintatapajonoDTO, HakutoiveenValintatapajonoDTO> setVarasijaNumero(
