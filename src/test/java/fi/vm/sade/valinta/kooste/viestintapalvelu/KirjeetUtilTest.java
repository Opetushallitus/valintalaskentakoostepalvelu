...
public class KirjeetUtilTest {
  ...
  public void testShouldSortByPriorityAscending() {
    List<Integer> priot =
        flatten(
            ...
            .sorted(KirjeetUtil.sortByPrioriteetti())
            .map(prioriteetit)
  ...
  public void testShouldSortByVarasijaNumero() {
    List<HakutoiveenValintatapajonoDTO> jonos =
        flatten(
                ...
                jono()
                    .map(setPriority(6))
                    .map(setTila(HakemuksenTila.VARALLA))
                    .map(setVarasijaNumero(2)),
                jono()
                    .map(setPriority(6))
                    .map(setTila(HakemuksenTila.VARALLA))
                    .map(setVarasijaNumero(1)),
                jono().map(setPriority(7)).map(setTila(HakemuksenTila.HYVAKSYTTY)))
            .sorted(KirjeetUtil.sortByTila())
            .collect(Collectors.toList());
  ...
  private final Function<HakutoiveenValintatapajonoDTO, Integer> varasijanumerot =
      (jono) -> Optional.ofNullable(jono.getVarasijanNumero()).orElse(-1);
  private final Function<HakutoiveenValintatapajonoDTO, HakemuksenTila> tilat =
      (jono) -> jono.getTila();
  private final Function<HakutoiveenValintatapajonoDTO, Integer> prioriteetit =
      (jono) -> jono.getValintatapajonoPrioriteetti();

  private Stream<HakutoiveenValintatapajonoDTO> flatten(
      Stream<HakutoiveenValintatapajonoDTO>... jonot) {
    return Stream.of(jonot).flatMap(j -> j);
  }

  private List<HakutoiveenValintatapajonoDTO> asList(
      Stream<HakutoiveenValintatapajonoDTO>... jonot) {
  ...
  private Function<HakutoiveenValintatapajonoDTO, HakutoiveenValintatapajonoDTO> setPriority(
      Integer priority) {
    return (jono) -> {
  ...
  private Function<HakutoiveenValintatapajonoDTO, HakutoiveenValintatapajonoDTO> setTila(
      HakemuksenTila tila) {
    return (jono) -> {
      jono.setTila(tila);
      return jono;
