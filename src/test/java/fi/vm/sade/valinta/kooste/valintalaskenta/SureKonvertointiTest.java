...
public class SureKonvertointiTest {
  private static final Logger LOG = LoggerFactory.getLogger(SureKonvertointiTest.class);

  @Test
  public void testaaLukioKonvertointi() throws JsonSyntaxException, IOException {
    Oppija o = new Gson().fromJson(
        IOUtils.toString(new ClassPathResource("valintalaskenta/sure_pk_ja_yo.json").getInputStream()),
        new TypeToken<Oppija>() {
        }.getType());
    SuoritusJaArvosanat pk = o.getSuoritukset().stream()
        .filter(s -> new SuoritusJaArvosanatWrapper(s).isPerusopetus()).findFirst().get();
    SuoritusJaArvosanat yo = o.getSuoritukset().stream()
        .filter(s -> new SuoritusJaArvosanatWrapper(s).isYoTutkinto()).findFirst().get();
    ...
    List<AvainArvoDTO> aa = OppijaToAvainArvoDTOConverter.convert(o.getOppijanumero(), o.getSuoritukset(),
        new HakemusDTO(), pmetrit);

    String a1ESAinetunniste = aa.stream()
        .filter((AvainArvoDTO a) -> a.getAvain().startsWith("PK_A1") && "ES".equals(a.getArvo())).findAny()
        .get().getAvain().replace("_OPPIAINE", "");
  ...
  @Test
  public void testaaSureKonvertointiKaikkiAIneet() throws JsonSyntaxException, IOException {
    List<Oppija> oppijat = new Gson().fromJson(
        IOUtils.toString(new ClassPathResource("sure_kaikki_aineet.json").getInputStream()),
    ...
    final List<String> aineet = Arrays.asList("UE", "UO", "ET", "FF", "PS", "HI", "FY", "KE", "BI", "GE", "TE",
        "YH", "REAALI", "AINEREAALI", "PITKA_KIELI", "KESKIPITKA_KIELI", "LYHYT_KIELI", "AIDINKIELI", "O", "O5",
        "A", "A5", "BA", "BB", "CA", "CB", "CC", "DC", "EA", "EB", "EC", "FA", "FB", "FC", "GA", "GB", "GC",
        "HA", "HB", "I", "W", "IC", "QC", "J", "KC", "L1", "L7", "M", "N", "PA", "PB", "PC", "RR", "RO", "RY",
        "SA", "SB", "SC", "S9", "TA", "TB", "TC", "VA", "VB", "VC", "Z", "UE", "UO", "ET", "FF", "PS", "HI",
        "FY", "KE", "BI", "GE", "TE", "YH");
    ...
    final List<Map<String, String>> ainereaali = konvertoitu.stream().filter(k -> k.getAvain().equals("AINEREAALI"))
        .findFirst().get().getMetatiedot();
    assertEquals(14, ainereaali.size());

    final List<Map<String, String>> reaali = konvertoitu.stream().filter(k -> k.getAvain().equals("REAALI"))
        .findFirst().get().getMetatiedot();
    assertEquals(3, reaali.size());
    assertEquals("ET", reaali.stream().filter(m -> m.get("LISATIETO").equals("ET")).findFirst().get().get("ROOLI"));

    final List<Map<String, String>> pitka_kieli = konvertoitu.stream()
        .filter(k -> k.getAvain().equals("PITKA_KIELI")).findFirst().get().getMetatiedot();
    assertEquals(9, pitka_kieli.size());

    final List<Map<String, String>> keskipitka_kieli = konvertoitu.stream()
        .filter(k -> k.getAvain().equals("KESKIPITKA_KIELI")).findFirst().get().getMetatiedot();
    assertEquals(8, keskipitka_kieli.size());

    final List<Map<String, String>> lyhyt_kieli = konvertoitu.stream()
        .filter(k -> k.getAvain().equals("LYHYT_KIELI")).findFirst().get().getMetatiedot();
    assertEquals(10, lyhyt_kieli.size());

    final List<Map<String, String>> aidinkieli = konvertoitu.stream().filter(k -> k.getAvain().equals("AIDINKIELI"))
        .findFirst().get().getMetatiedot();
    assertEquals(9, aidinkieli.size());
  }

  @Test
  public void testaaSureKonvertointiKoetunnus() throws JsonSyntaxException, IOException {
    List<Oppija> oppijat = new Gson().fromJson(
        IOUtils.toString(new ClassPathResource("valintalaskenta/sure_yo_koetunnus.json").getInputStream()),
        new TypeToken<List<Oppija>>() {
        }.getType());
    ...
    assertEquals("21",
        va.getMetatiedot().stream().filter(m -> m.containsKey("ROOLI")).findFirst().get().get("ROOLI"));
  }

  @Test
  public void testMontaKertaaSamaKielikoodiYhdistetaan() throws IOException {
    List<Oppija> oppijat = new Gson().fromJson(
        IOUtils.toString(new ClassPathResource("monta_kielta.json").getInputStream()),
        new TypeToken<List<Oppija>>() {
        }.getType());
    ...
    List<AvainArvoDTO> arvot = OppijaToAvainArvoDTOConverter.convert(o.getOppijanumero(), o.getSuoritukset(),
        hakemus, null);

    Assert.assertEquals(4, arvot.size());
    String b1SVAinetunniste = arvot.stream()
        .filter(a -> a.getAvain().startsWith("PK_B1") && "SV".equals(a.getArvo())).findAny().get().getAvain()
        .replace("_OPPIAINE", "");
    String b1ENAinetunniste = arvot.stream()
        .filter(a -> a.getAvain().startsWith("PK_B1") && "EN".equals(a.getArvo())).findAny().get().getAvain()
        .replace("_OPPIAINE", "");
    assertEquals("9", arvot.stream().filter(a -> b1SVAinetunniste.equals(a.getAvain())).findAny().get().getArvo());
    assertEquals("10", arvot.stream().filter(a -> b1ENAinetunniste.equals(a.getAvain())).findAny().get().getArvo());
  }

  @Test
  public void testSuoritusmerkintaHuononpiKuinNumeerinen() {
    OppiaineArvosana a = ArvosanaToAvainArvoDTOConverter
        .parasArvosana(ImmutableList.of(new OppiaineArvosana("", "", false, 1, "4", "4-10"),
  ...
  public void testOppiainenumeroinninPalautusAntaaSamanNumeronSamanAineenArvosanoille() {
    ...
    List<OppiaineArvosana> en = r.stream().filter(o -> o.aine.startsWith("B") && "EN".equals(o.lisatieto))
        .collect(Collectors.toList());
    List<OppiaineArvosana> sv = r.stream().filter(o -> o.aine.startsWith("B") && "SV".equals(o.lisatieto))
        .collect(Collectors.toList());
    List<OppiaineArvosana> aEn = r.stream().filter(o -> o.aine.startsWith("A") && "EN".equals(o.lisatieto))
        .collect(Collectors.toList());
