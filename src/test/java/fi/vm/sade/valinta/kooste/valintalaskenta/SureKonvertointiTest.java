...
public class SureKonvertointiTest {
  ...
  public void testaaLukioKonvertointi() throws JsonSyntaxException, IOException {
    Oppija o =
        new Gson()
            .fromJson(
                IOUtils.toString(
                    new ClassPathResource("valintalaskenta/sure_pk_ja_yo.json").getInputStream()),
                new TypeToken<Oppija>() {}.getType());
    SuoritusJaArvosanat pk =
        o.getSuoritukset().stream()
            .filter(s -> new SuoritusJaArvosanatWrapper(s).isPerusopetus())
            .findFirst()
            .get();
    SuoritusJaArvosanat yo =
        o.getSuoritukset().stream()
    ...
    List<AvainArvoDTO> aa =
        OppijaToAvainArvoDTOConverter.convert(
            o.getOppijanumero(), o.getSuoritukset(), new HakemusDTO(), pmetrit);

    String a1ESAinetunniste =
        aa.stream()
            .filter(
                (AvainArvoDTO a) -> a.getAvain().startsWith("PK_A1") && "ES".equals(a.getArvo()))
            .findAny()
            .get()
            .getAvain()
            .replace("_OPPIAINE", "");
  ...
  public void testaaSureKonvertointiKaikkiAIneet() throws JsonSyntaxException, IOException {
    ...
    final List<Map<String, String>> ainereaali =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("AINEREAALI"))
            .findFirst()
            .get()
            .getMetatiedot();
    assertEquals(14, ainereaali.size());

    final List<Map<String, String>> reaali =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("REAALI"))
            .findFirst()
            .get()
            .getMetatiedot();
    ...
    assertEquals(
        "ET",
        reaali.stream()
            .filter(m -> m.get("LISATIETO").equals("ET"))
            .findFirst()
            .get()
            .get("ROOLI"));

    final List<Map<String, String>> pitka_kieli =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("PITKA_KIELI"))
            .findFirst()
            .get()
            .getMetatiedot();
    assertEquals(9, pitka_kieli.size());

    final List<Map<String, String>> keskipitka_kieli =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("KESKIPITKA_KIELI"))
            .findFirst()
            .get()
            .getMetatiedot();
    assertEquals(8, keskipitka_kieli.size());

    final List<Map<String, String>> lyhyt_kieli =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("LYHYT_KIELI"))
            .findFirst()
            .get()
            .getMetatiedot();
    assertEquals(10, lyhyt_kieli.size());

    final List<Map<String, String>> aidinkieli =
        konvertoitu.stream()
            .filter(k -> k.getAvain().equals("AIDINKIELI"))
            .findFirst()
            .get()
            .getMetatiedot();
  ...
  public void testaaSureKonvertointiKoetunnus() throws JsonSyntaxException, IOException {
    List<Oppija> oppijat =
        new Gson()
            .fromJson(
                IOUtils.toString(
                    new ClassPathResource("valintalaskenta/sure_yo_koetunnus.json")
                        .getInputStream()),
                new TypeToken<List<Oppija>>() {}.getType());
    ...
    AvainMetatiedotDTO va =
        konvertoitu.stream().filter(k -> k.getAvain().equals("VA")).findFirst().get();
    assertNotNull(va);

    assertEquals(
        "21",
        va.getMetatiedot().stream()
            .filter(m -> m.containsKey("ROOLI"))
            .findFirst()
            .get()
            .get("ROOLI"));
  }
  ...
  public void testMontaKertaaSamaKielikoodiYhdistetaan() throws IOException {
    ...
    assertEquals(
        "9",
        arvot.stream()
            .filter(a -> b1SVAinetunniste.equals(a.getAvain()))
            .findAny()
            .get()
            .getArvo());
    assertEquals(
        "10",
        arvot.stream()
            .filter(a -> b1ENAinetunniste.equals(a.getAvain()))
            .findAny()
            .get()
            .getArvo());
  }
    ...
    OppiaineArvosana e =
        new OppiaineArvosana(
            new Arvosana(
                ...
                new Arvio(),
                "EN"));
    List<OppiaineArvosana> r =
        ArvosanaToAvainArvoDTOConverter.palautaAinenumerointi(Stream.of(a, b, c, d, e))
            .collect(Collectors.toList());
    List<OppiaineArvosana> en =
        r.stream()
            .filter(o -> o.aine.startsWith("B") && "EN".equals(o.lisatieto))
            .collect(Collectors.toList());
    List<OppiaineArvosana> sv =
        r.stream()
            .filter(o -> o.aine.startsWith("B") && "SV".equals(o.lisatieto))
            .collect(Collectors.toList());
    List<OppiaineArvosana> aEn =
        r.stream()
            .filter(o -> o.aine.startsWith("A") && "EN".equals(o.lisatieto))
            .collect(Collectors.toList());
