...
public class ValintalaskentaTulosExcelTest {
  ...
  final HttpResourceBuilder.WebClientExposingHttpResource valintakoekutsutResource = new HttpResourceBuilder(
      getClass().getName()).address(root + "/valintalaskentaexcel/valintakoekutsut/aktivoi")
  ...
  final Haku haku = new Haku(HAKU1, new HashMap<>(), new HashSet<>(), null, null, null, null, null, null);
  ...
  public void testaaExcelinLuontiJsonLahteesta() throws Throwable {
    ...
    List<Hakemus> hakemuses = hakemusResource.gson().fromJson(listFull, new TypeToken<List<Hakemus>>() {
    }.getType());
    List<HakemusWrapper> hakemusWrappers = hakemuses.stream().map(HakuappHakemusWrapper::new)
        .collect(Collectors.toList());

    List<HakemusOsallistuminenDTO> osallistuminenDTOs = valintakoekutsutResource.gson().fromJson(osallistumiset,
        new TypeToken<List<HakemusOsallistuminenDTO>>() {
        }.getType());

    List<ValintakoeDTO> valintakoeDTOs = valintakoekutsutResource.gson().fromJson(valintakoe,
    ...
    try {
      ...
      lisatiedot.setValintakoeTunnisteet(
          Arrays.asList("7c0c20aa-c9a1-53eb-5e46-ca689b3625c0", "d579283e-ab61-e140-306c-7582a666fd85"));
      Response r = valintakoekutsutResource.getWebClient().query("hakuOid", "1.2.246.562.29.95390561488")
  ...
  public void testaaExcelinLuontiKaikkiOsallistuu() throws Throwable {
    ...
    try {
      ...
      MockValintalaskentaValintakoeAsyncResource.setHakemusOsallistuminenResult(Arrays.asList(
          hakemusOsallistuminen().setHakemusOid(HAKEMUS1).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE1).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS2).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE1).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS3).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE1, OsallistuminenDTO.EI_OSALLISTU).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS4).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE1, OsallistuminenDTO.EI_VAADITA).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS5).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE1, OsallistuminenDTO.VIRHE).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS1).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE2).build(),
          hakemusOsallistuminen().setHakemusOid(HAKEMUS2).setHakutoive(HAKUKOHDE1)
              .addOsallistuminen(SELVITETTY_TUNNISTE2).build()));
      MockValintaperusteetAsyncResource.setValintakokeetResult(Arrays.asList(
          valintakoe().setTunniste(TUNNISTE1).setNimi(VALINTAKOENIMI1)
              .setSelvitettyTunniste(SELVITETTY_TUNNISTE1).build(),
          valintakoe().setTunniste(TUNNISTE2).setNimi(VALINTAKOENIMI2).setKaikkiKutsutaan()
              .setSelvitettyTunniste(SELVITETTY_TUNNISTE2).build()));
      ...
      MockApplicationAsyncResource
          .setResult(Arrays.asList(hakemus().setOid(HAKEMUS2).build(), hakemus().setOid(HAKEMUS3).build(),
              hakemus().setOid(HAKEMUS4).build(), hakemus().setOid(HAKEMUS5).build()));
      MockValintalaskentaValintakoeAsyncResource.setResult(osallistumistiedot);

      DokumentinLisatiedot lisatiedot = new DokumentinLisatiedot();
      lisatiedot.setValintakoeTunnisteet(Arrays.asList(SELVITETTY_TUNNISTE1, SELVITETTY_TUNNISTE2));
      Response r = valintakoekutsutResource.getWebClient().query("hakuOid", HAKU1)
          .query("hakukohdeOid", HAKUKOHDE1).post(Entity.entity(lisatiedot, "application/json"));
      assertEquals(200, r.getStatus());
      ...
      assertTrue(rivit.stream().anyMatch(
          rivi -> rivi.getSolut().stream().anyMatch(r0 -> HAKEMUS1.equals(r0.toTeksti().getTeksti()))));
      assertTrue(rivit.stream().anyMatch(
          rivi -> rivi.getSolut().stream().anyMatch(r0 -> HAKEMUS2.equals(r0.toTeksti().getTeksti()))));
      // Ei osallistujat ei tule mukaan
      assertTrue(rivit.stream().anyMatch(
          rivi -> rivi.getSolut().stream().anyMatch(r0 -> HAKEMUS3.equals(r0.toTeksti().getTeksti()))));
      assertTrue(rivit.stream().anyMatch(
          rivi -> rivi.getSolut().stream().anyMatch(r0 -> HAKEMUS4.equals(r0.toTeksti().getTeksti()))));
      assertTrue(rivit.stream().anyMatch(
          rivi -> rivi.getSolut().stream().anyMatch(r0 -> HAKEMUS5.equals(r0.toTeksti().getTeksti()))));
    ...
    try {
      List<ValintakoeOsallistuminenDTO> osallistumistiedot = Arrays.asList(
          osallistuminen().setHakemusOid(HAKEMUS1).hakutoive().valinnanvaihe().valintakoe()
              .setTunniste(SELVITETTY_TUNNISTE1).setOsallistuu().build().build().build().build(),
      ...
      MockValintaperusteetAsyncResource.setValintakokeetResult(Arrays.asList(
          valintakoe().setTunniste(TUNNISTE1).setNimi(VALINTAKOENIMI1)
              .setSelvitettyTunniste(SELVITETTY_TUNNISTE1).build(),
          valintakoe().setTunniste(TUNNISTE2).setNimi(VALINTAKOENIMI2)
              .setSelvitettyTunniste(SELVITETTY_TUNNISTE2).build()));

      MockApplicationAsyncResource.setResultByOid(Arrays.asList(hakemus().setOid(HAKEMUS1).build(),
          hakemus().setOid(HAKEMUS2).build(), hakemus().setOid(HAKEMUS3).build(),
          hakemus().setOid(HAKEMUS4).build(), hakemus().setOid(HAKEMUS5).build()));
