...
public class ErillishakuResourceKayttajaPalauteTest {
  ...
  final HttpResourceBuilder.WebClientExposingHttpResource jsonResource =
      new HttpResourceBuilder(getClass().getName())
          .address(root + "/erillishaku/tuonti/ui")
          .buildExposingWebClientDangerously();
  final HttpResourceBuilder.WebClientExposingHttpResource excelResource =
      new HttpResourceBuilder(getClass().getName())
  ...
  public void tuontiVirheTyhjaTietoJoukkoPalaute() {
    final ProsessiId prosessiId =
        jsonClient()
            .post(
                Entity.json(new ErillishakuJson(emptyList()))
                //
                ,
                ProsessiId.class);
    ...
    assertTrue(
        odotaVirhettaTaiEpaonnistuTimeouttiin(prosessiId)
            ...
            .poikkeukset
            .contains(Poikkeus.koostepalvelupoikkeus(POIKKEUS_TYHJA_DATAJOUKKO)));
  }
  ...
  public void tuontiVirheitaTuoduissaTiedoissaPalaute() {
    ...
    final ProsessiId prosessiId =
        jsonClient()
            .post(
                Entity.json(viallinenJsonRiviPuuttuvillaTunnisteilla())
                //
                ,
                ProsessiId.class);
  ...
  public void tuontiVirheitaPuuttuviaPakollisiaKKExcelTietoja() {
    final ProsessiId prosessiId =
        excelClient()
            .post(
                Entity.entity(
                    ExcelTestData.kkHakuPuuttuviaPakollisiaTietoja(),
                    MediaType.APPLICATION_OCTET_STREAM),
                ProsessiId.class);

    Collection<Poikkeus> poikkeukset =
        odotaVirhettaTaiEpaonnistuTimeouttiin(prosessiId).poikkeukset;
