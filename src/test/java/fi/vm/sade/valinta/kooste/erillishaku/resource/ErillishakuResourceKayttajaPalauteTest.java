...
public class ErillishakuResourceKayttajaPalauteTest {
  ...
  final HttpResourceBuilder.WebClientExposingHttpResource jsonResource = new HttpResourceBuilder(getClass().getName())
      .address(root + "/erillishaku/tuonti/ui").buildExposingWebClientDangerously();
  final HttpResourceBuilder.WebClientExposingHttpResource excelResource = new HttpResourceBuilder(
      getClass().getName()).address(root + "/erillishaku/tuonti").buildExposingWebClientDangerously();
  ...
  public void tuontiVirheTyhjaTietoJoukkoPalaute() {
    final ProsessiId prosessiId = jsonClient().post(Entity.json(new ErillishakuJson(emptyList()))
    //
        , ProsessiId.class);
  ...
  public void tuontiVirheitaTuoduissaTiedoissaPalaute() {
    ...
    final ProsessiId prosessiId = jsonClient().post(Entity.json(viallinenJsonRiviPuuttuvillaTunnisteilla())
    //
        , ProsessiId.class);
    ...
    assertThat(odotaVirhettaTaiEpaonnistuTimeouttiin(prosessiId)
        // Odotetaan tyhjää datajoukko palautetta!
        .poikkeukset,
        equalTo(asList(Poikkeus.koostepalvelupoikkeus(POIKKEUS_VIALLINEN_DATAJOUKKO, asList(new Tunniste(
  ...
  public void tuontiVirheitaPuuttuviaPakollisiaKKExcelTietoja() {
    final ProsessiId prosessiId = excelClient().post(
        Entity.entity(ExcelTestData.kkHakuPuuttuviaPakollisiaTietoja(), MediaType.APPLICATION_OCTET_STREAM),
        ProsessiId.class);
  ...
  public void tuontiVirheHakemuspalveluKutsussaPalaute() {
    try {
      ...
      final ProsessiId prosessiId = excelClient().post(Entity
          .entity(ExcelTestData.kkHakuToisenAsteenValintatuloksella(), MediaType.APPLICATION_OCTET_STREAM),
          ProsessiId.class);
      assertThat(odotaVirhettaTaiEpaonnistuTimeouttiin(prosessiId)
  ...
  public void tuontiVirheHenkilopalveluKutsussaPalaute() {
    try {
      ...
      final ProsessiId prosessiId = jsonClient().post(Entity.json(new ErillishakuJson(asList(laillinenRivi())))
      //
          , ProsessiId.class);
      ...
      assertThat(odotaVirhettaTaiEpaonnistuTimeouttiin(prosessiId)
          // Odotetaan hakemuspalvelun epäonnistumisesta johtuvaa palautetta!
          .poikkeukset,
          equalTo(asList(Poikkeus.oppijanumerorekisteripoikkeus(POIKKEUS_OPPIJANUMEROREKISTERIN_VIRHE))));
    } finally {
  ...
  private Prosessi odotaVirhettaTaiEpaonnistuTimeouttiin(final ProsessiId prosessiId) {
    return DokumenttiProsessiPoller.pollDokumenttiProsessi(root, prosessiId,
        prosessiResponse -> !prosessiResponse.poikkeukset.isEmpty());
  }

  private WebClient jsonClient() {
    return jsonResource.getWebClient().query("hakutyyppi", KORKEAKOULU.toString()).query("hakuOid", hakuOid)
        .query("hakikohdeOid", hakukohdeOid).query("tarjoajaOid", tarjoajaOid)
        .query("valintatapajonoOid", valintatapajonoOid).query("valintatapajononNimi", "varsinainen jono")
        .header("If-Unmodified-Since", "Tue, 3 Jun 2008 11:05:30 GMT").accept(MediaType.APPLICATION_JSON);
  }

  private WebClient excelClient() {
    return excelResource.getWebClient().query("hakutyyppi", KORKEAKOULU.toString()).query("hakuOid", hakuOid)
        .query("hakikohdeOid", hakukohdeOid).query("tarjoajaOid", tarjoajaOid)
        .query("valintatapajonoOid", valintatapajonoOid).query("valintatapajononNimi", "varsinainen jono")
