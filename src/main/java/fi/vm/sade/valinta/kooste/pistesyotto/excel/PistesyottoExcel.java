...
public class PistesyottoExcel {
  ...
  private static final Collection<String> VAIHTOEHDOT = Arrays.asList(MERKITSEMATTA, OSALLISTUI, EI_OSALLISTUNUT,
      EI_VAADITA);
  private static final Map<String, String> VAIHTOEHDOT_KONVERSIO = new KonversioBuilder()
      .addKonversio(StringUtils.EMPTY, MERKITSEMATTA).addKonversio(VAKIO_MERKITSEMATTA, MERKITSEMATTA)
      .addKonversio(VAKIO_OSALLISTUI, OSALLISTUI).addKonversio(VAKIO_EI_VAADITA, EI_VAADITA)
      .addKonversio(VAKIO_EI_OSALLISTUNUT, EI_OSALLISTUNUT).build();
  private static final Map<String, String> VAIHTOEHDOT_TAKAISINPAIN_KONVERSIO = new KonversioBuilder()
      .addKonversio(StringUtils.EMPTY, VAKIO_MERKITSEMATTA).addKonversio(MERKITSEMATTA, VAKIO_MERKITSEMATTA)
      .addKonversio(OSALLISTUI, VAKIO_OSALLISTUI).addKonversio(EI_VAADITA, VAKIO_EI_VAADITA)
  ...
  private static final Collection<String> TOTUUSARVO_KIELIKOE = Arrays.asList(TYHJA, HYVAKSYTTY, HYLATTY);
  private static final Map<String, String> TOTUUSARVO_KIELIKOE_KONVERSIO = new KonversioBuilder()
      .addKonversio(HYVAKSYTTY, Boolean.TRUE.toString()).addKonversio(HYLATTY, Boolean.FALSE.toString())
  ...
  private static final Map<String, String> TOTUUSARVO_KONVERSIO = new KonversioBuilder()
      .addKonversio(KYLLA, Boolean.TRUE.toString()).addKonversio(EI, Boolean.FALSE.toString())
      .addKonversio(TYHJA, null).build();
  ...
  private Map<String, Map<String, ValintakoeDTO>> valintakoeOidit(String hakukohdeOid,
  ...
  public PistesyottoExcel(String hakuOid, String hakukohdeOid, String tarjoajaOid, String hakuNimi,
      String hakukohdeNimi, String tarjoajaNimi, Optional<String> aikaleima,
      Collection<HakemusWrapper> hakemukset, Set<String> kaikkiKutsutaanTunnisteet,
      Collection<String> valintakoeTunnisteet, List<ValintakoeOsallistuminenDTO> osallistumistiedot,
      List<ValintaperusteDTO> valintaperusteet, List<ApplicationAdditionalDataDTO> kaikkiPistetiedot,
    ...
    final Set<String> osallistujat = osallistumistiedot.stream().filter(
        o -> OsallistujatPredicate.vainOsallistujatTunnisteella(hakukohdeOid, valintakoeTunnisteet).apply(o))
        .map(ValintakoeOsallistuminenDTO::getHakemusOid).collect(Collectors.toSet());

    final List<String> tunnisteet = valintaperusteet.stream().map(ValintaperusteDTO::getTunniste)
    ...
    rivit.add(Rivi.tyhjaRivi());
    rivit.add(new RiviBuilder().addTyhja().addTyhja().addTyhja().addTyhja()
    ...
    final RiviBuilder otsikkoRiviBuilder = new RiviBuilder().addKeskitettyTeksti("Hakemus OID")
        .addKeskitettyTeksti("Tiedot").addKeskitettyTeksti("Henkilötunnus").addKeskitettyTeksti("Syntymäaika");
    ...
    Predicate<ValintakoeDTO> osallistuuValintakokeeseen = valintakoe -> (valintakoe != null
        && Osallistuminen.OSALLISTUU.equals(Optional.ofNullable(valintakoe.getOsallistuminenTulos())
            .orElse(new OsallistuminenTulosDTO()).getOsallistuminen()));
    Map<String, HakemusWrapper> oidToWrapper = hakemukset.stream()
        .collect(Collectors.toMap(HakemusWrapper::getOid, h -> h));
    List<ApplicationAdditionalDataDTO> pistetiedotHakuAppistaLoytyvilleHakemuksille = filteroiPistetiedoistaPoisNeJoilleEiLoydyAktiivistaHakemustaHakuAppista(
        hakemukset, kaikkiPistetiedot).sorted((a0, a1) -> {
    ...
    for (ApplicationAdditionalDataDTO data : pistetiedotHakuAppistaLoytyvilleHakemuksille) {
      ...
      Map<String, ValintakoeDTO> tunnisteDTO = Optional.ofNullable(tunnisteValintakoe.get(data.getOid()))
      ...
      for (ValintaperusteDTO valintaperuste : valintaperusteet) {
        ValintakoeDTO valintakoe = Optional.ofNullable(tunnisteDTO.get(valintaperuste.getTunniste()))
        ...
        if (syotettavissaKaikille || (mahdollinenOsallistuja && osallistuuValintakokeeseen.test(valintakoe))) {
          ...
          if (Funktiotyyppi.LUKUARVOFUNKTIO.equals(valintaperuste.getFunktiotyyppi())) {
            ...
            String value = StringUtils
                .trimToEmpty(data.getAdditionalData().get(valintaperuste.getTunniste()));
            ...
            s.add(new TekstiArvo(
                data.getAdditionalData().get(StringUtils.trimToEmpty(valintaperuste.getTunniste())),
                false));
          }
          s.add(new MonivalintaArvo(
              VAIHTOEHDOT_KONVERSIO.get(StringUtils.trimToEmpty(
                  data.getAdditionalData().get(valintaperuste.getOsallistuminenTunniste()))),
              VAIHTOEHDOT));
  ...
  private Stream<ApplicationAdditionalDataDTO> filteroiPistetiedoistaPoisNeJoilleEiLoydyAktiivistaHakemustaHakuAppista(
      Collection<HakemusWrapper> hakemukset, List<ApplicationAdditionalDataDTO> pistetiedot) {
    Set<String> hakuAppistaLoytyvatHakemusOidit = hakemukset.stream().map(HakemusWrapper::getOid)
  ...
  private Collection<PistesyottoDataArvo> getPistesyotonDataArvot(List<ValintaperusteDTO> valintaperusteet) {
    ...
    for (ValintaperusteDTO valintaperuste : valintaperusteet) {
      LOG.info("Tunniste=={}, osallistumisentunniste={}", valintaperuste.getTunniste(),
      ...
      if (Funktiotyyppi.LUKUARVOFUNKTIO.equals(valintaperuste.getFunktiotyyppi())) {
        ...
        if (isKielikoe(valintaperuste)) {
          dataArvot.add(new BooleanDataArvo(TOTUUSARVO_KIELIKOE_KONVERSIO, VAIHTOEHDOT_TAKAISINPAIN_KONVERSIO,
              StringUtils.trimToEmpty(valintaperuste.getTunniste()), VAKIO_OSALLISTUI,
              StringUtils.trimToEmpty(valintaperuste.getOsallistuminenTunniste())));
        } else {
          dataArvot.add(new BooleanDataArvo(TOTUUSARVO_KONVERSIO, VAIHTOEHDOT_TAKAISINPAIN_KONVERSIO,
              StringUtils.trimToEmpty(valintaperuste.getTunniste()), VAKIO_OSALLISTUI,
              StringUtils.trimToEmpty(valintaperuste.getOsallistuminenTunniste())));
        }
      } else {
  ...
  private List<String> createValintakokeet(List<ValintaperusteDTO> valintaperusteet) {
    return valintaperusteet.stream().map(valintaperuste -> {
      if (Funktiotyyppi.LUKUARVOFUNKTIO.equals(valintaperuste.getFunktiotyyppi())
          && !StringUtils.isBlank(valintaperuste.getMin()) && !StringUtils.isBlank(valintaperuste.getMax())) {
  ...
  private boolean isKielikoe(ValintaperusteDTO valintaperuste) {
    if (valintaperuste == null || valintaperuste.getTunniste() == null)
      return false;
