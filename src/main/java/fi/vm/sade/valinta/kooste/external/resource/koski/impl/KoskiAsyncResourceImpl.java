...
public class KoskiAsyncResourceImpl implements KoskiAsyncResource {
  ...
  private static final Type INPUT_TYPE = new TypeToken<List<String>>() {
  }.getType();
  private static final Type OUTPUT_TYPE = new TypeToken<List<KoskiOppija>>() {
  }.getType();
  ...
  public CompletableFuture<JsonElement> findVersionOfOpiskeluoikeus(String opiskeluoikeudenOid, int versionumero) {
    return httpClient
        .getResponse(urlConfiguration.url("koski.opiskeluoikeuden.versio", opiskeluoikeudenOid, versionumero),
            Duration.ofMinutes(1), addBasicAuthenticationCredentials)
  ...
  private CompletableFuture<Set<KoskiOppija>> batchedPostOppijasFuture(List<String> oppijanumerot, String url) {
    ...
    return CompletableFutureUtil.sequence(oidBatches.stream().map(oidBatch -> {
      ...
      return httpClient.<List<String>, List<KoskiOppija>>postJson(url, Duration.ofMinutes(5), oidBatch,
          INPUT_TYPE, OUTPUT_TYPE, addBasicAuthenticationCredentials);
    }).collect(Collectors.toList())).thenApplyAsync(
        (List<List<KoskiOppija>> r) -> r.stream().flatMap(List::stream).collect(Collectors.toSet()));
  }
  ...
  private void debugLogOpiskeluoikeusVersiot(KoskiOppija oppija) {
    oppija.getOpiskeluoikeudet().forEach(o -> {
      ...
      LOG.debug(String.format("Oppijan %s opiskeluoikeus %s : aikaleima=%s , versionumero=%s",
          oppija.getOppijanumero(), opiskeluoikeus.get("oid").getAsString(),
          opiskeluoikeus.get("aikaleima").getAsString(), opiskeluoikeus.get("versionumero").getAsInt()));
    });
  }
}
