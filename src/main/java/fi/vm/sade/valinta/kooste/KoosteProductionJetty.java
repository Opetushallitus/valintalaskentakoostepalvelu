...
public class KoosteProductionJetty {
    private static final int SERVICE_PORT_IN_ECS_CONFIGURATION = 8080;
    private static final Logger LOG = LoggerFactory.getLogger(KoosteProductionJetty.class);
    ...
    public static void start(WebAppContext webAppContext, Server server) {
        try {
            if (server.isStopped()) {
                webAppContext.setContextPath("/valintalaskentakoostepalvelu");
