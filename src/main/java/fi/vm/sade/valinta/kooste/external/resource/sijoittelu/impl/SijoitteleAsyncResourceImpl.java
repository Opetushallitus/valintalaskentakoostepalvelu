...
public class SijoitteleAsyncResourceImpl extends UrlConfiguredResource implements SijoitteleAsyncResource {
  ...
  public void sijoittele(String hakuOid, Consumer<String> callback, Consumer<Throwable> failureCallback) {
    ...
    // Luodaan sijoitteluajo, saadaan palautusarvona sen id, jota käytetään
    // pollattaessa toista
    // rajapintaa.
    String luontiUrl = getUrl("sijoittelu-service.sijoittele", hakuOid);
    try {
      sijoitteluajoId = this.<Long>getAsObservableLazily(luontiUrl, Long.class,
          client -> client.accept(MediaType.WILDCARD_TYPE)).timeout(30, SECONDS).blockingFirst();
    } catch (Exception e) {
      LOGGER.error(String.format("(Haku %s) sijoittelun rajapintakutsu epäonnistui", hakuOid), e);
    }
    // Jos rajapinta palauttaa -1 tai kutsu epäonnistuu, uutta sijoitteluajoa ei
    // luotu. Ei aloiteta
    // pollausta.
    if (sijoitteluajoId == -1) {
      String msg = String.format("Uuden sijoittelun luonti haulle %s epäonnistui: luontirajapinta palautti -1",
          hakuOid);
      LOGGER.error(msg);
    ...
    LOGGER.info("(Haku: {}) Sijoittelu on käynnistynyt id:llä {}. Pollataan kunnes se on päättynyt.", hakuOid,
        sijoitteluajoId);
    ...
    while (!done.get()) {
      ...
      try {
        status = this
            .<String>getAsObservableLazily(pollingUrl, String.class,
                webClient -> webClient.accept(MediaType.WILDCARD_TYPE))
            .timeout(15, SECONDS).blockingFirst();
        ...
        LOGGER.error(String.format("Sijoittelussa %s haulle %s tapahtui virhe", sijoitteluajoId, hakuOid), e);
