...
public class ValintakoelaskentaPalvelukutsu extends
	...
	private LaskeDTO muodostaLaskeDTO() {
		return new LaskeDTO(hakukohdeOid, muodostaHakemuksetDTO(),
				valintaperusteetPalvelukutsu.getValintaperusteet());
	}

	private List<HakemusDTO> muodostaHakemuksetDTO() {
		List<Hakemus> hakemukset = hakemuksetPalvelukutsu.getHakemukset();
		final Map<String, ApplicationAdditionalDataDTO> appData = lisatiedotPalvelukutsu
				.getLisatiedot()
				.parallelStream()
				.collect(
				//
		...
		List<HakemusDTO> hakemusDtot = hakemuksetPalvelukutsu.getHakemukset()
				.parallelStream().map(h -> Converter.hakemusToHakemusDTO(h))
				.collect(Collectors.toList());
		return hakemusDtot;
	}
