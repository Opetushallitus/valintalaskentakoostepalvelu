...
public class KoskiOpiskeluoikeusHistoryService {
  ...
  LocalDate etsiKoskiDatanLeikkuriPvm(List<ValintaperusteetDTO> valintaperusteetDTOS, String hakukohdeOid) {
    List<String> leikkuriPvmMerkkijonot = etsiTutkintojenIterointiFunktioKutsut(valintaperusteetDTOS)
        .flatMap(tutkintojenIterointiFunktio -> tutkintojenIterointiFunktio.getSyoteparametrit().stream()
            .filter(parametri -> ITEROIAMMATILLISETTUTKINNOT_LEIKKURIPVM_PARAMETRI
                .equals(parametri.getAvain()) && StringUtils.isNotBlank(parametri.getArvo()))
            .map(SyoteparametriDTO::getArvo))
        .collect(Collectors.toList());
    LocalDate kaytettavaLeikkuriPvm = leikkuriPvmMerkkijonot.stream()
        .map(pvm -> LocalDate.parse(pvm, FINNISH_DATE_FORMAT)).max(Comparator.naturalOrder())
  ...
  static Stream<ValintaperusteetFunktiokutsuDTO> etsiTutkintojenIterointiFunktioKutsut(
    ...
    return valintaperusteetDTOS.stream()
        ...
        .flatMap(kriteeri -> etsiFunktiokutsutRekursiivisesti(kriteeri.getFunktiokutsu(),
  ...
  private static List<ValintaperusteetFunktiokutsuDTO> etsiFunktiokutsutRekursiivisesti(
      ValintaperusteetFunktiokutsuDTO juuriFunktioKutsu, Predicate<ValintaperusteetFunktiokutsuDTO> predikaatti) {
    ...
    tulokset.addAll(juuriFunktioKutsu.getFunktioargumentit().stream().flatMap(
        argumentti -> etsiFunktiokutsutRekursiivisesti(argumentti.getFunktiokutsu(), predikaatti).stream())
  ...
  void haeVanhemmatOpiskeluoikeudetTarvittaessa(Set<KoskiOppija> koskioppijat, LocalDate leikkuriPvm) {
    List<KoskiOppija> oppijatJoillaOnLiianUusiaOpiskeluoikeuksia = koskioppijat.stream()
        .filter(o -> Lists.newArrayList(o.getOpiskeluoikeudet()).stream().anyMatch(opiskeluoikeus -> {
    ...
    if (!oppijatJoillaOnLiianUusiaOpiskeluoikeuksia.isEmpty()) {
      LOG.info(String.format(
          ...
          koskioppijat.size(), oppijatJoillaOnLiianUusiaOpiskeluoikeuksia.size(), leikkuriPvm,
          koskiPyyntojenRinnakkaisuus));
      Lists.partition(oppijatJoillaOnLiianUusiaOpiskeluoikeuksia, koskiPyyntojenRinnakkaisuus)
          .forEach(yhdenEränOppijat -> haeLiianUusienOpiskeluoikeuksienTilalleRiittävänVanhat(leikkuriPvm,
              yhdenEränOppijat));
    }
  }

  private void haeLiianUusienOpiskeluoikeuksienTilalleRiittävänVanhat(LocalDate leikkuriPvm,
      List<KoskiOppija> yhdenEränOppijat) {
  ...
  private CompletableFuture<JsonArray> haeOpiskeluoikeuksistaPaivanMukainenVersio(KoskiOppija koskiOppija,
      LocalDate leikkuriPvm) {
    return CompletableFutureUtil
        .sequence(Lists.newArrayList(koskiOppija.getOpiskeluoikeudet()).stream().map(opiskeluoikeus -> {
          if (OpiskeluoikeusJsonUtil.onUudempiKuin(leikkuriPvm, opiskeluoikeus)) {
            ...
            LOG.info(String.format(
                ...
                koskiOppija.getOppijanumero(), opiskeluoikeudenOid, versionumero, aikaleima,
                FINNISH_DATE_FORMAT.format(leikkuriPvm)));
            return haePaivamaaranMukainenVersio(koskiOppija, leikkuriPvm,
  ...
  private CompletableFuture<Optional<JsonElement>> haePaivamaaranMukainenVersio(KoskiOppija koskiOppija,
      LocalDate leikkuriPvm, CompletableFuture<JsonElement> opiskeluoikeusF) {
    return opiskeluoikeusF.thenComposeAsync(opiskeluoikeus -> {
      ...
      if (ohittavatOpiskeluoikeudetOideittain.containsKey(opiskeluoikeudenOid)) {
        LOG.warn(String.format("Ohitetaan oppijan %s opiskeluoikeus %s konfiguraatiosta tulevalla arvolla.",
            koskiOppija.getOppijanumero(), opiskeluoikeudenOid));
        return CompletableFuture
            .completedFuture(Optional.of(ohittavatOpiskeluoikeudetOideittain.get(opiskeluoikeudenOid)));
      }
      ...
      if (versionumero <= 1) {
        LOG.info(String.format(
            ...
            koskiOppija.getOppijanumero(), opiskeluoikeudenOid, aikaleima,
            FINNISH_DATE_FORMAT.format(leikkuriPvm), versionumero));
        return CompletableFuture.completedFuture(Optional.empty());
      }
      CompletableFuture<JsonElement> edellisenVersionOpiskeluoikeus = koskiAsyncResource
          .findVersionOfOpiskeluoikeus(opiskeluoikeudenOid, versionumero - 1);
  ...
  private static Map<String, JsonElement> lueOhitettavatOpiskeluoikeudetKonfiguraatiosta() {
    try {
      ...
      File ohitusTiedostojenHakemisto = new File(
          String.format("%s%soph-configuration%svalintalaskentakoostepalvelu%skoski-ohitukset", kotihakemisto,
      ...
      return Arrays.stream(ohitustiedostot).collect(Collectors.toMap(t -> t.getName().replace(".json", ""), t -> {
