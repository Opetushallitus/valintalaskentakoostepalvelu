...
public class PistesyottoResource {
  ...
  public void koostaPistetiedotYhdelleHakemukselle(@PathParam("hakemusOid") String hakemusOid,
    ...
    response.setTimeoutHandler(handler -> {
      ...
      handler.resume(Response.serverError()
          .entity("koostaPistetiedotYhdelleHakemukselle-palvelukutsu on aikakatkaistu").build());
    });
    Observable.zip(
        authorityCheckService.getAuthorityCheckForRoles(
            asList("ROLE_APP_HAKEMUS_READ_UPDATE", "ROLE_APP_HAKEMUS_READ", "ROLE_APP_HAKEMUS_CRUD",
                "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD")),
        ...
        (authorityCheck, pistetiedotHakukohteittain) -> {
          ...
          if (hakutoiveOids.stream().anyMatch(authorityCheck)) {
            return Response.ok().header("Content-Type", "application/json")
                .entity(pistetiedotHakukohteittain).build();
          } else {
        ...
        }).subscribe(response::resume, error -> {
  ...
  public void tallennaKoostetutPistetiedotHakemukselle(@PathParam("hakemusOid") String hakemusOid,
      ApplicationAdditionalDataDTO pistetiedot, @Suspended final AsyncResponse response) {
    final AuditSession auditSession = createAuditSession(httpServletRequestJaxRS);
    ...
    response.setTimeoutHandler(handler -> {
      LOG.error(
          ...
          hakemusOid);
      handler.resume(Response.serverError()
          .entity("tallennaKoostetutPistetiedotHakemukselle-palvelukutsu on aikakatkaistu").build());
    });

    if (!hakemusOid.equals(pistetiedot.getOid())) {
      String errorMessage = String.format("URLissa tuli hakemusOid %s , mutta PUT-datassa hakemusOid %s",
    ...
    Observable<HakemusWrapper> hakemusO = Observable
        .fromFuture(ataruAsyncResource.getApplicationsByOids(Collections.singletonList(hakemusOid)))
        .flatMap(hakemukset -> {
    ...
    Observable.merge(Observable.zip(
        authorityCheckService.getAuthorityCheckForRoles(asList("ROLE_APP_HAKEMUS_READ_UPDATE",
            "ROLE_APP_HAKEMUS_CRUD", "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD")),
        hakemusO, (authorityCheck, hakemus) -> {
          Collection<String> hakutoiveOids = hakemus.getHakutoiveOids();
          if (hakutoiveOids.stream().anyMatch(authorityCheck)) {
            return pistesyottoKoosteService.tallennaKoostetutPistetiedotHakemukselle(pistetiedot,
                ifUnmodifiedSince, auditSession);
          } else {
            String msg = String.format(
                "Käyttäjällä %s ei ole oikeuksia käsitellä hakukohteisiin %s hakeneen hakemuksen %s pistetietoja",
                auditSession.getPersonOid(), hakutoiveOids, hakemusOid);
            return Observable.error(new ForbiddenException(msg,
  ...
  public void koostaPistetiedotHakemuksille(@PathParam("hakuOid") String hakuOid,
      @PathParam("hakukohdeOid") String hakukohdeOid, @Suspended final AsyncResponse response) {
    final AuditSession auditSession = createAuditSession(httpServletRequestJaxRS);
    response.setTimeout(120L, TimeUnit.SECONDS);
    response.setTimeoutHandler(handler -> {
      LOG.error(
      ...
      handler.resume(Response.serverError().entity("koostaPistetiedotHakemuksille-palvelukutsu on aikakatkaistu")
          .build());
    });
    authorityCheckService
        .getAuthorityCheckForRoles(asList("ROLE_APP_HAKEMUS_READ_UPDATE", "ROLE_APP_HAKEMUS_READ",
            "ROLE_APP_HAKEMUS_CRUD", "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD"))
        .switchMap(authorityCheck -> {
          if (authorityCheck.test(hakukohdeOid)) {
            return Observable.fromFuture(pistesyottoKoosteService
                ...
                .thenApplyAsync(pistetiedot -> {
                  ...
                  return Response.ok().header("Content-Type", "application/json").entity(pistetiedot)
                      .build();
                }));
          } else {
            String msg = String.format(
  ...
  private Optional<String> ifUnmodifiedSinceFromHeader() {
    ...
    return list(h.getHeaderNames()).stream().map(String::toLowerCase)
        .filter(IF_UNMODIFIED_SINCE.toLowerCase()::equals).map(h::getHeader).findAny();
  }
  ...
  public void tallennaKoostetutPistetiedot(@PathParam("hakuOid") String hakuOid,
      @PathParam("hakukohdeOid") String hakukohdeOid, List<ApplicationAdditionalDataDTO> pistetiedot,
      @Suspended final AsyncResponse response) {
    final AuditSession auditSession = createAuditSession(httpServletRequestJaxRS);
    ...
    response.setTimeoutHandler(handler -> {
      LOG.error(
          ...
          hakuOid, hakukohdeOid);
      handler.resume(Response.serverError().entity("tallennaKoostetutPistetiedot-palvelukutsu on aikakatkaistu")
          .build());
    });
    authorityCheckService.getAuthorityCheckForRoles(asList("ROLE_APP_HAKEMUS_READ_UPDATE", "ROLE_APP_HAKEMUS_CRUD",
        "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD")).flatMap(authorityCheck -> {
          if (authorityCheck.test(hakukohdeOid)) {
          ...
          return Observable.error(new ForbiddenException(msg,
            ...
            .flatMap(hakemukset -> {
              ...
              Set<String> eiHakukohteeseenHakeneet = pistetiedot.stream()
                  ...
                  .filter(oid -> !hakukohteenHakemusOidit.contains(oid)).collect(Collectors.toSet());
        ...
        .flatMap(x -> Observable.fromFuture(pistesyottoKoosteService.tallennaKoostetutPistetiedot(hakuOid,
            hakukohdeOid, ifUnmodifiedSince, pistetiedot, auditSession)))
        .subscribe(x -> {
          if (x.isEmpty()) {
            response.resume(Response.noContent().build());
          } else {
  ...
  public void vienti(@QueryParam("hakuOid") String hakuOid, @QueryParam("hakukohdeOid") String hakukohdeOid,
    ...
    asyncResponse.setTimeoutHandler(handler -> {
      ...
      handler.resume(Response.serverError().entity("vienti-palvelukutsu on aikakatkaistu").build());
    });
    ...
    authorityCheckService
        ...
        .flatMap(authorityCheck -> {
          if (authorityCheck.test(hakukohdeOid)) {
            DokumenttiProsessi prosessi = new DokumenttiProsessi("Pistesyöttö", "vienti", hakuOid,
                asList(hakukohdeOid));
        ...
        }).subscribe(id -> asyncResponse.resume(Response.ok(id).build()), error -> {
  ...
  public void tuonti(@QueryParam("hakuOid") String hakuOid, @QueryParam("hakukohdeOid") String hakukohdeOid,
      InputStream file, @Suspended AsyncResponse asyncResponse) {
    asyncResponse.setTimeout(120L, TimeUnit.SECONDS);
    asyncResponse.setTimeoutHandler(handler -> {
      LOG.error("tuonti-palvelukutsu on aikakatkaistu: POST /tuonti");
      handler.resume(Response.serverError().entity("tuonti-palvelukutsu on aikakatkaistu").build());
    });

    try {
      final AuditSession auditSession = createAuditSession(httpServletRequestJaxRS);
      Observable<Object> authCheck = authorityCheckService
          .getAuthorityCheckForRoles(asList("ROLE_APP_HAKEMUS_READ_UPDATE", "ROLE_APP_HAKEMUS_CRUD",
              "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD"))
          .flatMap(authorityCheck -> {
            if (authorityCheck.test(hakukohdeOid)) {
              return Observable.just("OK");
      ...
      Observable<Set<TuontiErrorDTO>> map = authCheck.flatMap(x -> {
        DokumenttiProsessi prosessi = new DokumenttiProsessi("Pistesyöttö", "tuonti", hakuOid,
            singletonList(hakukohdeOid));
        dokumenttiKomponentti.tuoUusiProsessi(prosessi);
        ...
        dokumenttiAsyncResource
            .tallenna(uuid, "pistesyotto.xlsx", expirationTime, tags, "application/octet-stream",
        ...
        return tuontiService.tuo(auditSession, hakuOid, hakukohdeOid, prosessi,
            new ByteArrayInputStream(xlsx.toByteArray()));
      });

      map.subscribe(failedIds -> {
        ...
        if (error instanceof PistesyotonTuontivirhe) {
          ...
          LOG.warn(String.format(
              "tallennaKoostetutPistetiedot epäonnistui, vaikuttaa huonolta syötteeltä: " + "%s",
  ...
  public void ulkoinenTuonti(@QueryParam("hakuOid") String hakuOid,
      @QueryParam("valinnanvaiheOid") String valinnanvaiheOid, List<HakemusDTO> hakemukset,
      @Suspended AsyncResponse asyncResponse) {
    try {
      final AuditSession auditSession = createAuditSession(httpServletRequestJaxRS);
      if (hakemukset == null || hakemukset.isEmpty()) {
        asyncResponse.resume(Response.serverError()
            .entity("Ulkoinen pistesyotto API requires at least one hakemus").build());
      } else {
        ...
        asyncResponse.setTimeoutHandler(asyncResponse1 -> {
          ...
          asyncResponse1.resume(Response.serverError()
              .entity("Ulkoinen pistesyotto -palvelukutsu on aikakatkaistu").build());
        });

        authorityCheckService
            .getAuthorityCheckForRoles(asList("ROLE_APP_HAKEMUS_READ_UPDATE", "ROLE_APP_HAKEMUS_CRUD",
                "ROLE_APP_HAKEMUS_LISATIETORU", "ROLE_APP_HAKEMUS_LISATIETOCRUD"))
            .subscribe(authorityCheck -> {
