...
public class ValintaTulosServiceProxyResource {
  ...
  public void valintatuloksetIlmanTilaaHakijalle(@PathParam("hakuOid") String hakuOid,
      @PathParam("hakukohdeOid") String hakukohdeOid, @QueryParam("valintatapajonoOid") String valintatapajonoOid,
    ...
    valintaTulosServiceResource.findValintatuloksetIlmanHakijanTilaa(hakuOid, hakukohdeOid).map(valintatulokset -> {
      if (StringUtils.isBlank(valintatapajonoOid)) {
        return valintatulokset;
      } else {
        return valintatulokset.stream().filter(v -> valintatapajonoOid.equals(v.getValintatapajonoOid()))
            .collect(Collectors.toList());
      }
    }).map(valintatulokset -> Response.ok(valintatulokset).type(MediaType.APPLICATION_JSON_TYPE).build())
        .subscribe(asyncResponse::resume, error -> {
          LOG.error("Valintatulosten haku ilman valintatuloksen tilaa valinta-tulos-servicestä epäonnistui",
              error);
          respondWithError(asyncResponse,
              "ValintatulosserviceProxy -palvelukutsu epäonnistui virheeseen: " + error.getMessage());
        });
  }
  ...
  @Consumes("application/json")
  public void tietoSiitaEttaVastaanottoAikarajaOnMennyt(@PathParam("hakuOid") String hakuOid,
      @PathParam("hakukohdeOid") String hakukohdeOid, Set<String> hakemusOids,
      @Suspended AsyncResponse asyncResponse) {
    setAsyncTimeout(asyncResponse, String.format(
        "ValintatulosserviceProxy -palvelukutsu vastaanottoaikarajojen menneeksi on aikakatkaistu: /myohastyneet/haku/%s/hakukohde/%s",
        hakuOid, hakukohdeOid));

    valintaTulosServiceResource.findVastaanottoAikarajaMennyt(hakuOid, hakukohdeOid, hakemusOids).map(
        aikarajaMennytDtos -> Response.ok(aikarajaMennytDtos).type(MediaType.APPLICATION_JSON_TYPE).build())
        .subscribe(asyncResponse::resume, error -> {
          LOG.error("Vastaanottoaikarajojen haku valinta-tulos-servicestä haun " + hakuOid + " hakukohteelle "
              + hakukohdeOid + " epäonnistui", error);
          respondWithError(asyncResponse,
              "ValintatulosserviceProxy -palvelukutsu epäonnistui virheeseen: " + error.getMessage());
        });
  }
  ...
  @Consumes("application/json")
  public void vastaanottotilaHakijalle(@PathParam("hakuOid") String hakuOid,
      @PathParam("hakukohdeOid") String hakukohdeOid, @PathParam("valintatapajonoOid") String valintatapajonoOid,
      Set<String> hakemusOids, @Suspended AsyncResponse asyncResponse) {
    setAsyncTimeout(asyncResponse, String.format(
    ...
    valintaTulosServiceResource.findTilahakijalle(hakuOid, hakukohdeOid, valintatapajonoOid, hakemusOids)
        ...
        .subscribe(asyncResponse::resume, error -> {
          LOG.error(
              "Hakijan tilojen haku valinta-tulos-servicestä haun " + hakuOid + " hakukohteen "
                  + hakukohdeOid + " valintatapajonolle " + valintatapajonoOid + " epäonnistui",
              error);
          respondWithError(asyncResponse,
              "ValintatulosserviceProxy -palvelukutsu epäonnistui virheeseen: " + error.getMessage());
        });
  ...
  @Consumes("application/json")
  public void hakemuksenSijoittelunTulos(@PathParam("hakemusOid") String hakemusOid,
      @PathParam("hakuOid") String hakuOid, @PathParam("hakukohdeOid") String hakukohdeOid,
      @PathParam("valintatapajonoOid") String valintatapajonoOid, @Suspended AsyncResponse asyncResponse) {
    setAsyncTimeout(asyncResponse, String.format(
    ...
    valintaTulosServiceResource.findValintatuloksetByHakemus(hakuOid, hakemusOid).map(valintatulokset -> {
      if (StringUtils.isBlank(valintatapajonoOid)) {
        return valintatulokset;
      } else {
        return valintatulokset.stream().filter(v -> valintatapajonoOid.equals(v.getValintatapajonoOid()))
  ...
  @Consumes("application/json")
  public void kaikkiHakemuksenSijoittelunTulokset(@PathParam("hakemusOid") String hakemusOid,
      @PathParam("hakuOid") String hakuOid, @Suspended AsyncResponse asyncResponse) {
    setAsyncTimeout(asyncResponse, String.format(
        "ValintatulosserviceProxy -palvelukutsu on aikakatkaistu: /hakemus/%s/haku/%s", hakemusOid, hakuOid));
  ...
  private static class DateTimeJsonSerializer extends JsonSerializer<DateTime> {
    ...
    public void serialize(DateTime dateTime, JsonGenerator jsonGenerator, SerializerProvider serializerProvider)
        throws IOException {
      String timestampAsString = ValintaTulosServiceAsyncResource.valintaTulosServiceCompatibleFormatter
          .print(dateTime);
