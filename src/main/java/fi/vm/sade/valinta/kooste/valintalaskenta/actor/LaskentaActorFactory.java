...
public class LaskentaActorFactory {
  ...
  @Autowired
  public LaskentaActorFactory(@Value("${valintalaskentakoostepalvelu.laskennan.splittaus:1}") int splittaus,
      ...
      ValintapisteAsyncResource valintapisteAsyncResource, KoskiService koskiService,
  ...
  private Observable<Pair<Collection<String>, List<LaskeDTO>>> fetchRecursively(
    ...
    return tulosObservable.switchMap(haetutResurssit -> {
      ...
      if (oids.isEmpty()) {
        ...
        Observable<Pair<Collection<String>, List<LaskeDTO>>> aiemmatJaUusiResurssi = haeLaskeDTOHakukohteelle
            .apply(headWithTail.getKey())
            .map(laskeDTO -> Pair.of(headWithTail.getRight(),
                Stream.concat(Stream.of(laskeDTO), haetutResurssit.getRight().stream())
                    .collect(Collectors.toList())));
        return aiemmatJaUusiResurssi
            .switchMap(haetut -> fetchRecursively(haeLaskeDTOHakukohteelle, Observable.just(haetut)));
      }
    });
  }

  private LaskentaActor createValintaryhmaActor(AuditSession auditSession, LaskentaSupervisor laskentaSupervisor,
      Haku haku, LaskentaActorParams a, Date nyt) {
    LaskentaActorParams fakeOnlyOneHakukohdeParams = new LaskentaActorParams(a.getLaskentaStartParams(),
        Collections.singletonList(new HakukohdeJaOrganisaatio()), a.getParametritDTO());
    fakeOnlyOneHakukohdeParams.setValintaryhmalaskenta(true);
    final SuoritustiedotDTO suoritustiedot = new SuoritustiedotDTO();
    return laskentaHakukohteittainActor(laskentaSupervisor, fakeOnlyOneHakukohdeParams, hakukohdeJaOrganisaatio -> {
      String uuid = a.getUuid();
      Collection<String> hakukohdeOids = a.getHakukohdeOids().stream()
      ...
      Observable<Pair<Collection<String>, List<LaskeDTO>>> recursiveSequentialFetch = just(
          of(hakukohdeOids, emptyList()));

      Function<String, Observable<LaskeDTO>> fetchLaskeDTO = h -> Observable.fromFuture(
          fetchResourcesForOneLaskenta(auditSession, uuid, haku, h, a, true, true, suoritustiedot, nyt));

      Observable<String> laskenta = fetchRecursively(fetchLaskeDTO, recursiveSequentialFetch)
          .switchMap(hksAndDtos -> {
            ...
            if (!hksAndDtos.getKey().isEmpty()) { // sanity check
              ...
              throw new RuntimeException("Hakukohteita oli " + hakukohdeOids.size()
                  + " mutta haettuja laskeDTOita oli " + allLaskeDTOs.size() + "!");
            }
  ...
  private LaskentaActor createValintakoelaskentaActor(AuditSession auditSession,
      LaskentaSupervisor laskentaSupervisor, Haku haku, LaskentaActorParams actorParams, Date nyt) {
    final String uuid = actorParams.getUuid();
    return laskentaHakukohteittainActor(laskentaSupervisor, actorParams, hakukohdeJaOrganisaatio -> {
      String hakukohdeOid = hakukohdeJaOrganisaatio.getHakukohdeOid();
      SuoritustiedotDTO suoritustiedot = new SuoritustiedotDTO();
      Observable<String> laskenta = Observable.fromFuture(fetchResourcesForOneLaskenta(auditSession, uuid, haku,
          hakukohdeOid, actorParams, false, false, suoritustiedot, nyt))
          .switchMap(timedSwitchMap((took, exception) -> {
            if (exception.isPresent()) {
              LOG.error("(Uuid={}) (Kesto {}s) Laskenta hakukohteelle {} on päättynyt virheeseen: {}",
                  uuid, millisToString(took), hakukohdeOid, exception.get());
            } else {
              LOG.info("(Uuid={}) (Kesto {}s) Laskenta hakukohteelle {} on päättynyt onnistuneesti.",
                  uuid, millisToString(took), hakukohdeOid);
            }
  ...
  private <A, T> io.reactivex.functions.Function<A, Observable<T>> timedSwitchMap(
      BiConsumer<Long, Optional<Throwable>> log, Function<A, Observable<T>> f) { // Function<A,Observable<T>>
    // switchMap) {
    return (A a) -> {
      long start = System.currentTimeMillis();
      Observable<T> t = wrapAsRunOnlyOnceObservable(f.apply(a));
      t.subscribe((n) -> log.accept(System.currentTimeMillis() - start, Optional.empty()),
  ...
  private LaskentaActor createValintalaskentaActor(AuditSession auditSession, LaskentaSupervisor laskentaSupervisor,
      Haku haku, LaskentaActorParams actorParams) {
    final String uuid = actorParams.getUuid();
    ...
    return laskentaHakukohteittainActor(laskentaSupervisor, actorParams, hakukohdeJaOrganisaatio -> {
      ...
      SuoritustiedotDTO suoritustiedot = new SuoritustiedotDTO();
      Observable<String> laskenta = Observable.fromFuture(fetchResourcesForOneLaskenta(auditSession, uuid, haku,
          hakukohdeOid, actorParams, false, true, suoritustiedot, nyt))
          .switchMap(timedSwitchMap((took, exception) -> {
            if (exception.isPresent()) {
              LOG.error("(Uuid={}) (Kesto {}s) Laskenta hakukohteelle {} on päättynyt virheeseen: {}",
                  uuid, millisToString(took), hakukohdeOid, exception.get());
  ...
  private LaskentaActor createValintalaskentaJaValintakoelaskentaActor(AuditSession auditSession,
      LaskentaSupervisor laskentaSupervisor, Haku haku, LaskentaActorParams actorParams, Date nyt) {
    final String uuid = actorParams.getUuid();
    return laskentaHakukohteittainActor(laskentaSupervisor, actorParams, hakukohdeJaOrganisaatio -> {
      String hakukohdeOid = hakukohdeJaOrganisaatio.getHakukohdeOid();
      LOG.info("(Uuid={}) Haetaan laskennan + valintakoelaskennan resursseja hakukohteelle {}", uuid,
          hakukohdeOid);
      SuoritustiedotDTO suoritustiedot = new SuoritustiedotDTO();
      Observable<String> laskenta = Observable.fromFuture(fetchResourcesForOneLaskenta(auditSession, uuid, haku,
          hakukohdeOid, actorParams, false, true, suoritustiedot, nyt))
  ...
  public LaskentaActor createLaskentaActor(AuditSession auditSession, LaskentaSupervisor laskentaSupervisor,
      Haku haku, LaskentaActorParams actorParams) {
    final Date nyt = new Date();
    LOG.info(String.format(
        "Jos laskennassa %s on jonoja, joita ei lasketa %s jälkeen, ei haeta niille tietoja Koskesta.",
    ...
    LOG.info("Muodostetaan KAIKKI VAIHEET LASKENTA koska valinnanvaihe oli {} ja valintakoelaskenta ehto {}",
        actorParams.getValinnanvaihe(), actorParams.isValintakoelaskenta());
  ...
  private void auditLogLaskentaStart(AuditSession auditSession, LaskentaActorParams actorParams, String hakuOid,
      String tyyppi) {
    ...
    additionalAuditInfo.put("hakukohteet", actorParams.getLaskentaStartParams().getHakukohdeDtos().stream()
        .map(HakukohdeJaOrganisaatio::getHakukohdeOid).collect(Collectors.toList()).toString());
    AuditLog.log(KoosteAudit.AUDIT, auditSession.asAuditUser(), ValintaperusteetOperation.LASKENTATOTEUTUS_LUONTI,
        ValintaResource.LASKENTATOTEUTUS, hakuOid, Changes.EMPTY, additionalAuditInfo);
  }
  ...
  private LaskentaActor laskentaHakukohteittainActor(LaskentaSupervisor laskentaSupervisor,
    ...
    return new LaskentaActorForSingleHakukohde(actorParams, r, laskentaSupervisor, laskentaSeurantaAsyncResource,
        splittaus);
  }

  private CompletableFuture<LaskeDTO> getLaskeDtoFuture(String uuid, Haku haku, String hakukohdeOid,
      LaskentaActorParams actorParams, boolean withHakijaRyhmat,
      CompletableFuture<List<ValintaperusteetDTO>> valintaperusteetF, CompletableFuture<List<Oppija>> oppijatF,
    ...
    return CompletableFuture.allOf(valintapisteetForHakukohdesF, hakijaryhmatF, valintaperusteetF, hakemuksetF,
        oppijatF, hakukohdeRyhmasForHakukohdesF, koskiOppijaByOppijaOidF).thenApplyAsync(x -> {
          ...
          LOG.info("(Uuid: {}) Kaikki resurssit hakukohteelle {} saatu. Kootaan ja palautetaan LaskeDTO.",
              uuid, hakukohdeOid);
          ...
          if (!withHakijaRyhmat) {
            return new LaskeDTO(uuid, haku.isKorkeakouluhaku(), actorParams.isErillishaku(), hakukohdeOid,
                hakemuksetConverterUtil.muodostaHakemuksetDTOfromHakemukset(haku, hakukohdeOid,
                    ...
                    oppijat, actorParams.getParametritDTO(), true, true),
                valintaperusteet);
            ...
            return new LaskeDTO(uuid, haku.isKorkeakouluhaku(), actorParams.isErillishaku(), hakukohdeOid,
                hakemuksetConverterUtil.muodostaHakemuksetDTOfromHakemukset(haku, hakukohdeOid,
                    ryhmatHakukohteittain, hakemukset, pisteetWithLastModified.valintapisteet,
                    oppijat, actorParams.getParametritDTO(), true, true),
                valintaperusteet, hakijaryhmatF.join());
          }
        });
  }

  private void verifyValintalaskentaKaytossaOrThrowError(String uuid, String hakukohdeOid,
      List<ValintaperusteetDTO> valintaperusteetList) {
    boolean jokinValintatapajonoKayttaaValintalaskentaa = valintaperusteetList.stream()
        .map(ValintaperusteetDTO::getValinnanVaihe).flatMap(v -> v.getValintatapajono().stream())
  ...
  private void verifyJonokriteeritOrThrowError(String uuid, String hakukohdeOid,
      List<ValintaperusteetDTO> valintaperusteetList) {
    ...
    Optional<ValintatapajonoJarjestyskriteereillaDTO> valintatapajonoPuutteellisellaJonokriteerilla = valintaperusteetList
        .stream().map(ValintaperusteetDTO::getValinnanVaihe).flatMap(v -> v.getValintatapajono().stream())
        .filter(valintatapajonoHasPuuttuvaJonokriteeri).findFirst();

    if (valintatapajonoPuutteellisellaJonokriteerilla.isPresent()) {
      ValintatapajonoJarjestyskriteereillaDTO valintatapajono = valintatapajonoPuutteellisellaJonokriteerilla
          .get();
      String errorMessage = String.format(
  ...
  private CompletableFuture<LaskeDTO> fetchResourcesForOneLaskenta(final AuditSession auditSession, final String uuid,
      Haku haku, final String hakukohdeOid, LaskentaActorParams actorParams, boolean retryHakemuksetAndOppijat,
      boolean withHakijaRyhmat, SuoritustiedotDTO suoritustiedotDTO, Date nyt) {
    final String hakuOid = haku.oid;

    PyynnonTunniste tunniste = new PyynnonTunniste("Please put individual resource source identifier here!", uuid,
        hakukohdeOid);
    ...
    if (haku.isHakemuspalvelu()) {
      ...
      hakemukset = createResurssiFuture(tunniste, "applicationAsyncResource.getApplications",
          () -> ataruAsyncResource.getApplicationsByHakukohde(hakukohdeOid, haetaanHarkinnanvaraisuudet),
          retryHakemuksetAndOppijat);
    } else {
      hakemukset = createResurssiFuture(tunniste, "applicationAsyncResource.getApplicationsByOid",
    ...
    CompletableFuture<List<HenkiloViiteDto>> henkiloViitteet = hakemukset.thenComposeAsync(hws -> {
      List<HenkiloViiteDto> viitteet = hws.stream()
          .map(hw -> new HenkiloViiteDto(hw.getApplicationPersonOid(), hw.getPersonOid()))
          .collect(Collectors.toList());
      return CompletableFuture.completedFuture(viitteet);
    });

    CompletableFuture<List<Oppija>> oppijasForOidsFromHakemukses = henkiloViitteet.thenComposeAsync(hws -> {
      LOG.info("Got henkiloViittees: {}", hws);
      Map<String, String> masterToOriginal = hws.stream()
          .collect(Collectors.toMap(HenkiloViiteDto::getMasterOid, HenkiloViiteDto::getHenkiloOid));
      List<String> oppijaOids = new ArrayList<>(masterToOriginal.keySet());
      LOG.info("Got personOids from hakemukses and getting Oppijas for these: {} for hakukohde {}", oppijaOids,
          hakukohdeOid);
      return createResurssiFuture(tunniste, "suoritusrekisteriAsyncResource.getSuorituksetByOppijas",
          () -> suoritusrekisteriAsyncResource.getSuorituksetByOppijas(oppijaOids, hakuOid),
          retryHakemuksetAndOppijat).thenApply(oppijat -> {
            oppijat.forEach(
                oppija -> oppija.setOppijanumero(masterToOriginal.get(oppija.getOppijanumero())));
            return oppijat;
          });
    });
    ...
    CompletableFuture<PisteetWithLastModified> valintapisteetHakemuksille = hakemukset
        .thenComposeAsync(hakemusWrappers -> {
          ...
          return createResurssiFuture(tunniste,
              ...
              () -> valintapisteAsyncResource.getValintapisteetWithHakemusOidsAsFuture(hakemusOids,
                  auditSession),
              retryHakemuksetAndOppijat);
        });
    CompletableFuture<List<ValintaperusteetHakijaryhmaDTO>> hakijaryhmat = withHakijaRyhmat
        ? createResurssiFuture(tunniste, "valintaperusteetAsyncResource.haeHakijaryhmat",
    ...
    LOG.info(
        ...
        uuid, hakukohdeOid);
    return getLaskeDtoFuture(uuid, haku, hakukohdeOid, actorParams, withHakijaRyhmat, valintaperusteet,
        oppijasForOidsFromHakemukses, hakukohdeRyhmasForHakukohdes, valintapisteetHakemuksille, hakijaryhmat,
        hakemukset, koskiOppijaByOppijaOid);
  }

  private <T> CompletableFuture<T> createResurssiFuture(PyynnonTunniste tunniste, String resurssi,
      Supplier<CompletableFuture<T>> sourceFuture, boolean retry) {
    return LaskentaResurssinhakuWrapper.luoLaskentaResurssinHakuFuture(sourceFuture, tunniste.withNimi(resurssi),
        retry);
  }
