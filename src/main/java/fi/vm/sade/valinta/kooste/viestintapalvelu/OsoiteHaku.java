...
public class OsoiteHaku {
  ...
  private static Organisaatio responseToOrganisaatio(Response organisaatioResponse)
      throws IOException {
    ...
    try {
      ...
      hakijapalveluidenOsoite =
          LueHakijapalvelunOsoite.lueHakijapalvelunOsoite(
              haeOsoiteKomponentti, kieli, rdto, organisaationimi);
      if (rdto == null) {
        LOG.error(
            "Organisaatiopalvelusta ei saatu organisaatiota tunnisteelle {}. Eli ei saatu hakijapalveluiden osoitetta.",
            Arrays.toString(oids.toArray()));
        return null;
      }
      if (oids == null) {
      ...
      if (hakijapalveluidenOsoite != null) {
        LOG.error(
            "Hakijapalveluiden osoite saatiin tarjoajalta {}.\r\n{}",
            Arrays.toString(oids.toArray()),
            new GsonBuilder().setPrettyPrinting().create().toJson(hakijapalveluidenOsoite));
        return hakijapalveluidenOsoite;
      }
      if (rdto.getParentOid() != null) {
        LOG.error(
            "Ei saatu hakijapalveluiden osoitetta talta organisaatiolta. Tutkitaan seuraava {}",
            Arrays.toString(oids.toArray()));
        return haeOsoiteHierarkisesti(
            haeOsoiteKomponentti,
            organisaatioAsyncResource,
            kieli,
            oids,
            responseToOrganisaatio(
                organisaatioAsyncResource
                    .haeOrganisaatio(rdto.getParentOid())
                    .timeout(1, MINUTES)
                    .blockingFirst()),
            organisaationimi);
      } else {
        LOG.error(
            "Ei saatu hakijapalveluiden osoitetta! Kaytiin lapi organisaatiot {}!",
            Arrays.toString(oids.toArray()));
        return null;
      }
    } catch (Exception e) {
  ...
  public static Osoite organisaatioResponseToHakijapalveluidenOsoite(
    ...
    return haeOsoiteHierarkisesti(
        haeOsoiteKomponentti, organisaatioAsyncResource, kieli, oids, org, organisaationimi);
  }
}
