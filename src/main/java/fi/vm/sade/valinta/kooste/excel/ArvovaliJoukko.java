...
public class ArvovaliJoukko {
    ...
    public ArvovaliJoukko(Number min, Number max, XSSFSheet sheet,
                          XSSFDataValidationHelper dvHelper) {
        this.dvHelper = dvHelper;
        this.sheet = sheet;

        this.dvConstraint = (XSSFDataValidationConstraint) dvHelper
                .createNumericConstraint(ValidationType.DECIMAL, OperatorType.BETWEEN, min.toString(), max.toString());
    }

    public void addAddress(int row, int col) {
        XSSFDataValidation validation = (XSSFDataValidation) dvHelper
                .createValidation(dvConstraint, new org.apache.poi.ss.util.CellRangeAddressList(row, row, col, col));
