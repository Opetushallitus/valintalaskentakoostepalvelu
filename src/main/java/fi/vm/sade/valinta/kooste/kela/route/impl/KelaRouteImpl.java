...
public class KelaRouteImpl extends AbstractDokumenttiRouteBuilder {
  ...
  @Autowired
  public KelaRouteImpl(@Value(KelaRoute.SEDA_KELA_LUONTI) String kelaLuonti,
      DokumenttiAsyncResource dokumenttiAsyncResource, KelaHakijaRiviKomponenttiImpl kelaHakijaKomponentti,
      KelaDokumentinLuontiKomponenttiImpl kelaDokumentinLuontiKomponentti,
      TarjontaAsyncResource tarjontaAsyncResource, HaunTyyppiKomponentti haunTyyppiKomponentti,
      OppijanumerorekisteriAsyncResource oppijanumerorekisteriAsyncResource,
      OppilaitosKomponentti oppilaitosKomponentti, LinjakoodiKomponentti linjakoodiKomponentti,
      ValintaTulosServiceAsyncResource valintaTulosServiceAsyncResource, KelaResource kelaResource) {
  ...
  private DefaultErrorHandlerBuilder deadLetterChannel() {
    return deadLetterChannel(KelaRoute.DIRECT_KELA_FAILED).logExhaustedMessageHistory(true).logExhausted(true)
  ...
  public final void configure() {
    ...
    from(KelaRoute.DIRECT_KELA_FAILED).routeId("KELALUONTI_DEADLETTERCHANNEL").process(exchange -> {
      String virhe = null;
    ...
    from(haeHakuJaValmistaHaku).routeId("KELALUONTI_HAKU").errorHandler(deadLetterChannel())
        .process(Reititys.<KelaLuonti, KelaLuontiJaHaut>funktio(luonti -> {
          ...
          for (String hakuOid : luonti.getHakuOids()) {
            try {
              ...
              luonti.getProsessi().getPoikkeuksetUudelleenYrityksessa()
    ...
    from(tarkistaHaunTyyppi).routeId("KELALUONTI_TARKISTA_TYYPPI").errorHandler(deadLetterChannel())
        .process(Reititys.<KelaLuontiJaHaut, KelaLuontiJaHaut>funktio(luontiJaHaut -> {
          Collection<Poikkeus> poikkeuksetUudelleenYrityksessa = luontiJaHaut.getLuonti().getProsessi()
          ...
          if (luontiJaHaut.getHaut().isEmpty()) {
            ...
            poikkeuksetUudelleenYrityksessa
                .add(new Poikkeus(Poikkeus.KOOSTEPALVELU, virhe, luontiJaHaut.getLuonti().getUuid()));
            throw new RuntimeException(virhe);
          }
          boolean kaikkiHautKk = luontiJaHaut.getHaut().stream().allMatch(Haku::isKorkeakouluhaku);
          if (!kaikkiHautKk && luontiJaHaut.getHaut().stream().anyMatch(Haku::isKorkeakouluhaku)) {
    ...
    from(keraaHakujenDatat).routeId("KELALUONTI_KERAA_HAKUJEN_DATAT").errorHandler(deadLetterChannel())
        .process(Reititys.<KelaLuontiJaHaut, KelaLuontiJaAbstraktitHaut>funktio(luontiJaHaut -> {
          ...
          for (Haku haku : luontiJaHaut.getHaut()) {
            ...
            try {
              Collection<ValintaTulosServiceDto> hakijat = valintaTulosServiceAsyncResource
                  .getHaunValintatulokset(haku.oid).timeout(30, MINUTES).blockingFirst().stream()
    ...
    from(kelaLuonti).errorHandler(deadLetterChannel()).routeId("KELALUONTI").to(haeHakuJaValmistaHaku)
        .process(Reititys.<KelaLuontiJaAbstraktitHaut>kuluttaja(luonti -> {
          List<String> henkiloOidit = luonti.getHaut().stream().flatMap(h -> h.getPersonOids().stream())
          ...
          HakukohdeSource hakukohdeSource = new HakukohdeSource() {
            ...
            public AbstractHakukohde getHakukohdeByOid(String oid) {
              try {
                return hakukohdeCache.get(oid,
                    () -> tarjontaAsyncResource.haeHakukohde(oid).get(5, MINUTES));
              } catch (Exception t) {
                LOG.error(
          ...
          OppilaitosSource oppilaitosSource = new OppilaitosSource() {
            ...
            public String getOppilaitosnumero(String tarjoajaOid) {
              try {
                return numeroCache.get(tarjoajaOid,
            ...
            public TasoJaLaajuusDTO getTutkinnontaso(String hakukohdeOid) {
              if (!c.containsKey(hakukohdeOid)) {
                try {
                  ...
                  LOG.info("Tutkinnon tasokoodi hakukohteelle {} oli {}", hakukohdeOid,
                      tutkinnontaso.getTasoCode());
                  c.put(hakukohdeOid, tutkinnontaso);
                  return tutkinnontaso;
                } catch (Exception e) {
                  LOG.error("Ei saatu kela-rajapinnalta tutkinnon tasoa hakukohteelle" + hakukohdeOid,
                      e);
                  throw e;
                }
              } else {
            ...
            public String getKoulutusaste(String hakukohdeOid) {
              if (!d.containsKey(hakukohdeOid)) {
                try {
                  ...
                  LOG.error("Ei saatu kela-rajapinnalta koulutusastetta hakukohteelle" + hakukohdeOid,
                      e);
                  throw e;
                }
              }
          ...
          TilaSource tilaSource = (hakemusOid, hakuOid, hakukohdeOid, valintatapajonoOid) -> {
            ...
            for (int tries = 0; tries < maxTries; ++tries) {
              try {
                List<Muutoshistoria> muutoshistoriat = valintaTulosServiceAsyncResource
                    .getMuutoshistoria(hakemusOid, valintatapajonoOid).timeout(30, MINUTES)
                    .blockingFirst();

                final Predicate<Change> isVastaanottoChange = (change) -> "vastaanottotila"
                    .equals(change.getField());
                final Predicate<Map.Entry<String, Date>> isVastaanotto = entry -> asList(
                    "VASTAANOTTANUT_SITOVASTI", "VASTAANOTTANUT", "EHDOLLISESTI_VASTAANOTTANUT")
                        .contains(entry.getKey());

                Optional<Map.Entry<String, Date>> newestVastaanottoFieldStatus = muutoshistoriat
                    .stream()
                    .flatMap(m -> m.getChanges().stream().filter(isVastaanottoChange)
                        .map(c -> Maps.immutableEntry(c.getTo(),
                            Date.from(m.getTimestamp().toInstant()))))
                    .sorted(Comparator.<Map.Entry<String, Date>, Date>comparing(Map.Entry::getValue)
                        .reversed())
                    .findFirst();

                return newestVastaanottoFieldStatus.filter(isVastaanotto).map(Map.Entry::getValue)
                    .orElse(null);
              } catch (Exception e) {
                LOG.error("tilaResource ei jaksa palvella {}. Yritetään vielä uudestaan. " + tries
                    + "/20...", e);
                try {
                  Thread.sleep(10000L);
            ...
            throw new RuntimeException(
                "Tried " + maxTries + " times to get muutoshistoriat from VTS and gave up!");
          };
          for (KelaAbstraktiHaku kelahaku : luontiJaRivit.getHaut()) {
            rivit.addAll(kelahaku.createHakijaRivit(luontiJaRivit.getLuonti().getAlkuPvm(),
                luontiJaRivit.getLuonti().getLoppuPvm(), kelahaku.getHaku().oid,
                luontiJaRivit.getLuonti().getProsessi(), luontiJaRivit.getLuonti().getCache(),
                hakukohdeSource, linjakoodiSource, oppilaitosSource, tutkinnontasoSource, tilaSource));
          }

          if (rivit.isEmpty()) {
            String virhe = "Kela-dokumenttia ei voi luoda hauille joissa ei ole yhtään valittua hakijaa!";
            luontiJaRivit.getLuonti().getProsessi().getPoikkeuksetUudelleenYrityksessa()
          ...
          Collection<TKUVAYHVA> rivit = luontiJaRivit.getRivit().stream().map(rivi -> {
            try {
              return kelaHakijaKomponentti.luo(rivi);
            } catch (Throwable t) {
              LOG.error(
                  "Rivin luonti haussa {} ja hakukohteessa {} olevalle hakemukselle {} epäonnistui.",
                  rivi.getHakuOid(), rivi.getHakukohde(), rivi.getHakemusOid(), t);
              throw t;
            } finally {
          ...
          try {
            ...
            return new KelaLuontiJaDokumentti(luontiJaRivit.getLuonti(),
                kelaDokumentinLuontiKomponentti.luo(rivit,
    ...
    from(vientiDokumenttipalveluun).routeId("KELALUONTI_DOKUMENTTIPALVELUUN").errorHandler(deadLetterChannel())
        .process(Reititys.<KelaLuontiJaDokumentti>kuluttaja(luontiJaDokumentti -> {
          String id = generateId();
          LOG.info("Aloitetaan keladokumentin(uuid {} ja dokumenttiId) siirtovaihe dokumenttipalveluun.",
          ...
          try {
            ...
            dokumenttiAsyncResource.tallenna(id,
                ...
                expirationTime, tags, "application/octet-stream", filedata).subscribe(ok -> {
            ...
            luontiJaDokumentti.getLuonti().getProsessi().getPoikkeuksetUudelleenYrityksessa()
                .add(new Poikkeus(Poikkeus.DOKUMENTTIPALVELU,
