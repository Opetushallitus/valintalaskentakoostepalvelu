...
public class HakuParametritService {
    ...
    public HakuParametritService(OhjausparametritAsyncResource ohjausparametritAsyncResource,
        ...
        this.haunOhjausParametritCache = CacheBuilder.newBuilder().expireAfterWrite(cacheTtlMinutes, TimeUnit.MINUTES).build();
    ...
    public CompletableFuture<ParametritParser> getParametritForHakuAsync(String hakuOid) {
        try {
            CompletableFuture<ParametritParser> f = haunOhjausParametritCache.get(hakuOid, () -> fetchParametrit(hakuOid));
