...
public class KoutaHakukohde extends AbstractHakukohde {
  ...
  public KoutaHakukohde(String oid, Tila tila, Map<String, String> nimi, String hakuOid, Set<String> tarjoajaOids,
      Set<String> toteutusOids, Set<String> pohjakoulutusvaatimusUrit, Integer valintojenAloituspaikat,
      Map<String, String> ohjeetUudelleOpiskelijalle, Set<KoutaValintakoe> valintakokeet,
      Set<KoutaValintakoe> valintaperusteValintakokeet, BigDecimal alinHyvaksyttyKeskiarvo,
      List<PainotettuArvosana> painotetutArvosanat, String hakukohdeKoodiUri, String koulutustyyppikoodi) {
    super(oid, tila, nimi, hakuOid, tarjoajaOids, toteutusOids, null, pohjakoulutusvaatimusUrit,
        valintojenAloituspaikat, ohjeetUudelleOpiskelijalle);
  ...
  public KoutaHakukohde(KoutaHakukohdeDTO dto) {
    super(dto.oid, parseTila(dto.tila), new HashMap<>(), dto.hakuOid, Set.of(dto.tarjoaja),
        Collections.singleton(dto.toteutusOid), null, dto.pohjakoulutusvaatimusKoodiUrit, dto.aloituspaikat,
        new HashMap<>());

    this.valintakokeet = dto.valintakokeet.stream().map(KoutaValintakoe::new).collect(Collectors.toSet());
    dto.nimi.forEach((kieli, nimi) -> this.nimi.put("kieli_" + kieli, nimi));
    if (dto.uudenOpiskelijanUrl != null) {
      dto.uudenOpiskelijanUrl.forEach((kieli, uudenOpiskelijanUrl) -> this.ohjeetUudelleOpiskelijalle
          .put("kieli_" + kieli, uudenOpiskelijanUrl));
    }
    this.alinHyvaksyttyKeskiarvo = dto.alinHyvaksyttyKeskiarvo;
    this.valintaperusteValintakokeet = dto.valintaperusteValintakokeet.stream().map(KoutaValintakoe::new)
        .collect(Collectors.toSet());
    this.painotetutArvosanat = dto.painotetutArvosanat.stream().map(PainotettuArvosana::new)
  ...
  private static Optional<KoutaValintakoe> getValintakoeOfType(Collection<KoutaValintakoe> valintakokeet,
      String valintakoeTypeUri) {
    return valintakokeet.stream().filter(vk -> vk.valintakokeentyyppiUri.split("#")[0].equals(valintakoeTypeUri))
        .findFirst();
  }
