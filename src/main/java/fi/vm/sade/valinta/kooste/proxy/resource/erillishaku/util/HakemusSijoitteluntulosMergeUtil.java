...
public class HakemusSijoitteluntulosMergeUtil {
  ...
  public static List<MergeValinnanvaiheDTO> merge(String hakuOid, String hakukohdeOid, List<Hakemus> hakemukset,
      HakukohdeDTO hakukohdeDTO, List<ValinnanVaiheJonoillaDTO> valinnanvaiheet,
      List<ValintatietoValinnanvaiheDTO> laskennantulokset, List<Valintatulos> valintatulosDtos) {
    ...
    boolean laskennanTulosLoytyi = laskennantulokset.stream()
        ...
        .flatMap(v -> v.getValintatapajonot().stream()).flatMap(j -> j.getJonosijat().stream()).findAny()
        .isPresent();
    ...
    if (!laskennanTulosLoytyi && hakukohdeDTO.getValintatapajonot().isEmpty()) {
      ...
      handleEiSijoittelunTuloksia(hakuOid, hakukohdeOid, hakemukset, laskennantulokset, valintatulosDtos, result);
    } else {
      handleLaskentaJaSijoitteluOlemassa(hakuOid, hakukohdeOid, hakemukset, hakukohdeDTO, laskennantulokset,
          valintatulosDtos, result);
    }
  ...
  private static void handleLaskentaJaSijoitteluOlemassa(String hakuOid, String hakukohdeOid,
      List<Hakemus> hakemukset, HakukohdeDTO hakukohdeDTO, List<ValintatietoValinnanvaiheDTO> laskennantulokset,
      List<Valintatulos> valintatulosDtos, List<MergeValinnanvaiheDTO> result) {
    laskennantulokset.stream().forEach(vv -> {
      ...
      vv.getValintatapajonot().stream().forEach(jono -> {
        ...
        Optional<ValintatapajonoDTO> sJono = hakukohdeDTO.getValintatapajonot().stream()
            .filter(j -> j.getOid().equals(jonoDTO.getOid())).findFirst();
  ...
  private static void handleEiSijoittelunTuloksia(String hakuOid, String hakukohdeOid, List<Hakemus> hakemukset,
      List<ValintatietoValinnanvaiheDTO> laskennantulokset, List<Valintatulos> valintatulosDtos,
  ...
  private static void handleEiLaskennanTuloksia(String hakuOid, String hakukohdeOid, List<Hakemus> hakemukset,
      HakukohdeDTO hakukohdeDTO, List<Valintatulos> valintatulosDtos, List<MergeValinnanvaiheDTO> result) {
    List<MergeValintatapajonoDTO> valintatapajonot = hakukohdeDTO.getValintatapajonot().stream().map(jono -> {
  ...
  private static void handlePelkatHakemuksetTaiEiMitaanDataa(List<Hakemus> hakemukset,
      List<MergeValinnanvaiheDTO> result) {
    ...
    if (result.get(0).getValintatapajonot().isEmpty()) {
      ...
      jonoDTO.setKaytetaanValintalaskentaa(false);
      List<MergeHakemusDTO> luodut = hakemukset.stream().map(h -> luo(Optional.of(h)))
          .collect(Collectors.toList());
  ...
  private static List<MergeValinnanvaiheDTO> getMergeValinnanvaiheDTOs(String hakuOid, String hakukohdeOid,
  ...
  private static Optional<MergeValinnanvaiheDTO> findVaihe(List<MergeValinnanvaiheDTO> vaiheet,
      String valinnanvaiheOid) {
    return vaiheet.stream().filter(Objects::nonNull).filter(
        vaihe -> vaihe.getValinnanvaiheoid() != null && vaihe.getValinnanvaiheoid().equals(valinnanvaiheOid))
        .findFirst();
  }

  private static Optional<MergeValintatapajonoDTO> findJono(List<MergeValintatapajonoDTO> jonot, String jonoOid) {
    return jonot.stream().filter(Objects::nonNull).filter(j -> j.getOid() != null && j.getOid().equals(jonoOid))
        .findFirst();
  }

  private static boolean valinnanvaiheExists(String valinnanvaiheOid,
      List<ValinnanVaiheJonoillaDTO> valinnanvaiheet) {
  ...
  private static MergeValinnanvaiheDTO createValinnanvaihe(String hakuOid, String hakukohdeOid,
      ValintatietoValinnanvaiheDTO vv) {
    MergeValinnanvaiheDTO dto = new MergeValinnanvaiheDTO();
    dto.setJarjestysnumero(vv.getJarjestysnumero());
    dto.setHakuOid(hakuOid);
    dto.setHakukohdeOid(hakukohdeOid);
  ...
  private static MergeValinnanvaiheDTO createValinnanvaihe(String hakuOid, String hakukohdeOid,
    ...
    vv.getJonot().forEach(jono -> {
      ...
      jonoDTO.setOid(Optional.ofNullable(trimToNull(jono.getOid()))
  ...
  private static MergeValintatapajonoDTO luoSijoittelunTiedoista(String hakuOid, String hakukohdeOid,
    ...
    jonoDTO.setNimi(jono.getNimi());
    jonoDTO.setOid(
        Optional.ofNullable(trimToNull(jono.getOid())).orElse(oidHaustaJaHakukohteesta(hakuOid, hakukohdeOid))); // "null"
    // arvojen
    // pois
    // filtterointi
    jonoDTO.setPoissaOlevaTaytto(Boolean.TRUE.equals(jono.getPoissaOlevaTaytto()));
  ...
  private static void asetaSijoittelunTiedoista(String hakuOid, String hakukohdeOid, MergeValintatapajonoDTO jonoDTO,
      ValintatapajonoDTO jono, List<Valintatulos> valintatulosDtos) {
    jonoDTO.setAlinHyvaksyttyPistemaara(jono.getAlinHyvaksyttyPistemaara());
    jonoDTO.setAloituspaikat(jono.getAloituspaikat());
    jonoDTO.setEiVarasijatayttoa(Boolean.TRUE.equals(jono.getEiVarasijatayttoa()));
    jonoDTO.setHakeneet(jono.getHakeneet());
    ...
    jonoDTO.setNimi(jono.getNimi());
    jonoDTO.setOid(
        Optional.ofNullable(trimToNull(jono.getOid())).orElse(oidHaustaJaHakukohteesta(hakuOid, hakukohdeOid)));
    jonoDTO.setPoissaOlevaTaytto(Boolean.TRUE.equals(jono.getPoissaOlevaTaytto()));
  ...
  private static MergeValintatapajonoDTO luoLaskennanTiedoista(String hakuOid, String hakukohdeOid,
  ...
  private static MergeValintatapajonoDTO yhdistaLaskennanTiedoista(MergeValintatapajonoDTO jonoDTO,
      ValintatietoValintatapajonoDTO jono) {
    jonoDTO.setPrioriteetti(jono.getPrioriteetti());
  ...
  private static List<MergeHakemusDTO> mergaaSijoittelusta(List<Hakemus> hakemukset,
      List<HakemusDTO> sijoittelunHakemukset, List<Valintatulos> valintatulosDtos) {
    ...
    // Hakemuspalvelun palauttamat hakemukset, jotka puuttuvat sijoittelun
    // tuloksista
    ...
    List<MergeHakemusDTO> puuttuvat = puuttuvatOidit.stream().map(oid -> luo(Optional.of(hakemusOidMap.get(oid))))
        .collect(Collectors.toList());
  ...
  private static List<MergeHakemusDTO> mergaaLaskennasta(List<Hakemus> hakemukset,
      List<JonosijaDTO> laskennanHakemukset) {
    final Map<String, Hakemus> hakemusOidMap = hakemusOidMap(hakemukset);
  ...
  private static void mergaaLaskentaJaSijoittelu(List<MergeHakemusDTO> hakemukset,
      List<HakemusDTO> sijoittelunHakemukset, List<Valintatulos> valintatulosDtos) {
    ...
    sijoittelunHakemukset.forEach(h -> {
      if (!mergatutOidSet.contains(h.getHakemusOid())) {
        ...
        MergeHakemusDTO mergeHakemusDTO = hakemukset.stream()
            .filter(hakemus -> hakemus.getHakemusOid().equals(h.getHakemusOid())).findFirst().get();
  ...
  private static Optional<Valintatulos> haeValintatulos(List<Valintatulos> valintatulosDtos, HakemusDTO h) {
    return valintatulosDtos.parallelStream().filter(v -> v.getHakemusOid().equals(h.getHakemusOid())
        && v.getValintatapajonoOid().equals(h.getValintatapajonoOid())).findFirst();
  }
  ...
  private static MergeHakemusDTO asetaSijoittelunTiedot(MergeHakemusDTO dto, HakemusDTO h,
      Optional<Valintatulos> valintatulos) {
  ...
  private static Set<String> hakemusOiditSijoittelusta(List<HakemusDTO> sijoittelunHakemukset) {
    return sijoittelunHakemukset.stream().map(HakemusDTO::getHakemusOid).collect(Collectors.toSet());
  }
  ...
  private static Set<String> hakemusOiditMergatuista(List<MergeHakemusDTO> laskennanHakemukset) {
    return laskennanHakemukset.stream().map(MergeHakemusDTO::getHakemusOid).collect(Collectors.toSet());
  }
}
