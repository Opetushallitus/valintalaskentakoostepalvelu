...
public class HakemusSijoitteluntulosMergeUtil {
  ...
  public static List<MergeValinnanvaiheDTO> merge(
    ...
    List<MergeValinnanvaiheDTO> result =
        getMergeValinnanvaiheDTOs(hakuOid, hakukohdeOid, valinnanvaiheet);

    boolean laskennanTulosLoytyi =
        laskennantulokset.stream()
            .filter(
                laskennantulos ->
                    valinnanvaiheExists(laskennantulos.getValinnanvaiheoid(), valinnanvaiheet))
    ...
    if (!laskennanTulosLoytyi && hakukohdeDTO.getValintatapajonot().isEmpty()) {
      ...
      handleEiLaskennanTuloksia(
          hakuOid, hakukohdeOid, hakemukset, hakukohdeDTO, valintatulosDtos, result);
    } else if (hakukohdeDTO == null || hakukohdeDTO.getValintatapajonot().isEmpty()) {
      handleEiSijoittelunTuloksia(
          hakuOid, hakukohdeOid, hakemukset, laskennantulokset, valintatulosDtos, result);
    } else {
      handleLaskentaJaSijoitteluOlemassa(
          hakuOid,
          hakukohdeOid,
          hakemukset,
          hakukohdeDTO,
    ...
    Collections.sort(
        result, (v1, v2) -> Integer.compare(v1.getJarjestysnumero(), v2.getJarjestysnumero()));
    result.get(result.size() - 1).setViimeinenVaihe(true);
    return result;
  }

  private static void handleLaskentaJaSijoitteluOlemassa(
      String hakuOid,
      String hakukohdeOid,
      List<Hakemus> hakemukset,
      HakukohdeDTO hakukohdeDTO,
        ...
        .forEach(
            vv -> {
              ...
              vv.getValintatapajonot().stream()
                  .forEach(
                      jono -> {
                        Optional<MergeValintatapajonoDTO> jonoOpt =
                            findJono(dto.getValintatapajonot(), jono.getOid());
                        ...
                        Optional<ValintatapajonoDTO> sJono =
                            hakukohdeDTO.getValintatapajonot().stream()
                                .filter(j -> j.getOid().equals(jonoDTO.getOid()))
                                .findFirst();

                        if (sJono.isPresent()) {
                          asetaSijoittelunTiedoista(
                              hakuOid, hakukohdeOid, jonoDTO, sJono.get(), valintatulosDtos);
                        }
                      });
            });
  }

  private static void handleEiSijoittelunTuloksia(
      String hakuOid,
      String hakukohdeOid,
      List<Hakemus> hakemukset,
      List<ValintatietoValinnanvaiheDTO> laskennantulokset,
  ...
  private static void handleEiLaskennanTuloksia(
      String hakuOid,
      String hakukohdeOid,
      List<Hakemus> hakemukset,
      HakukohdeDTO hakukohdeDTO,
      List<Valintatulos> valintatulosDtos,
      List<MergeValinnanvaiheDTO> result) {
    List<MergeValintatapajonoDTO> valintatapajonot =
        hakukohdeDTO.getValintatapajonot().stream()
            .map(
                jono -> {
                  MergeValintatapajonoDTO jonoDTO =
                      luoSijoittelunTiedoista(hakuOid, hakukohdeOid, jono);
                  jonoDTO.setHakemukset(
                      mergaaSijoittelusta(hakemukset, jono.getHakemukset(), valintatulosDtos));
  ...
  private static void handlePelkatHakemuksetTaiEiMitaanDataa(
      List<Hakemus> hakemukset, List<MergeValinnanvaiheDTO> result) {
    ...
    if (result.get(0).getValintatapajonot().isEmpty()) {
      ...
      jonoDTO.setKaytetaanValintalaskentaa(false);
      List<MergeHakemusDTO> luodut =
          hakemukset.stream().map(h -> luo(Optional.of(h))).collect(Collectors.toList());
      jonoDTO.setHakemukset(luodut);
      result.get(0).getValintatapajonot().add(jonoDTO);
    } else {
      result.forEach(
          v ->
              v.getValintatapajonot()
                  .forEach(
                      j -> {
                        List<MergeHakemusDTO> luodut =
                            hakemukset.stream()
                                .map(h -> luo(Optional.of(h)))
                                .collect(Collectors.toList());
  ...
  private static List<MergeValinnanvaiheDTO> getMergeValinnanvaiheDTOs(
      String hakuOid, String hakukohdeOid, List<ValinnanVaiheJonoillaDTO> valinnanvaiheet) {
    ...
    if (valinnanvaiheet.isEmpty()) {
      ...
      valinnanvaiheet.forEach(
          vaihe -> result.add(createValinnanvaihe(hakuOid, hakukohdeOid, vaihe)));
    }
    return result;
  }

  private static Optional<MergeValinnanvaiheDTO> findVaihe(
      List<MergeValinnanvaiheDTO> vaiheet, String valinnanvaiheOid) {
    return vaiheet.stream()
        .filter(Objects::nonNull)
        .filter(
            vaihe ->
                vaihe.getValinnanvaiheoid() != null
                    && vaihe.getValinnanvaiheoid().equals(valinnanvaiheOid))
        .findFirst();
  }

  private static Optional<MergeValintatapajonoDTO> findJono(
      List<MergeValintatapajonoDTO> jonot, String jonoOid) {
    return jonot.stream()
        .filter(Objects::nonNull)
  ...
  private static boolean valinnanvaiheExists(
      String valinnanvaiheOid, List<ValinnanVaiheJonoillaDTO> valinnanvaiheet) {
    return valinnanvaiheet.stream()
        .anyMatch(valinnanvaihe -> valinnanvaihe.getOid().equals(valinnanvaiheOid));
  }

  private static MergeValinnanvaiheDTO createValinnanvaihe(
  ...
  private static MergeValinnanvaiheDTO createValinnanvaihe(
      String hakuOid, String hakukohdeOid, ValintatietoValinnanvaiheDTO vv) {
    MergeValinnanvaiheDTO dto = new MergeValinnanvaiheDTO();
    dto.setJarjestysnumero(vv.getJarjestysnumero());
    dto.setHakuOid(hakuOid);
    dto.setHakukohdeOid(hakukohdeOid);
  ...
  private static MergeValinnanvaiheDTO createValinnanvaihe(
      String hakuOid, String hakukohdeOid, ValinnanVaiheJonoillaDTO vv) {
    MergeValinnanvaiheDTO dto = new MergeValinnanvaiheDTO();
    dto.setHakuOid(hakuOid);
    dto.setHakukohdeOid(hakukohdeOid);
    dto.setNimi(vv.getNimi());
    ...
    vv.getJonot()
        .forEach(
            jono -> {
              ...
              jonoDTO.setKaikkiEhdonTayttavatHyvaksytaan(
                  Boolean.TRUE.equals(jono.getKaikkiEhdonTayttavatHyvaksytaan()));
              jonoDTO.setNimi(jono.getNimi());
              jonoDTO.setOid(
                  Optional.ofNullable(trimToNull(jono.getOid()))
              ...
              jonoDTO.setKaytetaanValintalaskentaa(
                  Boolean.TRUE.equals(jono.getKaytetaanValintalaskentaa()));
              dto.getValintatapajonot().add(jonoDTO);
            });
    return dto;
  }

  private static MergeValintatapajonoDTO luoSijoittelunTiedoista(
      String hakuOid, String hakukohdeOid, ValintatapajonoDTO jono) {
    MergeValintatapajonoDTO jonoDTO = new MergeValintatapajonoDTO();
    ...
    jonoDTO.setOid(
        Optional.ofNullable(trimToNull(jono.getOid()))
            .orElse(
                oidHaustaJaHakukohteesta(
                    hakuOid, hakukohdeOid))); // "null" arvojen pois filtterointi
    jonoDTO.setPoissaOlevaTaytto(Boolean.TRUE.equals(jono.getPoissaOlevaTaytto()));
  ...
  private static void asetaSijoittelunTiedoista(
      String hakuOid,
      String hakukohdeOid,
      MergeValintatapajonoDTO jonoDTO,
      ValintatapajonoDTO jono,
      List<Valintatulos> valintatulosDtos) {
    jonoDTO.setAlinHyvaksyttyPistemaara(jono.getAlinHyvaksyttyPistemaara());
    jonoDTO.setAloituspaikat(jono.getAloituspaikat());
    jonoDTO.setEiVarasijatayttoa(Boolean.TRUE.equals(jono.getEiVarasijatayttoa()));
  ...
  private static MergeValintatapajonoDTO luoLaskennanTiedoista(
      String hakuOid, String hakukohdeOid, ValintatietoValintatapajonoDTO jono) {
    MergeValintatapajonoDTO jonoDTO = new MergeValintatapajonoDTO();
    jonoDTO.setNimi(jono.getNimi());
    jonoDTO.setOid(
        Optional.ofNullable(trimToNull(jono.getOid()))
  ...
  private static MergeValintatapajonoDTO yhdistaLaskennanTiedoista(
      MergeValintatapajonoDTO jonoDTO, ValintatietoValintatapajonoDTO jono) {
    jonoDTO.setPrioriteetti(jono.getPrioriteetti());
  ...
  private static List<MergeHakemusDTO> mergaaSijoittelusta(
      List<Hakemus> hakemukset,
      List<HakemusDTO> sijoittelunHakemukset,
      List<Valintatulos> valintatulosDtos) {
    ...
    List<MergeHakemusDTO> mergatut =
        sijoittelunHakemukset.stream()
            .map(
                h ->
                    asetaSijoittelunTiedot(
                        luo(Optional.ofNullable(hakemusOidMap.get(h.getHakemusOid()))),
                        h,
                        haeValintatulos(valintatulosDtos, h)))
            .collect(Collectors.toList());

    // Luodaan sijoittelun tuloksista puuttuvat hakemukset
    List<MergeHakemusDTO> puuttuvat =
        puuttuvatOidit.stream()
  ...
  private static List<MergeHakemusDTO> mergaaLaskennasta(
      List<Hakemus> hakemukset, List<JonosijaDTO> laskennanHakemukset) {
    final Map<String, Hakemus> hakemusOidMap = hakemusOidMap(hakemukset);
    ...
    // Asetetaan sijoittelun tulokset
    List<MergeHakemusDTO> mergatut =
        laskennanHakemukset.stream()
            .map(
                h ->
                    asetaLaskennanTiedot(
                        luo(Optional.ofNullable(hakemusOidMap.get(h.getHakemusOid()))), h))
            .collect(Collectors.toList());
  ...
  private static void mergaaLaskentaJaSijoittelu(
      List<MergeHakemusDTO> hakemukset,
      List<HakemusDTO> sijoittelunHakemukset,
      List<Valintatulos> valintatulosDtos) {
    final Set<String> mergatutOidSet = hakemusOiditMergatuista(hakemukset);

    sijoittelunHakemukset.forEach(
        h -> {
          if (!mergatutOidSet.contains(h.getHakemusOid())) {
            hakemukset.add(
                asetaSijoittelunTiedot(
                    luo(Optional.empty()), h, haeValintatulos(valintatulosDtos, h)));
          } else {
            MergeHakemusDTO mergeHakemusDTO =
                hakemukset.stream()
                    .filter(hakemus -> hakemus.getHakemusOid().equals(h.getHakemusOid()))
                    .findFirst()
                    .get();
  ...
  private static Optional<Valintatulos> haeValintatulos(
      List<Valintatulos> valintatulosDtos, HakemusDTO h) {
    return valintatulosDtos.parallelStream()
        .filter(
            v ->
                v.getHakemusOid().equals(h.getHakemusOid())
                    && v.getValintatapajonoOid().equals(h.getValintatapajonoOid()))
        .findFirst();
  }

  private static Set<String> puuttuvatOidit(Set<String> hakemusOidit, Set<String> vertailtava) {
    return hakemusOidit.stream()
        .filter(oid -> !vertailtava.contains(oid))
  ...
  private static MergeHakemusDTO asetaSijoittelunTiedot(
      MergeHakemusDTO dto, HakemusDTO h, Optional<Valintatulos> valintatulos) {
    ...
    if (valintatulos.isPresent()) {
      ...
      dto.setEhdollisenHyvaksymisenEhtoKoodi(
          valintatulos.get().getEhdollisenHyvaksymisenEhtoKoodi());
  ...
  private static Map<String, Hakemus> hakemusOidMap(List<Hakemus> hakemukset) {
    return hakemukset.stream().collect(Collectors.toMap(Hakemus::getOid, h -> h, (h, n) -> n));
  }
  ...
  private static Set<String> hakemusOiditLaskennasta(List<JonosijaDTO> laskennanHakemukset) {
    return laskennanHakemukset.stream().map(JonosijaDTO::getHakemusOid).collect(Collectors.toSet());
  }
