...
public class HyvaksymiskirjeetServiceImpl implements HyvaksymiskirjeetService {
  ...
  @Autowired
  public HyvaksymiskirjeetServiceImpl(ViestintapalveluAsyncResource viestintapalveluAsyncResource,
      ...
      HakuParametritService hakuParametritService, KoodistoCachedAsyncResource koodistoCachedAsyncResource,
      DokumenttiAsyncResource dokumenttiAsyncResource,
      DokumenttiProsessiKomponentti dokumenttiProsessiKomponentti, KirjeetHakukohdeCache kirjeetHakukohdeCache,
  ...
  public ProsessiId hyvaksymiskirjeetHakemuksille(HyvaksymiskirjeDTO hyvaksymiskirjeDTO, List<String> hakemusOids) {
    ...
    return this.yhdenKirjeeranProsessi(this.smallBatchExecutor, prosessi -> {
      CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF = tarjontaAsyncResource.haeHaku(hakuOid)
      ...
      CompletableFuture<List<HakijaDTO>> hakijatF = hakijatByHakemusOids(hakuOid, hakemusOids)
          .thenComposeAsync(hakijat -> hakemuksetF.thenApplyAsync(hakemukset -> hyvaksytytHakijat(hakijat,
              hakemukset, hakukohdeOid, null, hyvaksymiskirjeDTO.getVainTulosEmailinKieltaneet())));
      return muodostaHyvaksymiskirjeet(prosessi, hakuParametritService.getParametritForHakuAsync(hakuOid),
          hakijatF, hakemuksetF, hyvaksymiskirjeDTO, null, false,
          Collections.singletonList(ContentStructureType.letter), KirjeenVastaanottaja.HAKIJA);
    }, String.format("Aloitetaan hakukohteen %s hyväksymiskirjeiden muodostaminen %d hakemukselle", hakukohdeOid,
        hakemusOids.size()),
        String.format("Hakukohteen %s hyväksymiskirjeiden muodostaminen %d hakemukselle valmistui",
            hakukohdeOid, hakemusOids.size()),
        String.format("Hakukohteen %s hyväksymiskirjeiden muodostaminen %d hakemukselle epäonnistui",
            hakukohdeOid, hakemusOids.size()));
  }

  @Override
  public ProsessiId jalkiohjauskirjeetHakemuksille(JalkiohjauskirjeDTO jalkiohjauskirjeDTO,
      List<String> hakemusOids) {
    String hakuOid = jalkiohjauskirjeDTO.getHakuOid();
    return this.yhdenKirjeeranProsessi(this.smallBatchExecutor,
        prosessi -> tarjontaAsyncResource.haeHaku(hakuOid).thenComposeAsync(haku -> {
          CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF = hakemuksetByOids(haku, hakemusOids);
          CompletableFuture<List<HakijaDTO>> hakijatF = hakijatByHakemusOids(hakuOid, hakemusOids)
              .thenComposeAsync(
                  hakijat -> hakemuksetF.thenApplyAsync(hakemukset -> hylatytHakijat(hakijat,
                      hakemukset, jalkiohjauskirjeDTO.getKielikoodi())));
          return muodostaJalkiohjauskirjeet(prosessi, hakijatF, hakemuksetF, jalkiohjauskirjeDTO, false,
              haku.isKorkeakouluhaku(), Collections.singletonList(ContentStructureType.letter),
  ...
  public ProsessiId jalkiohjauskirjeHakukohteelle(HyvaksymiskirjeDTO hyvaksymiskirjeDTO) {
    ...
    return this.yhdenKirjeeranProsessi(this.smallBatchExecutor, prosessi -> {
      ...
      CompletableFuture<List<HakijaDTO>> hakijatF = valintaTulosServiceAsyncResource
          .getKaikkiHakijat(hakuOid, hakukohdeOid).thenComposeAsync(hakijat -> hakemuksetF
              .thenApplyAsync(hakemukset -> hylatytHakijat(hakijat, hakemukset, null)));
      CompletableFuture<ParametritParser> haunParametritF = CompletableFuture
          .completedFuture(new ParametritParser(new ParametritDTO(), ""));
      return muodostaHyvaksymiskirjeet(prosessi, haunParametritF, hakijatF, hakemuksetF, hyvaksymiskirjeDTO, null,
          false, Collections.singletonList(ContentStructureType.letter), KirjeenVastaanottaja.HAKIJA);
  ...
  public ProsessiId hyvaksymiskirjeetHaulle(HyvaksymiskirjeDTO hyvaksymiskirjeDTO, String asiointikieli,
    ...
    String hakuOid = hyvaksymiskirjeDTO.getHakuOid();
    return this.yhdenKirjeeranProsessi(this.bigBatchExecutor, prosessi -> {
      CompletableFuture<List<HakijaDTO>> kaikkiHakijatF = valintaTulosServiceAsyncResource
          .getKoulutuspaikalliset(hakuOid);
      CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF = kaikkiHakijatF.thenComposeAsync(
          hakijat -> tarjontaAsyncResource.haeHaku(hakuOid).thenComposeAsync(haku -> hakemuksetByOids(haku,
              hakijat.stream().map(HakijaDTO::getHakemusOid).collect(Collectors.toList()))));
      CompletableFuture<List<HakijaDTO>> hakijatF = kaikkiHakijatF.thenComposeAsync(hakijat -> hakemuksetF
          .thenApplyAsync(hakemukset -> hyvaksytytHakijat(hakijat, hakemukset, null, asiointikieli, false)));
      return muodostaHyvaksymiskirjeet(prosessi, hakuParametritService.getParametritForHakuAsync(hakuOid),
          hakijatF, hakemuksetF, hyvaksymiskirjeDTO, asiointikieli, true,
          Arrays.asList(ContentStructureType.accessibleHtml), kirjeenVastaanottaja);
    }, String.format("Aloitetaan haun %s hyväksymiskirjeiden muodostaminen asiointikielelle %s", hakuOid,
        asiointikieli),
        String.format("Haun %s hyväksymiskirjeiden muodostaminen asiointikielelle %s valmistui", hakuOid,
            asiointikieli),
        String.format("Haun %s hyväksymiskirjeiden muodostaminen asiointikielelle %s epäonnistui", hakuOid,
            asiointikieli));
  }
  ...
  public ProsessiId jalkiohjauskirjeetHaulle(JalkiohjauskirjeDTO jalkiohjauskirjeDTO,
    ...
    return this.yhdenKirjeeranProsessi(this.bigBatchExecutor,
        prosessi -> tarjontaAsyncResource.haeHaku(hakuOid).thenComposeAsync(haku -> {
          CompletableFuture<List<HakijaDTO>> kaikkiHakijatF = valintaTulosServiceAsyncResource
              .getHakijatIlmanKoulutuspaikkaa(hakuOid);
          CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF = kaikkiHakijatF
              .thenComposeAsync(hakijat -> hakemuksetByOids(haku,
                  hakijat.stream().map(HakijaDTO::getHakemusOid).collect(Collectors.toList())));
          CompletableFuture<List<HakijaDTO>> hakijatF = kaikkiHakijatF.thenComposeAsync(hakijat -> hakemuksetF
              .thenApplyAsync(hakemukset -> hylatytHakijat(hakijat, hakemukset, asiointikieli)));
          return muodostaJalkiohjauskirjeet(prosessi, hakijatF, hakemuksetF, jalkiohjauskirjeDTO, true,
              haku.isKorkeakouluhaku(),
              getContentStructureTypesForHaunJalkiohjauskirjeet(haku.isKorkeakouluhaku()),
              kirjeenVastaanottaja);
        }),
        String.format("Aloitetaan haun %s jälkiohjauskirjeiden muodostaminen asiointikielelle %s", hakuOid,
            asiointikieli),
        String.format("Haun %s jälkiohjauskirjeiden muodostaminen asiointikielelle %s valmistui", hakuOid,
            asiointikieli),
        String.format("Haun %s jälkiohjauskirjeiden muodostaminen asiointikielelle %s epäonnistui", hakuOid,
            asiointikieli));
  }
  ...
  public ProsessiId hyvaksymiskirjeetHaulleHakukohteittain(HyvaksymiskirjeDTO hyvaksymiskirjeDTO) {
    ...
    this.bigBatchExecutor.execute(() -> {
      try {
        LOG.info(
            String.format("Aloitetaan haun %s hyväksymiskirjeiden muodostaminen hakukohteittain", hakuOid));

        CompletableFuture<Map<String, Koodi>> maatjavaltiot1F = koodistoCachedAsyncResource
            .haeKoodistoAsync(KoodistoCachedAsyncResource.MAAT_JA_VALTIOT_1);
        CompletableFuture<Map<String, Koodi>> postinumerotF = koodistoCachedAsyncResource
            .haeKoodistoAsync(KoodistoCachedAsyncResource.POSTI);
        CompletableFuture<ParametritParser> haunParametritF = hakuParametritService
            .getParametritForHakuAsync(hakuOid);
        CompletableFuture<List<HakijaDTO>> hakijatF = valintaTulosServiceAsyncResource
            .getKoulutuspaikalliset(hakuOid);
        CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF = hakijatF
            .thenComposeAsync(hakijat -> tarjontaAsyncResource.haeHaku(hakuOid)
                .thenComposeAsync(haku -> hakemuksetByOids(haku,
                    hakijat.stream().map(HakijaDTO::getHakemusOid).collect(Collectors.toList()))));
        CompletableFuture<Map<String, Map<String, List<SyotettyArvoDTO>>>> syotetytArvotF = hakijatF
            .thenComposeAsync(this::hakijoidenSyotetytArvot);
        ...
        CompletableFuture.allOf(maatjavaltiot1F, postinumerotF, haunParametritF, hakijatF, hakemuksetF,
            syotetytArvotF, hakukohteetF, osoitteetF).get(1, TimeUnit.HOURS);
        List<Pair<String, Future<String>>> fs = hakijatF.join().stream()
            .flatMap(hakija -> hakija.getHakutoiveet().stream())
            .filter(hakutoive -> hakutoive.getHakutoiveenValintatapajonot().stream()
                .anyMatch(valintatapajono -> valintatapajono.getTila().isHyvaksytty()))
            .map(HakutoiveDTO::getHakukohdeOid).distinct()
            .map(hakukohdeOid -> Pair.of(hakukohdeOid, this.smallBatchExecutor.submit(() -> {
              try {
                LOG.info(String.format(
                    "Aloitetaan haun %s hakukohteen %s hyväksymiskirjeiden muodostaminen", hakuOid,
                    hakukohdeOid));
                return haeHakukohteenVakiosisalto(hyvaksymiskirjeDTO.getSisalto(), hakukohdeOid)
                    .thenApplyAsync(vakiosisalto -> HyvaksymiskirjeetKomponentti
                        .teeHyvaksymiskirjeet(maatjavaltiot1F.join(), postinumerotF.join(),
                            osoitteetF.join(), hakukohteetF.join(),
                            hyvaksytytHakijat(hakijatF.join(), hakemuksetF.join(),
                                hakukohdeOid, null, false),
                            hakemuksetF.join(), syotetytArvotF.join(), hakukohdeOid,
                            hakuOid, Optional.empty(), vakiosisalto, hakuOid,
                            hyvaksymiskirjeDTO.getTemplateName(),
                            parsePalautusPvm(null, haunParametritF.join()),
                            parsePalautusAika(null, haunParametritF.join()), false,
                            Collections.singletonList(ContentStructureType.letter),
                            KirjeenVastaanottaja.HAKIJA))
                    .thenComposeAsync(
                        letterBatch -> letterBatchToViestintapalvelu(prosessi, letterBatch))
                    .thenComposeAsync(batchId -> dokumenttiAsyncResource
        ...
        List<Poikkeus> poikkeukset = fs.stream().flatMap(p -> {
          String hakukohdeOid = p.getLeft();
          try {
            String batchId = p.getRight().get(1, TimeUnit.HOURS);
            LOG.info(String.format("Haun %s hakukohteen %s hyväksymiskirjeiden muodostaminen valmistui",
            ...
            String msg = String.format(
                "Haun %s hakukohteen %s hyväksymiskirjeiden muodostaminen epäonnistui", hakuOid,
                hakukohdeOid);
            LOG.error(msg, e);
            prosessi.inkrementoiOhitettujaToita();
            return Stream.of(Poikkeus.koostepalvelupoikkeus(msg,
                Collections.singletonList(new Tunniste(hakukohdeOid, Poikkeus.HAKUKOHDEOID))));
          }
        }).collect(Collectors.toList());
        if (poikkeukset.isEmpty()) {
          LOG.info(String.format("Haun %s hyväksymiskirjeiden muodostaminen hakukohteittain valmistui",
              hakuOid));
          prosessi.setDokumenttiId(exampleBatchId.get());
        } else {
          LOG.error(String.format("Haun %s hyväksymiskirjeiden muodostaminen hakukohteittain epäonnistui",
              hakuOid));
          prosessi.getPoikkeukset().addAll(poikkeukset);
        }
      } catch (Exception e) {
        String msg = String.format("Haun %s hyväksymiskirjeiden muodostaminen hakukohteittain epäonnistui",
            hakuOid);
        LOG.error(msg, e);
  ...
  private CompletableFuture<Map<String, HakemusWrapper>> hakemuksetByHakukohde(String hakuOid, String hakukohdeOid) {
    return tarjontaAsyncResource.haeHaku(hakuOid)
        .thenComposeAsync(haku -> haku.isHakemuspalvelu()
            ? ataruAsyncResource.getApplicationsByHakukohde(hakukohdeOid)
            : applicationAsyncResource.getApplicationsByOidsWithPOST(hakuOid,
                Collections.singletonList(hakukohdeOid)))
        .thenApplyAsync(
            hakemukset -> hakemukset.stream().collect(Collectors.toMap(HakemusWrapper::getOid, h -> h)));
  }

  private CompletableFuture<Map<String, HakemusWrapper>> hakemuksetByOids(Haku haku, List<String> hakemusOids) {
    return (haku.isHakemuspalvelu()
        ? ataruAsyncResource.getApplicationsByOids(hakemusOids)
        : applicationAsyncResource.getApplicationsByhakemusOidsInParts(haku.oid, hakemusOids,
            ApplicationAsyncResource.DEFAULT_KEYS))
                .thenApplyAsync(hakemukset -> hakemukset.stream()
  ...
  private CompletableFuture<Map<String, List<SyotettyArvoDTO>>> syotetytArvotByHakukohde(String hakukohdeOid,
    ...
    return this.valintalaskentaAsyncResource.laskennantulokset(hakukohdeOid, executor)
        .thenApplyAsync(valinnanvaiheet -> valinnanvaiheet.stream()
            ...
            .flatMap(valintatapajono -> valintatapajono.getJonosijat().stream()).collect(
                Collectors.toMap(JonosijaDTO::getHakemusOid, JonosijaDTO::getSyotetytArvot, (l, ll) -> {
                  l.addAll(ll);
                  return l;
                })));
  }

  private CompletableFuture<Map<String, Map<String, List<SyotettyArvoDTO>>>> hakijoidenSyotetytArvot(
      List<HakijaDTO> hakijat) {
    Executor executor = createExecutorService(2, "syotetytArvotByHakukohde");
    return CompletableFutureUtil.sequence(hakijat.stream().flatMap(hakija -> hakija.getHakutoiveet().stream())
        .map(HakutoiveDTO::getHakukohdeOid).distinct()
        .collect(Collectors.toMap(Function.identity(), hk -> syotetytArvotByHakukohde(hk, executor))));
  }

  private CompletableFuture<String> muodostaHyvaksymiskirjeet(DokumenttiProsessi prosessi,
      CompletableFuture<ParametritParser> haunParametritF, CompletableFuture<List<HakijaDTO>> hakijatF,
      CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF, HyvaksymiskirjeDTO hyvaksymiskirjeDTO,
      String asiointikieli, boolean sahkoinenMassaposti, List<ContentStructureType> sisaltotyypit,
      KirjeenVastaanottaja kirjeenVastaanottaja) {
    CompletableFuture<Map<String, MetaHakukohde>> hakukohteetF = hakijatF
        .thenComposeAsync(this::kiinnostavatHakukohteet);
    CompletableFuture<Map<String, Koodi>> maatjavaltiot1F = koodistoCachedAsyncResource
    ...
    CompletableFuture<Map<String, Koodi>> postinumerotF = koodistoCachedAsyncResource
        .haeKoodistoAsync(KoodistoCachedAsyncResource.POSTI);
    CompletableFuture<Map<String, Optional<Osoite>>> osoitteetF = hakukohteetF.thenComposeAsync(
        hakukohteet -> this.hakukohteidenHakutoimistojenOsoitteet(hakukohteet, asiointikieli));
    CompletableFuture<String> vakiosisaltoF = this.haeHakukohteenVakiosisalto(hyvaksymiskirjeDTO.getSisalto(),
        hyvaksymiskirjeDTO.getHakukohdeOid());
    CompletableFuture<Map<String, Map<String, List<SyotettyArvoDTO>>>> syotetytArvotF = hakijatF
        .thenComposeAsync(this::hakijoidenSyotetytArvot);
    return CompletableFuture
        .allOf(maatjavaltiot1F, postinumerotF, haunParametritF, hakijatF, hakemuksetF, syotetytArvotF,
            hakukohteetF, osoitteetF, vakiosisaltoF)
        .thenApplyAsync(v -> HyvaksymiskirjeetKomponentti.teeHyvaksymiskirjeet(maatjavaltiot1F.join(),
            postinumerotF.join(), osoitteetF.join(), hakukohteetF.join(), hakijatF.join(),
            hakemuksetF.join(), syotetytArvotF.join(), hyvaksymiskirjeDTO.getHakukohdeOid(),
            hyvaksymiskirjeDTO.getHakuOid(), Optional.ofNullable(asiointikieli), vakiosisaltoF.join(),
            hyvaksymiskirjeDTO.getTag(), hyvaksymiskirjeDTO.getTemplateName(),
            ...
            sahkoinenMassaposti, sisaltotyypit, kirjeenVastaanottaja))
        .thenComposeAsync(letterBatch -> letterBatchToViestintapalvelu(prosessi, letterBatch));
  }

  private CompletableFuture<String> muodostaJalkiohjauskirjeet(DokumenttiProsessi prosessi,
      CompletableFuture<List<HakijaDTO>> hakijatF, CompletableFuture<Map<String, HakemusWrapper>> hakemuksetF,
      JalkiohjauskirjeDTO jalkiohjauskirjeDTO, boolean sahkoinenMassaposti, boolean isKkHaku,
      List<ContentStructureType> sisaltotyypit, KirjeenVastaanottaja kirjeenVastaanottaja) {
    CompletableFuture<Map<String, MetaHakukohde>> hakukohteetF = hakijatF
        .thenComposeAsync(this::kiinnostavatHakukohteet);
    CompletableFuture<Map<String, Koodi>> maatjavaltiot1F = koodistoCachedAsyncResource
        .haeKoodistoAsync(KoodistoCachedAsyncResource.MAAT_JA_VALTIOT_1);
    ...
    CompletableFuture<Map<String, Map<String, List<SyotettyArvoDTO>>>> syotetytArvotF = hakijatF
        .thenComposeAsync(this::hakijoidenSyotetytArvot);
    return CompletableFuture
        .allOf(maatjavaltiot1F, postinumerotF, hakijatF, hakemuksetF, hakukohteetF, syotetytArvotF)
        .thenApplyAsync(v -> JalkiohjauskirjeetKomponentti.teeJalkiohjauskirjeet(maatjavaltiot1F.join(),
            postinumerotF.join(), jalkiohjauskirjeDTO.getKielikoodi(), hakijatF.join(), hakemuksetF.join(),
            syotetytArvotF.join(), hakukohteetF.join(), jalkiohjauskirjeDTO.getHakuOid(),
            jalkiohjauskirjeDTO.getTemplateName(), jalkiohjauskirjeDTO.getSisalto(),
            jalkiohjauskirjeDTO.getTag(), sahkoinenMassaposti, isKkHaku, sisaltotyypit,
            kirjeenVastaanottaja))
        .thenComposeAsync(letterBatch -> letterBatchToViestintapalvelu(prosessi, letterBatch));
  }

  private ProsessiId yhdenKirjeeranProsessi(ExecutorService executor,
      Function<DokumenttiProsessi, CompletableFuture<String>> task, String startMessage, String successMessage,
      String errorMessage) {
    DokumenttiProsessi prosessi = new DokumenttiProsessi("", "", "", Collections.emptyList());
    dokumenttiProsessiKomponentti.tuoUusiProsessi(prosessi);
    ...
    executor.execute(() -> {
      try {
        ...
        prosessi.inkrementoiOhitettujaToita();
        prosessi.getPoikkeukset().add(Poikkeus
            .koostepalvelupoikkeus(e.getCause() != null ? e.getCause().getMessage() : e.getMessage()));
      }
    });
  ...
  private static List<HakijaDTO> hyvaksytytHakijat(List<HakijaDTO> hakijat, Map<String, HakemusWrapper> hakemukset,
      String hakukohdeJossaHyvaksytty, String asiointikieli, boolean vainTulosEmailinKieltaneet) {
    List<HakijaDTO> l = hakijat.stream().filter(hakija -> hakemukset.containsKey(hakija.getHakemusOid()))
        .filter(hakija -> !vainTulosEmailinKieltaneet
            || !hakemukset.get(hakija.getHakemusOid()).getLupaTulosEmail())
        .filter(hakija -> asiointikieli == null
            || asiointikieli.equalsIgnoreCase(hakemukset.get(hakija.getHakemusOid()).getAsiointikieli()))
        .filter(hakija -> hakija.getHakutoiveet().stream()
            .filter(hakutoive -> hakukohdeJossaHyvaksytty == null
  ...
  private static List<HakijaDTO> hylatytHakijat(List<HakijaDTO> hakijat, Map<String, HakemusWrapper> hakemukset,
      String asiointikieli) {
    List<HakijaDTO> l = hakijat.stream().filter(hakija -> hakemukset.containsKey(hakija.getHakemusOid()))
        .filter(hakija -> asiointikieli == null || asiointikieli
            .equalsIgnoreCase(hakemukset.get(hakija.getHakemusOid()).getAsiointikieli()))
        .filter(hakija -> hakija.getHakutoiveet().stream()
            .flatMap(hakutoive -> hakutoive.getHakutoiveenValintatapajonot().stream())
  ...
  private CompletableFuture<String> haeHakukohteenVakiosisalto(String annettuVakiosisalto, String hakukohdeOid) {
    ...
    return this.tarjontaAsyncResource.haeHakukohde(hakukohdeOid).thenComposeAsync(hakukohde -> CompletableFutureUtil
        .sequence(hakukohde.toteutusOids.stream().map(tarjontaAsyncResource::haeToteutus)
            .collect(Collectors.toList()))
        .thenComposeAsync(toteutukset -> viestintapalveluAsyncResource.haeKirjepohja(hakukohde.hakuOid,
            hakukohde.tarjoajaOids.iterator().next(), "hyvaksymiskirje",
            KirjeetHakukohdeCache.getOpetuskieli(toteutukset.stream()
                .flatMap(toteutus -> toteutus.opetuskielet.stream()).collect(Collectors.toList())),
            hakukohde.oid)))
        .thenComposeAsync(kirjepohjat -> kirjepohjat.stream()
            ...
            .filter(tdd -> VAKIODETAIL.equals(tdd.getName())).map(TemplateDetail::getDefaultValue)
            .map(CompletableFuture::completedFuture).findAny()
            .orElse(CompletableFuture.failedFuture(
                new RuntimeException(String.format("Ei %s tai %s templateDetailia hakukohteelle %s",
  ...
  private CompletableFuture<Map<String, Optional<Osoite>>> hakukohteidenHakutoimistojenOsoitteet(
    ...
    return CompletableFutureUtil
        .sequence(hakukohteet.values().stream().map(MetaHakukohde::getTarjoajaOid).distinct().collect(
            Collectors.toMap(tarjoajaOid -> tarjoajaOid, organisaatioAsyncResource::haeHakutoimisto)))
  ...
  private CompletableFuture<String> letterBatchToViestintapalvelu(DokumenttiProsessi prosessi,
      LetterBatch letterBatch) {
    return vieLetterBatch(prosessi, letterBatch).thenComposeAsync(batchId -> {
      CompletableFuture<String> f = new CompletableFuture<>();
      Disposable s = Observable.interval(pollingIntervalMillis, MILLISECONDS)
          .flatMap(i -> letterBatchProcessingStatus(batchId)).firstOrError()
  ...
  private CompletableFuture<String> vieLetterBatch(DokumenttiProsessi prosessi, LetterBatch letterBatch) {
    ...
    return viestintapalveluAsyncResource.vieLetterBatch(letterBatch).thenApplyAsync(response -> {
      if (response.getStatus().equals(LetterResponse.STATUS_SUCCESS)) {
        return response.getBatchId();
      }
      throw new RuntimeException(response.getErrors().entrySet().stream()
          .map(e -> e.getKey() + ": " + e.getValue()).collect(Collectors.joining("\n")));
    });
  }

  private Observable<String> letterBatchProcessingStatus(String batchId) {
    return Observable.fromFuture(viestintapalveluAsyncResource.haeLetterBatchStatus(batchId)).flatMap(response -> {
  ...
  private List<ContentStructureType> getContentStructureTypesForHaunJalkiohjauskirjeet(boolean isKkHaku) {
    if (isKkHaku) {
