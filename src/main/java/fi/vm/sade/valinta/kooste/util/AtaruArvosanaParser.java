...
public class AtaruArvosanaParser {
  ...
  // Toisen asteen ataruhakemuksilta löytyy tällä hetkellä vain peruskoulun
  // arvosanoja
  ...
  public static List<AvainArvoDTO> convertValinnaisetKielet(Map<String, AvainArvoDTO> keyValues, String hakemusOid) {

    List<AvainArvoDTO> r = new ArrayList<>();

    Map<String, List<AvainArvoDTO>> grouped = keyValues.values().stream()
        .filter(dto -> dto.getAvain().contains("valinnainen-kieli") && !dto.getAvain().contains("-a-"))
        .collect(Collectors.groupingBy(dto -> dto.getAvain().substring(dto.getAvain().length() - 1)));

    Set<String> langs = new HashSet<>(); // A1, A2, B2 jne.

    grouped.forEach((key, value) -> {
      String lang = value.stream().filter(dto -> dto.getAvain().contains("oppiaine")).findFirst()
          .map(dto -> StringUtils.substringAfterLast(dto.getArvo(), "-")).orElse("").toUpperCase();
      if (!lang.isEmpty()) {
        langs.add(lang);
      }
    });
    ...
    for (String aineKey : langs) {
      ...
      try {
        List<Map.Entry<String, List<AvainArvoDTO>>> valuesForMatchingLangs = grouped.entrySet().stream()
            .filter(entry -> entry.getValue().stream().filter(dto -> dto.getAvain().contains("oppiaine"))
                .findFirst().map(dto -> StringUtils.substringAfterLast(dto.getArvo(), "-")).orElse("")
                .toUpperCase().equals(aineKey))
        ...
        for (Map.Entry<String, List<AvainArvoDTO>> entry : valuesForMatchingLangs) {
          ...
          String kieli = entry.getValue().stream().filter(dto -> dto.getAvain().startsWith("oppimaara"))
              .findFirst().map(AvainArvoDTO::getArvo).orElse("").toUpperCase();
          String arvosana = entry.getValue().stream().filter(dto -> dto.getAvain().startsWith("arvosana"))
              .findFirst().map(dto -> StringUtils.substringAfterLast(dto.getArvo(), "-")).orElse("")
              .toUpperCase();
          ...
          if (aineKey.equals("A1")) {
            // Muunnetaan "valinnainen a1-kieli" pakolliseksi, avain muotoon PK_A12, PK_A13
            // jne
            langPrefix = aineKey + A1index;
            LOG.warn("Muunnetaan hakemuksen {} valinnainen A1-kieli {} pakolliseksi avaimelle {}",
                hakemusOid, kieli, prefix + langPrefix);
            A1index++;
          } else if (aineKey.equals("A2") || aineKey.equals("B2")) {
            A2B2index++;
            if (A2B2index > 1) {
              langPrefix = aineKey + A2B2index;
              LOG.warn("Muunnetaan hakemuksen {} valinnainen kieli {} pakolliseksi avaimelle {}",
                  hakemusOid, kieli, prefix + langPrefix);
            }
          } else if (index > 0) {
          ...
          if (!arvosana.isEmpty() && !kieli.isEmpty() && !aineKey.isEmpty()) {
            ...
            if (index == 0 || aineKey.equals("A1") || aineKey.equals("A2") || aineKey.equals("B2")) {
        ...
        LOG.error("Hakemuksen {} valinnaisen kielen parsiminen ei onnistunut: {}", hakemusOid, aineKey, e);
      }
    }
  ...
  public static List<AvainArvoDTO> convertAtaruArvosanas(Map<String, AvainArvoDTO> keyValues, String hakemusOid) {
    List<AvainArvoDTO> r = new ArrayList<>();
