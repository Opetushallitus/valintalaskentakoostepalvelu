...
public class SuoritusrekisteriAsyncResourceImpl extends UrlConfiguredResource
  ...
  public Observable<List<Oppija>> getOppijatByHakukohde(String hakukohdeOid, String hakuOid) {
    return getAsObservableLazily(getUrl("suoritusrekisteri.oppijat"), new TypeToken<List<Oppija>>() {
    }.getType(), client -> {
  ...
  @Override
  public CompletableFuture<List<Oppija>> getOppijatByHakukohdeWithoutEnsikertalaisuus(String hakukohdeOid,
      String hakuOid) {
    ...
    return httpClient.getJson(url, Duration.ofMinutes(5), new com.google.gson.reflect.TypeToken<List<Oppija>>() {
    }.getType());
  }

  @Override
  public Observable<Oppija> getSuorituksetByOppija(String opiskelijaOid, String hakuOid) {
    return getAsObservableLazily(getUrl("suoritusrekisteri.oppijat.opiskelijaoid", opiskelijaOid), Oppija.class,
        client -> {
  ...
  private Observable<List<Oppija>> batchedPostOppijas(List<String> opiskelijaOids, String url) {
    ...
    Observable<Observable<List<Oppija>>> obses = Observable.fromIterable(oidBatches)
        .map(oidBatch -> postAsObservableLazily(url, new TypeToken<List<Oppija>>() {
        }.getType(), Entity.entity(oidBatch, MediaType.APPLICATION_JSON_TYPE), client -> {
          client.accept(MediaType.APPLICATION_JSON_TYPE);
  ...
  private CompletableFuture<List<Oppija>> batchedPostOppijasFuture(List<String> opiskelijaOids, String url) {
    ...
    Type inputType = new com.google.gson.reflect.TypeToken<List<String>>() {
    }.getType();
    Type outputType = new com.google.gson.reflect.TypeToken<List<Oppija>>() {
    }.getType();

    return CompletableFutureUtil.sequence(oidBatches.stream().map(oidBatch -> {
      LOG.info("Calling POST url {} with {} opiskelijaOids", url, oidBatch.size());
      return httpClient.<List<String>, List<Oppija>>postJson(url, Duration.ofMinutes(5), oidBatch, inputType,
          outputType);
    }).collect(Collectors.toList())).thenApplyAsync(
        (List<List<Oppija>> r) -> r.stream().flatMap(List::stream).collect(Collectors.toList()));
  }

  @Override
  public CompletableFuture<Suoritus> postSuoritus(Suoritus suoritus) {
    Type suoritusType = new com.google.gson.reflect.TypeToken<Suoritus>() {
    }.getType();
    return httpClient.postJson(getUrl("suoritusrekisteri.suoritukset"), Duration.ofMinutes(10), suoritus,
        suoritusType, suoritusType);
  }

  @Override
  public CompletableFuture<Arvosana> postArvosana(Arvosana arvosana) {
    Type arvosanaType = new com.google.gson.reflect.TypeToken<Arvosana>() {
    }.getType();
    return httpClient.postJson(getUrl("suoritusrekisteri.arvosanat"), Duration.ofMinutes(10), arvosana,
        arvosanaType, arvosanaType);
  }
  ...
  public CompletableFuture<Arvosana> updateExistingArvosana(String arvosanaId, Arvosana arvosanaWithUpdatedValues) {
    ...
    return httpClient.postJson(getUrl("suoritusrekisteri.arvosanat.id", arvosanaId), Duration.ofMinutes(10),
        arvosanaWithUpdatedValues, arvosanaType, arvosanaType);
  }
