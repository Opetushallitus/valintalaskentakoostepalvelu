...
public class OidRivi extends Rivi {
	...
	public boolean validoi(Rivi rivi) {
		...
		ImmutableList<String> tekstit = FluentIterable.from(soluja)
				.filter(new Predicate<Solu>() {
		...
		if (!Sets.newHashSet(tekstit).equals(oidit)) {
			...
			throw new ExcelValidointiPoikkeus("Odotettiin oidit " + oidstr
					+ " mutta saatiin " + tekstr);
		}
