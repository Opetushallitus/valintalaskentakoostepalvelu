...
public class ModuloiPaivamaaraJaTunnit {
	private static final Logger LOG = LoggerFactory
			.getLogger(ModuloiPaivamaaraJaTunnit.class);

	public static DateTime moduloiSeuraava(DateTime start, DateTime now,
			int moduloitavaTuntiMaara) {
		...
		return new DateTime(now.getYear(), now.getMonthOfYear(),
				now.getDayOfMonth(), start.getHourOfDay(),
				start.getMinuteOfHour()).plusHours(tavoiteTunti
				- start.getHourOfDay());
	}
