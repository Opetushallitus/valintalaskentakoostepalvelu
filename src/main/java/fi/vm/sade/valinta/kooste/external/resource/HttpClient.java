...
public class HttpClient {
  ...
  public <O> CompletableFuture<O> getJson(String url, Duration timeout, Type outputType, Executor executor) {
    HttpRequest request = buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url)))
        .header("Accept", "application/json").GET().timeout(timeout).build();
  ...
  public CompletableFuture<HttpResponse<InputStream>> getResponse(String url, Duration timeout,
    ...
    HttpRequest request = requestCustomisation
        .apply(buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url))).header("Accept", "*/*")
            .GET().timeout(timeout))
        .build();
    return this.makeRequest(request, null);
  }

  public <O> CompletableFuture<O> post(String url, Duration timeout, HttpRequest.BodyPublisher bodyPublisher,
      Function<HttpRequest.Builder, HttpRequest.Builder> requestCustomisation,
      Function<HttpResponse<InputStream>, O> parseResponse) {
    HttpRequest request = requestCustomisation
        .apply(buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url))).POST(bodyPublisher)
            .timeout(timeout))
        .build();
  ...
  public <I, O> CompletableFuture<O> postJson(String url, Duration timeout, I body, Type inputType, Type outputType,
    ...
    return this.post(url, timeout, createJsonBodyPublisher(body, inputType),
        addJsonHeaders.andThen(requestCustomisation), response -> this.parseJson(response, outputType));
  }
  ...
  public <I, O> CompletableFuture<O> putJson(String url, Duration timeout, I body, Type inputType, Type outputType,
      Function<HttpRequest.Builder, HttpRequest.Builder> requestCustomisation) {
    HttpRequest request = requestCustomisation
        .apply(buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url)))
            .header("Accept", "application/json").header("Content-Type", "application/json")
            .PUT(HttpRequest.BodyPublishers.ofString(this.gson.toJson(body, inputType),
  ...
  public CompletableFuture<HttpResponse<InputStream>> putResponse(String url, Duration timeout, byte[] body,
      String contentType, Function<HttpRequest.Builder, HttpRequest.Builder> requestCustomisation) {
    HttpRequest request = requestCustomisation
        .apply(buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url))).header("Accept", "*/*")
            .header("Content-Type", contentType).PUT(HttpRequest.BodyPublishers.ofByteArray(body))
            .timeout(timeout))
        .build();
    return this.makeRequest(request, null);
  }
  ...
  public CompletableFuture<String> delete(String url, Duration timeout) {
    HttpRequest request = buildWithCallerIdAndCsrfHeaders(HttpRequest.newBuilder(URI.create(url)))
        .header("Accept", "*/*").DELETE().timeout(timeout).build();
  ...
  private HttpRequest.Builder buildWithCallerIdAndCsrfHeaders(HttpRequest.Builder builder) {
    return builder.header("Caller-Id", CALLER_ID).header("CSRF", CSRF_VALUE).header("Cookie",
        String.format("CSRF=%s;", CSRF_VALUE));
  }
  ...
  private CompletableFuture<HttpResponse<InputStream>> makeRequest(HttpRequest request, Executor executor) {
    ...
    return this.session.getSessionToken()
        .thenComposeAsync(sessionToken -> this.sendAsync(request, executor).thenComposeAsync(response -> {
  ...
  private CompletableFuture<HttpResponse<InputStream>> sendAsync(HttpRequest request, Executor executor) {
    ...
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofInputStream()).handleAsync((response, e) -> {
  ...
  private CompletableFuture<HttpResponse<InputStream>> sendAsync(HttpRequest request) {
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofInputStream()).handle((response, e) -> {
      if (e != null) {
        throw new IllegalStateException(request.uri().toString(), e);
      }
      return response;
  ...
  public <O> O parseJson(HttpResponse<InputStream> response, Type outputType) {
    if (isSuccess(response)) {
      if (!isJson(response)) {
        throw new IllegalStateException(String.format("%s %d: Unexpected Content-Type %s",
      ...
      try (Reader r = new InputStreamReader(response.body())) {
        ...
        throw new IllegalStateException(String.format("%s %d: Failed to parse JSON response",
            response.uri().toString(), response.statusCode()), e);
      }
    }
    try (InputStream is = response.body()) {
      throw new IllegalStateException(String.format("%s %d: %s", response.uri().toString(), response.statusCode(),
          new String(is.readAllBytes(), Charset.forName("UTF-8"))));
    } catch (IOException e) {
      throw new IllegalStateException(String.format("%s %d: Failed to parse error response",
          response.uri().toString(), response.statusCode()), e);
    }
  }
  ...
  public String parseTxt(HttpResponse<InputStream> response) {
    if (isSuccess(response)) {
      try (Reader r = new InputStreamReader(response.body())) {
        ...
        throw new IllegalStateException(String.format("%s %d: Failed to parse String response",
            response.uri().toString(), response.statusCode()), e);
      }
    }
    try (InputStream is = response.body()) {
  ...
  private static boolean isRedirectToCas(HttpResponse<?> response) {
    return response.statusCode() == 302 && response.headers().allValues("Location").stream()
