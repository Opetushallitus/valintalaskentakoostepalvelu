...
public class AmmatillisenKielikoetuloksetSurestaConverter {
  ...
  public static List<AvainArvoDTO> convert(List<SuoritusJaArvosanat> oppijanSuorituksetJaArvosanat,
      ParametritDTO parametritDTO, HakemusDTO hakemusDTO) {
    if (oppijanSuorituksetJaArvosanat == null) {
      return Collections.emptyList();
    }

    Stream<SuoritusJaArvosanat> suorituksetEnnenLaskennanAlkamistaMyonnettyjenArvosanojenKanssa = OppijaToAvainArvoDTOConverter
        .removeLaskennanAlkamisenJalkeenMyonnetytArvosanat(oppijanSuorituksetJaArvosanat.stream(),
            parametritDTO);
    List<SuoritusJaArvosanat> kielikoesuoritukset = suorituksetEnnenLaskennanAlkamistaMyonnettyjenArvosanojenKanssa
        .filter(Objects::nonNull).filter(s -> s.getSuoritus() != null)
        .filter(s -> wrap(s).isAmmatillisenKielikoe()).filter(s -> s.getSuoritus().isVahvistettu())
        .filter(s -> s.getArvosanat() != null).filter(s -> !s.getArvosanat().isEmpty())
  ...
  private static List<AvainArvoDTO> suorituksetKielikohtaisiksiAvainArvoiksi(
    ...
    Stream<String> kaikkiKielet = oppijanKielikoesuoritukset.stream().map(SuoritusJaArvosanat::getArvosanat)
        .flatMap(arvosanat -> arvosanat.stream().map(Arvosana::getLisatieto)).distinct();
    kaikkiKielet.forEach(kieli -> {
      List<SuoritusJaArvosanat> kaikkiKielenSuoritukset = oppijanKielikoesuoritukset.stream().filter(
          sja -> sja.getArvosanat().stream().anyMatch(arvosana -> kieli.equals(arvosana.getLisatieto())))
          .collect(Collectors.toList());
      ...
      boolean hyvaksyttyTallaHakemuksella = containsSuoritusWithValue(kielenTamanHakemuksenSuoritukset, kieli,
          hyvaksytty);
      boolean hyvaksyttyMillaVainHakemuksella = containsSuoritusWithValue(kaikkiKielenSuoritukset, kieli,
          hyvaksytty);
      boolean hylattyTallaHakemuksella = containsSuoritusWithValue(kielenTamanHakemuksenSuoritukset, kieli,
          hylatty);
      boolean hylattyMillaVainHakemuksella = containsSuoritusWithValue(kaikkiKielenSuoritukset, kieli, hylatty);
      boolean eiOsallistunutTallaHakemuksella = containsSuoritusWithValue(kielenTamanHakemuksenSuoritukset, kieli,
          ei_osallistunut);
      boolean eiOsallistunutMillaVainHakemuksella = containsSuoritusWithValue(kaikkiKielenSuoritukset, kieli,
          ei_osallistunut);

      if (hyvaksyttyTallaHakemuksella) {
        relevanttiArvoKullekinKielelle
            .addAll(createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(kieli, hyvaksytty,
        ...
        relevanttiArvoKullekinKielelle
            .addAll(createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(kieli, hylatty,
                Osallistuminen.OSALLISTUI));
        ...
        relevanttiArvoKullekinKielelle
            .addAll(createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(kieli,
                ei_osallistunut, Osallistuminen.EI_OSALLISTUNUT));
      } else if (eiOsallistunutMillaVainHakemuksella) {
        relevanttiArvoKullekinKielelle
            .addAll(createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(kieli,
                ei_osallistunut, Osallistuminen.MERKITSEMATTA));
      } else {
        relevanttiArvoKullekinKielelle
            .addAll(createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(kieli, null, null));
      }
    });
    return relevanttiArvoKullekinKielelle;
  }

  private static boolean containsSuoritusWithValue(List<SuoritusJaArvosanat> oppijanSuoritusJaArvosanaParit,
      String kieli, SureHyvaksyttyArvosana expectedValue) {
    return oppijanSuoritusJaArvosanaParit.stream().anyMatch(s -> hasValueInAnyArvosana(s, kieli, expectedValue));
  }

  private static boolean hasValueInAnyArvosana(SuoritusJaArvosanat suoritusJaArvosanat, String kieli,
    ...
    return suoritusJaArvosanat.getArvosanat().stream().anyMatch(a -> {
      ...
      if (!SURE_ASTEIKKO_HYVAKSYTTY.equals(arvio.getAsteikko())) {
        throw new IllegalArgumentException(String.format("Suorituksen %s arvosanan %s asteikko on '%s'",
  ...
  private static List<AvainArvoDTO> createAmmatillisenKielikoeAvainArvoDtoCompatibleWithOldHakuAppData(String kieli,
      SureHyvaksyttyArvosana valueForAvainArvoDto, Osallistuminen osallistuminen) {
    ...
    return Arrays.asList(new AvainArvoDTO("kielikoe_" + kieli.toLowerCase(), valueForAvainArvoDto.arvoForHakuApp),
