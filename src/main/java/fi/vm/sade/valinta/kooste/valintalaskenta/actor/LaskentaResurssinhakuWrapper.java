...
public class LaskentaResurssinhakuWrapper {
  ...
  public static <R> CompletableFuture<R> luoLaskentaResurssinHakuFuture(Supplier<CompletableFuture<R>> source,
      PyynnonTunniste tunniste, boolean retry) {
  ...
  private static <R> CompletableFuture<R> executeWithRetry(Supplier<CompletableFuture<R>> action,
      PyynnonTunniste tunniste) {
    return action.get().handleAsync((r, t) -> {
      if (t != null) {
        return retry(action, t, 0, tunniste);
      } else {
        return CompletableFuture.completedFuture(r);
      }
    }).thenCompose(java.util.function.Function.identity()).whenComplete((r, t) -> {
      if (t != null) {
        LOG.info(
            String.format("%s : Kaikki uudelleenyritykset (%s kpl) on käytetty, ei yritetä enää. Virhe: %s",
                tunniste, MAX_RETRIES, t.getMessage()));
      }
    });
  }

  private static <R> CompletableFuture<R> retry(Supplier<CompletableFuture<R>> action, Throwable throwable, int retry,
      PyynnonTunniste tunniste) {
    ...
    return action.get().handleAsync((r, t) -> {
      if (t != null) {
        ...
        LOG.info(String.format("%s : Resurssin haussa tapahtui virhe %s, uudelleenyritys # %s", tunniste,
            t.getMessage(), retry));
        Executor delayedExecutor = CompletableFuture.delayedExecutor(retry * secondsToWaitMultiplier, SECONDS);
        return CompletableFuture.supplyAsync(() -> "OK", delayedExecutor).thenComposeAsync(
            x -> LaskentaResurssinhakuWrapper.retry(action, throwable, retry + 1, tunniste));
      }
      return CompletableFuture.completedFuture(r);
    }).thenCompose(java.util.function.Function.identity());
  }

  private static <T> BiConsumer<T, Throwable> lopputuloksenKasittelija(PyynnonTunniste tunniste,
      long starTimeMillis) {
    return (r, error) -> {
      if (error != null) {
        ...
        long l = System.currentTimeMillis();
        long duration = l - starTimeMillis;
        LOG.info(String.format("(Uuid=%s) (Kesto %s s) Saatiin resurssi %s hakukohteelle %s", tunniste.uuid,
            MILLISECONDS.toSeconds(duration), tunniste.resurssi, tunniste.hakukohdeOid));
      }
    };
  }
