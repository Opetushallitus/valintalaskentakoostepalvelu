...
public class OmatSivutHakemusResource {
  ...
  public void getValintatulos(@PathParam("hakuOid") String hakuOid, @PathParam("hakemusOid") String hakemusOid,
    ...
    asyncResponse.setTimeoutHandler(handler -> {
      LOG.error(
          ...
          hakuOid, hakemusOid);
      handler.resume(
          Response.serverError().entity("Valintatulokset proxy -palvelukutsu on aikakatkaistu").build());
    });
    valintaTulosServiceAsyncResource.getHakemuksenValintatulosAsString(hakuOid, hakemusOid)
        .subscribe(
            toiveenValintaTulokset -> asyncResponse.resume(Response.ok()
                .header("Content-Type", "application/json").entity(toiveenValintaTulokset).build()),
            error -> {
