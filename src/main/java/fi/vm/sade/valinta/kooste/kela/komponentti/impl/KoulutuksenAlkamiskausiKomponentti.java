...
public class KoulutuksenAlkamiskausiKomponentti {
  private static final Logger LOG =
      LoggerFactory.getLogger(KoulutuksenAlkamiskausiKomponentti.class);

  @Autowired private KoodiService koodiService;

  public int koulutuksenAlkamiskausiUri(String koulutuksenAlkamiskausiUri) {
    for (KoodiType koodi :
        koodiService.searchKoodis(toSearchCriteria(koulutuksenAlkamiskausiUri))) {
      if ("S".equals(StringUtils.upperCase(koodi.getKoodiArvo()))) { // syksy
        ...
        LOG.error(
            ...
            new Object[] {koodi.getKoodiArvo(), koulutuksenAlkamiskausiUri});
        break;
      }
    }
    throw new RuntimeException(
        "Koodistosta ei saatu alkamiskautta URI:lle " + koulutuksenAlkamiskausiUri);
  }
}
