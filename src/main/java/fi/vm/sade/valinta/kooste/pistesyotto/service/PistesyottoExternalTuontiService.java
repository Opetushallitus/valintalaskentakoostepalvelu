...
public class PistesyottoExternalTuontiService {
  ...
  private Supplier<Stream<OsallistuminenHakutoiveeseen>>
      osallistumisenTunnistePuuttuuPalvelunKutsujanSyotteesta(
          String hakutoiveOid,
          HakemusDTO hakemus,
          fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO koe) {
    return () -> {
      boolean osallistumisenTunnistePuuttuuPalvelunKutsujanSyotteesta =
          koe.getOsallistuminen() == null;
      if (osallistumisenTunnistePuuttuuPalvelunKutsujanSyotteesta) {
        ...
        return Stream.of(
            new OsallistuminenHakutoiveeseen(koe.getTunniste(), hakutoiveOid, virheDTO));
      } else {
        return Stream.empty();
      }
    };
  ...
  private Supplier<Stream<OsallistuminenHakutoiveeseen>> eiOsallistunutJaPisteetAnnettu(
    ...
    return () -> {
      if (koe.getOsallistuminen()
              == fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO.Osallistuminen
                  .EI_OSALLISTUNUT
          && koe.getPisteet() != null) {
        VirheDTO virheDTO = new VirheDTO();
        virheDTO.setHakemusOid(hakemus.getHakemusOid());
  ...
  private Optional<String> validoiSyote(
      fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO koe, ValintaperusteDTO peruste) {
    if (fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO.Osallistuminen.OSALLISTUI.equals(
        koe.getOsallistuminen())) {
      if (Funktiotyyppi.LUKUARVOFUNKTIO.equals(peruste.getFunktiotyyppi())) {
        BigDecimal pisteet;
        Optional<String> badPointsMessage =
            Optional.of(
                String.format(
                    "Pisteiden arvon '%s' muuntaminen numeroksi ei onnistunut", koe.getPisteet()));
        try {
        ...
        if (peruste.getArvot() != null && !peruste.getArvot().isEmpty()) {
          // Diskreettiarvo
          boolean diskreettiArvoLoytyi =
              peruste.getArvot().stream().map(BigDecimal::new).anyMatch(pisteet::equals);
        ...
        boolean pisteetIsBooleanString =
  ...
  private Supplier<Stream<OsallistuminenHakutoiveeseen>> valintaperusteetKaikkiKutsutaan(
    ...
    return () -> {
      final boolean valintaperusteetKaikkiKutsutaan =
          Boolean.TRUE.equals(peruste.getSyotettavissaKaikille());
  ...
  private Supplier<Stream<OsallistuminenHakutoiveeseen>> valintalaskentaKutsutaan(
      ...
      ValintaperusteDTO peruste) {
    return () -> {
      Optional<HakutoiveDTO> hakutoiveDTO =
          valintakoeOsallistuminenDTO.flatMap(
              vk ->
                  vk.getHakutoiveet().stream()
                      .filter(hk -> hakutoiveOid.equals(hk.getHakukohdeOid()))
                      .findAny());
      Optional<fi.vm.sade.valintalaskenta.domain.dto.valintakoe.ValintakoeDTO> valintakoeDTO1 =
          hakutoiveDTO.flatMap(
              hk ->
                  hk.getValinnanVaiheet().stream()
                      .flatMap(vv -> vv.getValintakokeet().stream())
                      .filter(vk -> peruste.getTunniste().equals(vk.getValintakoeTunniste()))
                      .findAny());
      return valintakoeDTO1
          .map(
              vk -> {
                final Optional<OsallistuminenTulosDTO> o =
                    Optional.ofNullable(vk.getOsallistuminenTulos());
                final boolean osallistuu =
                    o.map(o0 -> Osallistuminen.OSALLISTUU.equals(o0.getOsallistuminen()))
                        .orElse(false);
                final boolean laskentaKaikkiKutsutaan =
                ...
                if (laskentaKaikkiKutsutaan || osallistuu) {
                  ...
                  String errorMsg =
                      "Hakemuksen "
                          ...
                          + " hakijaa ei ole kutsuttu hakutoiveen "
                          + hakutoiveOid
                          + " tunnisteella "
                          + koe.getTunniste();
                  LOG.error(errorMsg);
          ...
          .orElseGet(
              () -> {
                StringBuilder errorMsg =
                    new StringBuilder("Puuttuva osallistumisen tulos hakutoiveelle ")
                        .append(hakutoiveOid)
                        .append(" tunnisteella ")
                        .append(koe.getTunniste());
  ...
  private Stream<OsallistuminenHakutoiveeseen> getOsallistuminenHakutoiveeseenStream(
    ...
    OsallistuminenHakutoiveeseen osallistuminenHakutoiveeseen =
        syoteVirhe
            .map(
                virhe -> {
                  VirheDTO virheDto = new VirheDTO();
                  virheDto.setHakemusOid(hakemus.getHakemusOid());
                  virheDto.setVirhe(
                      "Validointivirhe pisteiden ("
                          + koe.getPisteet()
                          + ") tuonnissa tunnisteelle "
                          + koe.getTunniste()
                          + ". "
                          + virhe);
                  return new OsallistuminenHakutoiveeseen(
                      koe.getTunniste(), hakutoiveOid, virheDto);
                })
            .orElseGet(
                () -> {
                  ApplicationAdditionalDataDTO additionalDataDTO =
                  ...
                  additionalDataDTO.setAdditionalData(
                      ImmutableMap.of(
                          peruste.getOsallistuminenTunniste(),
                          koe.getOsallistuminen().toString(),
                          peruste.getTunniste(),
                          // only set pisteet for osallistuja
                          Optional.of(koe.getOsallistuminen())
                              .filter(
                                  o ->
                                      fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO
                                          .Osallistuminen.OSALLISTUI
                                          .equals(o))
                              .map(o -> koe.getPisteet())
                              .orElse("")));
                  return new OsallistuminenHakutoiveeseen(
                      koe.getTunniste(), hakutoiveOid, additionalDataDTO);
                });
  ...
  private OsallistuminenHakutoiveeseen validoi(
      String hakutoiveOid,
      HakemusDTO pistetiedotHakemukselle,
      fi.vm.sade.valinta.kooste.pistesyotto.dto.ValintakoeDTO koe,
      ValintaperusteDTO valintaperusteDTO,
      Optional<ValintakoeOsallistuminenDTO> valintakoeOsallistuminenDTO) {
    return Stream.of(
            osallistumisenTunnistePuuttuuPalvelunKutsujanSyotteesta(
                hakutoiveOid, pistetiedotHakemukselle, koe),
            eiOsallistunutJaPisteetAnnettu(hakutoiveOid, pistetiedotHakemukselle, koe),
            valintaperusteetKaikkiKutsutaan(
                hakutoiveOid, pistetiedotHakemukselle, koe, valintaperusteDTO),
            valintalaskentaKutsutaan(
                hakutoiveOid,
  ...
  private Stream<OsallistuminenHakutoiveeseen> luoOsallistumisetHakemukselle(
    ...
    List<OsallistuminenHakutoiveeseen> hakemuksenKokeetStream =
        pistetiedotHakemukselle.getValintakokeet().stream()
            .flatMap(
                koe ->
                    hakutoives.stream()
                        .flatMap(
                            hakutoive -> {
                              ...
                              if (!hakutoive.valintaperusteetDTO.stream()
                                  .map(valintaperusteDTO -> valintaperusteDTO.getTunniste())
                                  .collect(Collectors.toSet())
                                  .contains(koe.getTunniste())) {
                                String errorMessage =
                                    "Valintakoetta ei löydy annetulle tunnisteelle ("
                                        + koe.getTunniste()
                                        + ") käsiteltäessä hakemusta "
                                        + pistetiedotHakemukselle.getHakemusOid()
                                        + " hakutoiveelle "
                                        + hakutoive.hakukohdeOid;
                                LOG.warn(errorMessage);
                                VirheDTO invalidIdentifier = new VirheDTO();
                                invalidIdentifier.setHakemusOid(
                                    pistetiedotHakemukselle.getHakemusOid());
                                invalidIdentifier.setVirhe(errorMessage);
                                return Arrays.asList(
                                    new OsallistuminenHakutoiveeseen(
                                        koe.getTunniste(),
                                        hakutoive.hakukohdeOid,
                                        invalidIdentifier))
                                    .stream();
                              }
                              Stream<ValintaperusteDTO> valintaperusteStream =
                                  hakutoive.valintaperusteetDTO.stream()
                                      .filter(v -> v.getTunniste().equals(koe.getTunniste()));
                              return valintaperusteStream.flatMap(
                                  valintaperuste -> {
                                    Optional<ValintakoeOsallistuminenDTO> osallistuminen =
                                        hakutoive.osallistuminenDTO.stream().findAny();
                                    return Stream.of(
                                        validoi(
                                            hakutoive.hakukohdeOid,
                                            pistetiedotHakemukselle,
    ...
    final Set<String> onnistuneetKokeet =
        hakemuksenKokeetStream.stream()
            .filter(k -> !k.virhe.isPresent())
            .map(k -> k.tunniste)
            .collect(Collectors.toSet());

    Stream<OsallistuminenHakutoiveeseen> osallistuminenHakutoiveeseenStream =
        hakemuksenKokeetStream.stream()
  ...
  private List<HakemusJaHakutoiveet> collect(
      List<HakemusDTO> hakemukset, List<HakemusWrapper> hakemuses) {
    Map<String, HakemusWrapper> hakemusOID2Hakemus =
        hakemuses.stream().collect(Collectors.toMap(HakemusWrapper::getOid, h -> h));
    Map<String, Collection<String>> hakemusOidJaHakutoiveet =
        hakemuses.stream()
            .collect(Collectors.toMap(HakemusWrapper::getOid, HakemusWrapper::getHakutoiveOids));
    return hakemukset.stream()
        .map(
            h ->
                new HakemusJaHakutoiveet(
                    h,
  ...
  public void tuo(
      ...
      List<HakemusDTO> pistetiedotHakemuksille,
      AuditSession auditSession,
      String hakuOid,
      BiConsumer<Integer, Collection<VirheDTO>> successHandler,
      Consumer<Throwable> exceptionHandler) {
    getHakemuksetByHakemusOids(
            pistetiedotHakemuksille.stream()
                .map(HakemusDTO::getHakemusOid)
                .collect(Collectors.toList()))
        .subscribe(
            hakemusWrappers -> {
              List<HakemusJaHakutoiveet> pistetiedotJaHakemukset =
                  collect(pistetiedotHakemuksille, hakemusWrappers);
              Set<String> hakutoiveet =
                  pistetiedotJaHakemukset.stream()
                      .flatMap(h -> h.hakutoiveet.stream())
                      .collect(Collectors.toSet());
              ...
              Observable.combineLatest(
                      ...
                      osallistumisetHakutoiveille,
                      (hakukohdeJaValintaperusteDTOs, osallistuminenDTOs) -> {
                        Map<String, HakukohdeJaValintaperusteDTO>
                            hakukohdeOidToValintaperusteDTOMap =
                                hakukohdeJaValintaperusteDTOs.stream()
                                    .collect(
                                        Collectors.toMap(
                                            HakukohdeJaValintaperusteDTO::getHakukohdeOid,
                                            hh -> hh));

                        osallistuminenDTOs.stream()
                            .forEach(
                                o ->
                                    o.getHakutoiveet()
                                        .forEach(
                                            ht ->
                                                ht.getValinnanVaiheet()
                                                    .forEach(vv -> vv.getValintakokeet())));
                        Map<String, List<ValintakoeOsallistuminenDTO>>
                                ...
                                osallistuminenDTOs.stream()
                                    .collect(
                                        Collectors.toMap(
                                            ...
                                            Arrays::asList,
                                            (h0, h1) ->
                                                Lists.newArrayList(Iterables.concat(h0, h1))));

                        return pistetiedotJaHakemukset.stream()
                            .flatMap(
                                pistetiedotJaHakemus -> {
                                  ...
                                  if (hakemus == null) {
                                    String errorMessage =
                                        "Hakemusta ei löydy tuotaessa pistetietoja hakemukselle "
                                  ...
                                  if (!pistetiedot.getHenkiloOid().equals(hakemus.getPersonOid())) {
                                    ...
                                    return Stream.of(
                                        new OsallistuminenHakutoiveeseen(
                                            null, null, conflictingPersonOid));
                                  }
                                  List<Hakutoive> hakutoiveetList =
                                      pistetiedotJaHakemus.hakutoiveet.stream()
                                          .map(
                                              hakukohdeOid ->
                                                  new Hakutoive(
                                                      hakukohdeOid,
                                                      ...
                                                      Optional.ofNullable(
                                                          ...
                                                          .orElse(Collections.emptyList())))
                                          .collect(Collectors.toList());
                                  return luoOsallistumisetHakemukselle(
                                          pistetiedotJaHakemus, hakutoiveetList)
                                      .map(
                                          osallistuminen -> {
                                            boolean isAuthorized =
                                                authorityCheck.test(osallistuminen.hakukohdeOid);
                                            if (isAuthorized) {
                                              return osallistuminen;
                                            } else {
                                              String errorMessage =
                                                  "Tarvittavat muokkausoikeudet puuttuvat hakutoiveelle "
                                              ...
                                              if (osallistuminen.isVirhe()) {
                                                ...
                                                virheDTO.setHakemusOid(
                                                    osallistuminen
                                                        .asApplicationAdditionalDataDTO()
                                                        .getOid());
                                              }
                        ...
                        if (!additionalData.isEmpty()) {
                          ...
                          Observable.fromFuture(
                                  valintapisteAsyncResource.putValintapisteet(
                                      Optional.empty(), vp, auditSession))
                              .subscribe(
                                  conflictingHakemusOids -> {
                                    additionalData.forEach(
                                        pistetieto -> {
                                          Map<String, String> additionalAuditInfo = new HashMap<>();
                                          additionalAuditInfo.put(
                                              "Username from params", auditSession.getPersonOid());
                                          additionalAuditInfo.put("hakuOid", hakuOid);
                                          additionalAuditInfo.put(
                                              "hakijaOid", pistetieto.getPersonOid());
                                          AuditLog.log(
                                              KoosteAudit.AUDIT,
                                    ...
                                    List<VirheDTO> valintapisteVirheet =
                                        conflictingHakemusOids.stream()
                                            .map(
                                                hakemusOid -> {
                                                  String errorMessage =
                                                      "Yritettiin kirjoittaa yli uudempia pistetietoja hakemukselle "
                                                          + hakemusOid;
                                                  LOG.error(errorMessage);
  ...
  private static class OsallistuminenHakutoiveeseen {
    ...
    public OsallistuminenHakutoiveeseen(
        String tunniste,
        String hakukohdeOid,
  ...
  private static class Hakutoive {
    ...
    public Hakutoive(
        String hakukohdeOid,
        List<ValintaperusteDTO> valintaperusteetDTO,
  ...
  private static class HakemusJaHakutoiveet {
    ...
    public HakemusJaHakutoiveet(
        HakemusDTO hakemusDTO, Collection<String> hakutoiveet, HakemusWrapper hakemus) {
