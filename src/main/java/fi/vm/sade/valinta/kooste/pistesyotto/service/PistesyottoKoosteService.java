...
public class PistesyottoKoosteService extends AbstractPistesyottoKoosteService {
  ...
  public CompletableFuture<PistesyottoValilehtiDTO> koostaOsallistujienPistetiedot(String hakuOid,
      String hakukohdeOid, AuditSession auditSession) {
    try {
      CompletableFuture<PisteetWithLastModified> valintapisteetF = valintapisteAsyncResource
          .getValintapisteet(hakuOid, hakukohdeOid, auditSession);
      CompletableFuture<Map<String, ValintakoeOsallistuminenDTO>> osallistuminenByHakemusF = valintalaskentaValintakoeAsyncResource
          .haeHakutoiveelle(hakukohdeOid)
          .thenApplyAsync(vs -> vs.stream().collect(Collectors.toMap(v -> v.getHakemusOid(), v -> v)));
      CompletableFuture<Map<String, Oppija>> oppijaByPersonOidF = suoritusrekisteriAsyncResource
      ...
      CompletableFuture<List<ValintaperusteDTO>> valintaperusteetF = valintaperusteetAsyncResource
          .findAvaimet(hakukohdeOid);
      CompletableFuture<ParametritDTO> ohjausparametritF = ohjausparametritAsyncResource
          .haeHaunOhjausparametrit(hakuOid);
      return CompletableFuture.allOf(valintapisteetF, valintaperusteetF, osallistuminenByHakemusF,
          oppijaByPersonOidF, ohjausparametritF).thenApplyAsync(x -> {
            ...
            return new PistesyottoValilehtiDTO(valintapisteet.lastModified.orElse(null),
                valintapisteet.valintapisteet.stream()
                    .map(vps -> new HakemuksenKoetulosYhteenveto(vps,
                        Pair.of(hakukohdeOid, valintaperusteet),
                        valintakokeet.get(vps.getHakemusOID()), oppijat.get(vps.getOppijaOID()),
                        ohjausparametrit))
  ...
  private Observable<List<Pair<String, List<ValintaperusteDTO>>>> getValintaperusteet(HakemusWrapper hakemus) {
    return Observable.merge(hakemus.getHakutoiveOids().stream()
        ...
        .collect(Collectors.toList())).toList().toObservable();
  }
  ...
  public Observable<HenkiloValilehtiDTO> koostaOsallistujanPistetiedot(String hakemusOid, AuditSession auditSession) {
    return getHakemus(hakemusOid).flatMap(hakemus -> Observable.zip(getValintaperusteet(hakemus),
        Observable.fromFuture(valintapisteAsyncResource
            .getValintapisteetWithHakemusOidsAsFuture(Collections.singletonList(hakemusOid), auditSession)),
        Observable.fromFuture(valintalaskentaValintakoeAsyncResource.haeHakemukselle(hakemusOid)),
        ...
        (valintaperusteetByHakukohdeOid, valintapisteet, valintakoeOsallistuminen, oppija,
            ohjausparametrit) -> new HenkiloValilehtiDTO(valintapisteet.lastModified.orElse(null),
                valintaperusteetByHakukohdeOid.stream().collect(Collectors.toMap(Pair::getLeft,
                    valintaperusteet -> new HakemuksenKoetulosYhteenveto(
                        valintapisteet.valintapisteet.iterator().next(), valintaperusteet,
                        valintakoeOsallistuminen, oppija, ohjausparametrit))))));
  }
  ...
  private static Map<String, HakutoiveDTO> kielikokeidenHakukohteet(ValintakoeOsallistuminenDTO voDTO) {
    ...
    voDTO.getHakutoiveet()
        .forEach(h -> h.getValinnanVaiheet().stream().flatMap(vaihe -> vaihe.getValintakokeet().stream())
            ...
            .forEach(koe -> {
              ...
              if (kielikokeidenHakukohteet.containsKey(koetunniste)) {
                throw new IllegalStateException(String.format(
                    ...
                    voDTO.getHakemusOid(), koetunniste, kielikokeidenHakukohteet.get(koetunniste),
                    h.getHakukohdeOid()));
              }
  ...
  private static ApplicationAdditionalDataDTO poistaKielikoepistetiedot(ApplicationAdditionalDataDTO pistetietoDTO) {
    ApplicationAdditionalDataDTO a = new ApplicationAdditionalDataDTO(pistetietoDTO.getOid(),
        pistetietoDTO.getPersonOid(), pistetietoDTO.getFirstNames(), pistetietoDTO.getLastName(),
        new HashMap<>());
  ...
  public Observable<Set<TuontiErrorDTO>> tallennaKoostetutPistetiedotHakemukselle(
      ApplicationAdditionalDataDTO pistetietoDTO, Optional<String> ifUnmodifiedSince, AuditSession auditSession) {
    return Observable.fromFuture(valintalaskentaValintakoeAsyncResource.haeHakemukselle(pistetietoDTO.getOid()))
        .flatMap(vo -> {
          ...
          if (kielikoePistetiedot.isEmpty()) {
            return Observable
                .fromFuture(valintapisteAsyncResource.putValintapisteet(ifUnmodifiedSince,
                    singletonList(new Valintapisteet(
                        Pair.of(auditSession.getPersonOid(), pistetietoDTO))),
                    auditSession))
                .map(hakemusOids -> hakemusOids.stream()
                    .map(hakemusOid -> new TuontiErrorDTO(hakemusOid,
                        pistetietoDTO.getFirstNames() + " " + pistetietoDTO.getLastName(),
                        "Yritettiin kirjoittaa yli uudempia pistetietoja"))
                    .collect(Collectors.toSet()));
          } else {
            Observable<Set<TuontiErrorDTO>> errors = Observable
                .merge(kielikoePistetiedot.keySet().stream().map(kielikoetunniste -> {
                  ApplicationAdditionalDataDTO a = poistaKielikoepistetiedot(pistetietoDTO);
                  a.getAdditionalData().put(kielikoetunniste,
                      kielikoePistetiedot.get(kielikoetunniste));
                  return Observable.fromFuture(tallennaKoostetutPistetiedot(hakuOid,
                      kh.get(kielikoetunniste).getHakukohdeOid(), ifUnmodifiedSince,
  ...
  public CompletableFuture<Set<TuontiErrorDTO>> tallennaKoostetutPistetiedot(String hakuOid, String hakukohdeOid,
      Optional<String> ifUnmodifiedSince, List<ApplicationAdditionalDataDTO> pistetietoDTOs,
    ...
    try {
      pistetiedotHakemukselle = createAdditionalDataAndPopulateKielikoetulokset(pistetietoDTOs,
          kielikoetuloksetSureen);
      return tallennaKoostetutPistetiedot(hakuOid, hakukohdeOid, ifUnmodifiedSince, pistetiedotHakemukselle,
          kielikoetuloksetSureen, ValintaperusteetOperation.PISTETIEDOT_KAYTTOLIITTYMA, auditSession)
              .thenApplyAsync(hakemusOids -> pistetiedotHakemukselle.stream()
                  .filter(dto -> hakemusOids.contains(dto.getOid()))
                  .map(dto -> new TuontiErrorDTO(dto.getOid(),
                      dto.getFirstNames() + " " + dto.getLastName(),
                      "Yritettiin kirjoittaa yli uudempia pistetietoja"))
                  .collect(Collectors.toSet()));
    } catch (Exception e) {
      LOG.error(String.format("Ongelma käsiteltäessä pistetietoja haun %s kohteelle %s , käyttäjä %s ", hakuOid,
          hakukohdeOid, auditSession.getPersonOid()), e);
      return CompletableFuture.failedFuture(e);
    }
  }
  ...
  private List<ApplicationAdditionalDataDTO> createAdditionalDataAndPopulateKielikoetulokset(
    ...
    return pistetietoDTOs.stream().flatMap(pistetieto -> {
      ...
      siirraKielikoepistetiedotKielikoetulosMapiin(valmistuminen, kielikoetuloksetSureen, hakemusOid,
          additionalData);
