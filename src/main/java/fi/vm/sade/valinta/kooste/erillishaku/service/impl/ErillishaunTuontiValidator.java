...
public class ErillishaunTuontiValidator {
  ...
  protected List<String> validoi(
    ...
    // ehdollisenHyvaksymisenEhtoKoodi and just take the code part:
  ...
  private void validateTunniste(ErillishakuRivi rivi, List<String> errors) {
    // Yksilöinti onnistuu, eli joku kolmesta löytyy:
    ...
    (!isBlank(rivi.getSyntymaAika()) && !Sukupuoli.EI_SUKUPUOLTA.equals(rivi.getSukupuoli()))
      ...
      errors.add(
          "Henkilötunnus, syntymäaika + sukupuoli ja henkilö-oid olivat tyhjiä (vähintään yksi tunniste on syötettävä).");
    }
  }
  ...
  private void validateEhdollinenHyvaksynta(ErillishakuRivi rivi, List<String> errors) {
    if (rivi.getEhdollisestiHyvaksyttavissa()) {
      ...
      if (!ehdot.isEmpty()) {
        ...
        if (!sallittuehto)
          errors.add(
              "Jos ehdollinen hyväksyntä on aktiivinen, on hyväksymisen ehdon oltava jokin pudotusvalikon arvoista.");
      }
    }
  }
