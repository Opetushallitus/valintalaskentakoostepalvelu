...
public class AbstractSecurityTicketOutInterceptor<T extends Message> extends
	...
	static class ProxyAuthenticator {
		...
		public void proxyAuthenticate(String casTargetService, String authMode,
				Callback callback) {
			...
			try {
				...
				if (authentication != null && "dev".equals(authMode)) {
					...
					proxyAuthenticateCas(casTargetService, callback,
							authentication);
				}
		...
		protected void proxyAuthenticateCas(String casTargetService,
				Callback callback, Authentication authentication) {
			String proxyTicket = getCachedProxyTicket(casTargetService,
					authentication, callback);
			if (proxyTicket == null) {
				throw new BadCredentialsException(
						"got null proxyticket, cannot attach to request, casTargetService: "
								+ casTargetService + ", authentication: "
								+ authentication);
			} else {
		...
		static class PERA {
			...
			public static final String X_PALVELUKUTSU_LAHETTAJA_PROXY_AUTH = "X-Palvelukutsu.Lahettaja.ProxyAuth"; // ei
																													// perassa

			public static void setKayttajaHeaders(HttpRequestBase req,
					String currentUser, String callAsUser) {
				req.setHeader(X_KUTSUKETJU_ALOITTAJA_KAYTTAJA_TUNNUS,
						currentUser);
				req.setHeader(X_PALVELUKUTSU_LAHETTAJA_KAYTTAJA_TUNNUS,
						callAsUser);
			}

			public static void setProxyKayttajaHeaders(
					ProxyAuthenticator.Callback callback, String currentUser) {
				callback.setRequestHeader(
						X_KUTSUKETJU_ALOITTAJA_KAYTTAJA_TUNNUS, currentUser);
				callback.setRequestHeader(
						X_PALVELUKUTSU_LAHETTAJA_KAYTTAJA_TUNNUS, currentUser);
				callback.setRequestHeader(X_PALVELUKUTSU_LAHETTAJA_PROXY_AUTH,
						"true");
			}
		}

		protected void proxyAuthenticateDev(Callback callback,
				Authentication authentication) {
			callback.setRequestHeader("CasSecurityTicket",
					"oldDeprecatedSecurity_REMOVE");
			String user = authentication.getName();
			String authorities = toString(authentication.getAuthorities());
			callback.setRequestHeader("oldDeprecatedSecurity_REMOVE_username",
					user);
			callback.setRequestHeader(
		...
		public String getCachedProxyTicket(final String targetService,
				final Authentication authentication, final Callback callback) {
			String proxyTicket = obtainNewCasProxyTicket(targetService,
					authentication);
			if (callback != null) {
		...
		protected String obtainNewCasProxyTicket(String casTargetService,
				Authentication authentication) {
			if (authentication == null
					|| authentication instanceof AnonymousAuthenticationToken) {
				throw new RuntimeException("current user is not authenticated");
			}
			String ticket = ((CasAuthenticationToken) authentication)
	...
	public void handleMessage(final T message) throws Fault {
		final String casTargetService = getCasTargetService((String) message
	...
	public void handleFault(Message message) {
		...
		if (authentication instanceof CasAuthenticationToken) {
			String casTargetService = getCasTargetService((String) message
					.get(Message.ENDPOINT_ADDRESS));
