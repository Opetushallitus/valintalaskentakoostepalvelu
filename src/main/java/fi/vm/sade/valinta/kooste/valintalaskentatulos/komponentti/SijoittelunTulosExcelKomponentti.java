...
public class SijoittelunTulosExcelKomponentti {
  ...
  public InputStream luoXls(List<Valintatulos> valintatulokset, String preferoitukielikoodi, String hakukohdeNimi,
      String tarjoajaNimi, String hakukohdeOid, List<HakemusWrapper> hakemuksetList,
      List<Lukuvuosimaksu> lukuvuosimaksut, HakukohdeDTO hakukohde, Haku hakuDTO,
      List<ValintatietoValinnanvaiheDTO> valinnanVaiheet) {
    Map<String, Koodi> countryCodes = koodistoCachedAsyncResource
    ...
    Map<String, HakemusWrapper> hakemukset = hakemuksetList.stream()
    ...
    List<ValintatapajonoDTO> valintatapajonot = ofNullable(hakukohde.getValintatapajonot())
        .orElse(Collections.emptyList()).stream()
        .filter(v -> v.getHakemukset() != null && !v.getHakemukset().isEmpty()).collect(Collectors.toList());
    if (valintatapajonot.isEmpty()) {
      LOG.error(
          "Yritettiin muodostaa sijoittelun tuloksista taulukkolaskenta kohteelle({}) jolla ei ole valintatapajonoja saatavilla!",
          hakukohdeOid);
      throw new RuntimeException("Yritettiin muodostaa sijoittelun tuloksista taulukkolaskenta kohteelle("
          + hakukohdeOid + ") jolla ei ole valintatapajonoja saatavilla!");
    }
    Collections.sort(valintatapajonot, (o1, o2) -> {
    ...
    otsikkoRivi.addAll(Arrays.asList("Hakemus", "Hakija", "Oppijanumero", "Henkilötunnus", "Syntymäaika",
        "Sukupuoli", "Äidinkieli", "Asiointikieli", "Kansalaisuus", "Lähiosoite", "Postinumero",
        "Postitoimipaikka", "Osoite (ulkomaa)", "Postinumero (ulkomaa)", "Kaupunki (ulkomaa)", "Asuinmaa",
        "Kansallinen ID", "Passin numero", "Maksun tila", "Sähköposti", "Puhelinnumero", "Lupa julkaisuun",
        "Lupa sähköiseen asiointiin", "Hakutoive", "Hakijaryhmät"));
    List<Object> valintatapajonoOtsikkoRivi = new ArrayList<>(Collections.nCopies(otsikkoRivi.size(), "")); // alun
    // tyhjat
    // pystyrivit
    {
      int index = 0;
      for (ValintatapajonoDTO jono : valintatapajonot) {
        ...
        List<Object> otsikot = isKkHaku
            ? Arrays.asList("Jonosija", "Pisteet", "Sijoittelun tila", "Hylkäyksen syy", "Vastaanottotieto",
                "Ilmoittautumistieto", "Ehdollinen valinta", "Ehdollisen hyväksymisen ehto",
                "Ehdollisen hyväksymisen FI", "Ehdollisen hyväksymisen SV",
                "Ehdollisen hyväksymisen EN", "Muokattu")
            : Arrays.asList("Jonosija", "Pisteet", "Sijoittelun tila", "Hylkäyksen syy", "Vastaanottotieto",
    ...
    Map<String, Map<String, HakemusDTO>> jonoOidHakemusOidHakemusDto = valintatapajonot.stream()
        .collect(Collectors.toMap(ValintatapajonoDTO::getOid,
            v -> v.getHakemukset().stream().collect(Collectors.toMap(HakemusDTO::getHakemusOid, h -> h))));

    Map<String, Lukuvuosimaksu> personOidToLukuvuosimaksu = lukuvuosimaksut.stream()
        .collect(Collectors.toMap(Lukuvuosimaksu::getPersonOid, l -> l));
    ...
    for (HakemusDTO hDto : distinctHakemuksetFromAllQueues) {
      ...
      if (wrapper == null) {
        throw new NullPointerException(String.format("Hakijan %s hakemuksen %s tietoja ei löydy! "
            ...
            hDto.getHakijaOid(), hDto.getHakemusOid(), hakuDTO.oid, hakukohdeOid,
            hDto.getValintatapajonoOid(), hakemukset.size(), distinctHakemuksetFromAllQueues.size()));
      }
      String nimi = wrapper.getSukunimi() + ", " + wrapper.getEtunimet();
      List<Object> hakemusRivi = Lists.newArrayList();
      Maksuntila maksuntila = ofNullable(personOidToLukuvuosimaksu.get(hDto.getHakijaOid()))
          .map(Lukuvuosimaksu::getMaksuntila).orElse(Maksuntila.MAKSAMATTA);
      hakemusRivi.addAll(Arrays.asList(hDto.getHakemusOid(), nimi, wrapper.getPersonOid(),
          wrapper.getHenkilotunnus(), wrapper.getSyntymaaika(), wrapper.getSukupuoli(),
          wrapper.getAidinkieli(), wrapper.getAsiointikieli(), wrapper.getKansalaisuus(),
          wrapper.getSuomalainenLahiosoite(), wrapper.getSuomalainenPostinumero(),
          postitoimipaikka(postCodes, wrapper), wrapper.getUlkomainenLahiosoite(),
          wrapper.getUlkomainenPostinumero(), wrapper.getKaupunkiUlkomaa(),
          countryNameInEnglish(countryCodes, wrapper), wrapper.getKansallinenId(), wrapper.getPassinnumero(),
          wrapper.isMaksuvelvollinen(hakukohdeOid) ? maksuntila.toString() : "",
          wrapper.getSahkopostiOsoite(), wrapper.getPuhelinnumero(),
      ...
      for (ValintatapajonoDTO jono : valintatapajonot) {
        ...
        if (jonoOidToHakemusOid.containsKey(hDto.getHakemusOid())) {
          ...
          String ilmoittautumistieto = HakemusUtil.tilaConverter(
              hakemusJaJonoMappaus.get(hakemusOidJaValintatapajonoOidYhdiste(hakemusOid, jono.getOid())),
              preferoitukielikoodi);

          List<Valintatulos> hakemuksenValintatulokset = valintatulokset.stream()
              .filter(t -> hakemusOid.equals(t.getHakemusOid())).collect(Collectors.toList());

          // Etsitään nyt käsiteltävää sijoittelujonoa vastaava valintalaskennan
          // valintatietojono.
          // Huom. Eri DTO kuin ylempänä samalla nimellä, eri sisältö.
          List<fi.vm.sade.valintalaskenta.domain.dto.ValintatapajonoDTO> valintatietoJono = valinnanVaiheet
              .stream().flatMap(vaihe -> vaihe.getValintatapajonot().stream()
                  .filter(j -> j.getOid().equals(jono.getOid())))
          ...
          if (!valintatietoJono.isEmpty() && hakemusDto.getTila() == HakemuksenTila.HYLATTY) {
            ...
            // Näytetään löytynyt kuvaus preferoidulla kielellä jos mahdollista, mutta
            // fallback
            // muille kielille tarvittaessa: FI > SV > EN
            if (jonosija != null) {
          ...
          for (Valintatulos valintatulos : hakemuksenValintatulokset) {
            if (jono.getOid().equals(valintatulos.getValintatapajonoOid())) {
              if (valintatulos.getTila() != null) {
                ...
                ehdollinenValinta = HakemusUtil
                    .ehdollinenValinta(valintatulos.getEhdollisestiHyvaksyttavissa());
          ...
          List<Object> jonoHakemusSarakkeet = isKkHaku
              ? Arrays.asList(hakemusDto.getJonosija(), Formatter.suomennaNumero(hakemusDto.getPisteet()),
                  HakemusUtil.tilaConverter(hakemusDto.getTila(), preferoitukielikoodi,
                      hakemusDto.isHyvaksyttyHarkinnanvaraisesti(), false, true,
                      hakemusDto.getVarasijanNumero(), ehdollisenHyvaksymisenEhto),
                  hylkayksenSyy, valintaTieto, ilmoittautumistieto, ehdollinenValinta,
                  ehdollisenHyvaksymisenEhto, ehdollisenHyvaksymisenEhtoFI,
                  ehdollisenHyvaksymisenEhtoSV, ehdollisenHyvaksymisenEhtoEN,
                  muokattu(hakemusDto.getTilaHistoria()))
              : Arrays.asList(hakemusDto.getJonosija(), Formatter.suomennaNumero(hakemusDto.getPisteet()),
                  HakemusUtil.tilaConverter(hakemusDto.getTila(), preferoitukielikoodi,
                      hakemusDto.isHyvaksyttyHarkinnanvaraisesti(), false, true,
                      hakemusDto.getVarasijanNumero(), ehdollisenHyvaksymisenEhto),
                  hylkayksenSyy, valintaTieto, ilmoittautumistieto,
                  muokattu(hakemusDto.getTilaHistoria()));

          if (index % 2 == 1) {
            jonoHakemusSarakkeet = jonoHakemusSarakkeet.stream().map(Highlight::new)
                .collect(Collectors.toList());
          }
  ...
  private Map<String, IlmoittautumisTila> valintatapajononTilat(List<Valintatulos> tilat) {
    ...
    try {
      for (Valintatulos tulos : tilat) {
        t.put(hakemusOidJaValintatapajonoOidYhdiste(tulos.getHakemusOid(), tulos.getValintatapajonoOid()),
  ...
  private String postitoimipaikka(Map<String, Koodi> postCodes, HakemusWrapper wrapper) {
    return KoodistoCachedAsyncResource.haeKoodistaArvo(postCodes.get(wrapper.getSuomalainenPostinumero()),
  ...
  private String countryNameInEnglish(Map<String, Koodi> countryCodes, HakemusWrapper wrapper) {
    return KoodistoCachedAsyncResource.haeKoodistaArvo(countryCodes.get(wrapper.getAsuinmaa()), KieliUtil.ENGLANTI,
        wrapper.getAsuinmaa());
  }
  ...
  private List<HakijaryhmaDTO> getHakijaryhmatWithHakemuksia(HakukohdeDTO hakukohde) {
    return ofNullable(hakukohde.getHakijaryhmat()).orElse(Collections.emptyList()).stream()
        .filter(h -> h.getHakemusOid() != null && !h.getHakemusOid().isEmpty()).collect(Collectors.toList());
  }

  /**
   * Palauttaa mäppäyksen Hakijaryhmän hakemusOid:n ja siihen liittyvien
   * hakijaryhmien nimien välillä.
   *
   * @param hakijaryhmat
   *          Lista HakijaryhmaDTO-olioita
   ...
   *
   * @param hakemukset
   *          Lista HakemusDTO-olioita
   */
  private void sortHakemuksetByTila(List<HakemusDTO> hakemukset) {
    Collections.sort(hakemukset, new Comparator<HakemusDTO>() {
