...
public class SijoittelunTulosExcelKomponentti {
  ...
  @Autowired private KoodistoCachedAsyncResource koodistoCachedAsyncResource;
  ...
  public InputStream luoXls(
    ...
    List<ValintatapajonoDTO> valintatapajonot =
        ofNullable(hakukohde.getValintatapajonot()).orElse(Collections.emptyList()).stream()
    ...
    if (valintatapajonot.isEmpty()) {
      ...
      throw new RuntimeException(
          "Yritettiin muodostaa sijoittelun tuloksista taulukkolaskenta kohteelle("
              + hakukohdeOid
              + ") jolla ei ole valintatapajonoja saatavilla!");
    }
    Collections.sort(
        valintatapajonot,
        (o1, o2) -> {
    ...
    Map<String, List<String>> mapHakemusOidToHakijaryhmaNimet =
        mapHakemusOidToHakijaryhmaNimet(hakijaryhmat);
    ...
    otsikkoRivi.addAll(
        Arrays.asList(
            ...
            "Hakutoive",
            "Hakijaryhmät"));
    List<Object> valintatapajonoOtsikkoRivi =
        new ArrayList<>(Collections.nCopies(otsikkoRivi.size(), "")); // alun tyhjat pystyrivit
    {
      int index = 0;
      for (ValintatapajonoDTO jono : valintatapajonot) {
        ++index;
        ...
        valintatapajonoOtsikkoRivi.add(
            new Span("Valintatapajono: " + jono.getNimi(), spanColumns, highlight));
        List<Object> otsikot =
            isKkHaku
                ? Arrays.asList(
                    "Jonosija",
        ...
        valintatapajonot.stream()
            .collect(
                Collectors.toMap(
                    ...
                    v ->
                        v.getHakemukset().stream()
                            .collect(Collectors.toMap(HakemusDTO::getHakemusOid, h -> h))));

    Map<String, Lukuvuosimaksu> personOidToLukuvuosimaksu =
        lukuvuosimaksut.stream().collect(Collectors.toMap(Lukuvuosimaksu::getPersonOid, l -> l));
    ...
    for (HakemusDTO hDto : distinctHakemuksetFromAllQueues) {
      ...
      if (wrapper == null) {
        throw new NullPointerException(
            String.format(
                "Hakijan %s hakemuksen %s tietoja ei löydy! "
      ...
      for (ValintatapajonoDTO jono : valintatapajonot) {
        ++index;
        Map<String, HakemusDTO> jonoOidToHakemusOid =
            jonoOidHakemusOidHakemusDto.get(jono.getOid());
        if (jonoOidToHakemusOid.containsKey(hDto.getHakemusOid())) {

          HakemusDTO hakemusDto =
              jonoOidHakemusOidHakemusDto.get(jono.getOid()).get(hDto.getHakemusOid());
          String hakemusOid = hakemusDto.getHakemusOid();

          String ilmoittautumistieto =
              HakemusUtil.tilaConverter(
                  hakemusJaJonoMappaus.get(
                      hakemusOidJaValintatapajonoOidYhdiste(hakemusOid, jono.getOid())),
                  preferoitukielikoodi);

          List<Valintatulos> hakemuksenValintatulokset =
              valintatulokset.stream()
                  .filter(t -> hakemusOid.equals(t.getHakemusOid()))
                  .collect(Collectors.toList());

          // Etsitään nyt käsiteltävää sijoittelujonoa vastaava valintalaskennan valintatietojono.
          // Huom. Eri DTO kuin ylempänä samalla nimellä, eri sisältö.
          List<fi.vm.sade.valintalaskenta.domain.dto.ValintatapajonoDTO> valintatietoJono =
          ...
          if (!valintatietoJono.isEmpty() && hakemusDto.getTila() == HakemuksenTila.HYLATTY) {
            JonosijaDTO jonosija =
                valintatietoJono.get(0).getJonosijat().stream()
                    .filter(sija -> hakemusOid.equals(sija.getHakemusOid()))
                    .findFirst()
                    .orElse(null);
            // Näytetään löytynyt kuvaus preferoidulla kielellä jos mahdollista, mutta fallback
            // muille kielille tarvittaessa: FI > SV > EN
            if (jonosija != null) {
              Map<String, String> hylkayksenSyyt =
                  jonosija.getJarjestyskriteerit().first().getKuvaus();
          ...
          for (Valintatulos valintatulos : hakemuksenValintatulokset) {
            if (jono.getOid().equals(valintatulos.getValintatapajonoOid())) {
              if (valintatulos.getTila() != null) {
                valintaTieto =
                    HakemusUtil.tilaConverter(valintatulos.getTila(), preferoitukielikoodi);
                ehdollinenValinta =
                    HakemusUtil.ehdollinenValinta(valintatulos.getEhdollisestiHyvaksyttavissa());
          ...
          List<Object> jonoHakemusSarakkeet =
              isKkHaku
                  ? Arrays.asList(
          ...
          if (index % 2 == 1) {
            jonoHakemusSarakkeet =
                jonoHakemusSarakkeet.stream().map(Highlight::new).collect(Collectors.toList());
          }
  ...
  private String hakemusOidJaValintatapajonoOidYhdiste(
  ...
  private Map<String, IlmoittautumisTila> valintatapajononTilat(List<Valintatulos> tilat) {
    ...
    try {
      for (Valintatulos tulos : tilat) {
        t.put(
            hakemusOidJaValintatapajonoOidYhdiste(
                tulos.getHakemusOid(), tulos.getValintatapajonoOid()),
  ...
  private String postitoimipaikka(Map<String, Koodi> postCodes, HakemusWrapper wrapper) {
    return KoodistoCachedAsyncResource.haeKoodistaArvo(
        postCodes.get(wrapper.getSuomalainenPostinumero()),
        KieliUtil.SUOMI,
        wrapper.getSuomalainenPostinumero());
  }
  ...
  private String countryNameInEnglish(Map<String, Koodi> countryCodes, HakemusWrapper wrapper) {
    return KoodistoCachedAsyncResource.haeKoodistaArvo(
        countryCodes.get(wrapper.getAsuinmaa()), KieliUtil.ENGLANTI, wrapper.getAsuinmaa());
  }
  ...
  /**
   * Palauttaa mäppäyksen Hakijaryhmän hakemusOid:n ja siihen liittyvien hakijaryhmien nimien
   * välillä.
   *
   * @param hakijaryhmat Lista HakijaryhmaDTO-olioita
   * @return Map, jossa hakemusOid mäpätty listaan, jossa valintaryhmien nimet
   */
  private Map<String, List<String>> mapHakemusOidToHakijaryhmaNimet(
