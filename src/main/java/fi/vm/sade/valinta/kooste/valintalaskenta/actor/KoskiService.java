...
public class KoskiService {
  ...
  public KoskiService(
    ...
    this.koskiOpiskeluoikeusHistoryService = new KoskiOpiskeluoikeusHistoryService(
  ...
  public CompletableFuture<Map<String, KoskiOppija>> haeKoskiOppijat(String hakukohdeOid,
      ...
      CompletableFuture<List<HakemusWrapper>> hakemukset, SuoritustiedotDTO suoritustiedotDTO, Date nyt) {
    if (koskiHakukohdeOidFilter.test(hakukohdeOid)) {
      return CompletableFuture.allOf(valintaperusteet, hakemukset).thenComposeAsync(x -> {
        ...
        if (sisaltaaKoskiFunktioitaLaskennassaMukanaOlevissaJonoissa(valintaperusteet.join(), nyt)) {
          LocalDate paivaJonkaMukaisiaTietojaKoskiDatastaKaytetaan = koskiOpiskeluoikeusHistoryService
              .etsiKoskiDatanLeikkuriPvm(valintaperusteet.join(), hakukohdeOid);
  ...
  private CompletionStage<Map<String, KoskiOppija>> haeKoskiOppijat(String hakukohdeOid,
      Collection<HakemusWrapper> hakemusWrappers, SuoritustiedotDTO suoritustiedotDTO,
      LocalDate paivaJonkaMukaisiaTietojaKoskiDatastaKaytetaan) {
    LOG.info(String.format("Haetaan Koskesta tiedot %d oppijalle hakukohteen %s laskemista varten.",
    ...
    int maaraJoilleTiedotJoLoytyvat = hakemusWrappers.size()
        - oppijanumerotJoiltaKoskiOpiskeluoikeudetPuuttuvat.size();
    return koskiAsyncResource.findKoskiOppijat(oppijanumerotJoiltaKoskiOpiskeluoikeudetPuuttuvat)
        .thenApplyAsync(koskioppijat -> {
          LOG.info(String.format(
              ...
              koskioppijat.size(), oppijanumerotJoiltaKoskiOpiskeluoikeudetPuuttuvat.size(),
              hakemusWrappers.size(), maaraJoilleTiedotJoLoytyvat, hakukohdeOid));
          koskioppijat.forEach(o -> o.poistaMuuntyyppisetOpiskeluoikeudetKuin(koskenOpiskeluoikeusTyypit));
          koskiOpiskeluoikeusHistoryService.haeVanhemmatOpiskeluoikeudetTarvittaessa(koskioppijat,
              paivaJonkaMukaisiaTietojaKoskiDatastaKaytetaan);
          Map<String, KoskiOppija> koskiOppijatOppijanumeroittain = koskioppijat.stream()
    ...
    return valintaperusteet.stream()
        .anyMatch(valintaperusteetDTO -> valintaperusteetDTO.getValinnanVaihe().getValintatapajono().stream()
            .anyMatch(jono -> mukanaLaskennassa(jono, nyt) && jono.getJarjestyskriteerit().stream()
                .anyMatch(kriteeri -> sisaltaaKoskiFunktioita(kriteeri.getFunktiokutsu()))));
  }
  ...
  private static Predicate<String> resolveKoskiHakukohdeOidFilter(String koskiHakukohdeOiditString) {
    if (StringUtils.isBlank(koskiHakukohdeOiditString)) {
      LOG.info("Saatiin '" + koskiHakukohdeOiditString
  ...
  private static Set<Funktionimi> resolveKoskenFunktionimet(String koskenFunktionimetString) {
    if (StringUtils.isBlank(koskenFunktionimetString)) {
      LOG.info("Saatiin '" + koskenFunktionimetString
          + "' funktionimiksi, joille haetaan dataa Koskesta => ei haeta ollenkaan tietoja Koskesta.");
      return Collections.emptySet();
    }
    Set<Funktionimi> funktionimet = Arrays.stream(koskenFunktionimetString.split(",")).map(Funktionimi::valueOf)
  ...
  private static Set<String> resolveKoskenOpiskeluoikeudet(String koskenOpiskeluoikeusTyypitString) {
    if (StringUtils.isBlank(koskenOpiskeluoikeusTyypitString)) {
      LOG.info("Saatiin '" + koskenOpiskeluoikeusTyypitString
