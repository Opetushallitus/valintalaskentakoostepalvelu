...
public class OsoiteHakemukseltaUtil {
  ...
  public static Osoite osoiteHakemuksesta(
      HakemusWrapper hakemus,
      Map<String, Koodi> maatJaValtiot1,
      Map<String, Koodi> posti,
      NimiPaattelyStrategy nimiPaattelyStrategy) {
    Koodi postiKoodi = posti.get(hakemus.getSuomalainenPostinumero());
    String postitoimipaikka =
        KoodistoCachedAsyncResource.haeKoodistaArvo(
            postiKoodi, KieliUtil.SUOMI, hakemus.getSuomalainenPostinumero());

    String maa =
        SUOMI.equalsIgnoreCase(hakemus.getAsuinmaa())
            ? "Suomi"
            : KoodistoCachedAsyncResource.haeKoodistaArvo(
    ...
    if (StringUtils.isEmpty(postitoimipaikka)) {
      postitoimipaikka =
          SUOMI.equalsIgnoreCase(maakoodi) ? "" : hakemus.getUlkomainenPostitoimipaikka();
    }
    String postinumero =
        SUOMI.equalsIgnoreCase(maakoodi)
    ...
    boolean ulkomaillaSuoritettuKoulutusTaiOppivelvollisuudenKeskeyttanyt =
        hakemus.ulkomaillaSuoritettuKoulutusTaiOppivelvollisuudenKeskeyttanyt();
    ...
    return new OsoiteBuilder()
        ...
        .setUlkomaillaSuoritettuKoulutusTaiOppivelvollisuudenKeskeyttanyt(
