...
public class LueHakijapalvelunOsoite {
  ...
  public static final Set<String> kiinnostavatYhteystiedot =
      Collections.unmodifiableSet(
          Sets.newHashSet(Arrays.asList(KAYNTI_TYYPPI, POSTI_TYYPPI, ULKOMAINEN_POSTI_TYYPPI)));
  private static final Logger LOG = LoggerFactory.getLogger(LueHakijapalvelunOsoite.class);

  /** Preferoi posti-tyyppisia ja palauttaa vaan kaynti ja posti tyyppisista osoitteen. */
  public static Osoite lueHakijapalvelunOsoite(
    ...
    List<Yhteystieto> yhteystiedot =
        Optional.ofNullable(
            ...
            .stream()
            .filter(kiinnostavaYhteystieto())
            .collect(Collectors.toList());
    List<Yhteystieto> puhelinnumerot =
        Optional.ofNullable(
                Optional.ofNullable(organisaatio.getMetadata())
                    .orElse(new Metadata())
                    .getYhteystiedot())
            .orElse(Collections.emptyList())
            .stream()
            .filter(puhelinnumero())
            .collect(Collectors.toList());
    List<Yhteystieto> emailit =
        Optional.ofNullable(
                Optional.ofNullable(organisaatio.getMetadata())
                    .orElse(new Metadata())
                    .getYhteystiedot())
    ...
    if (KieliUtil.ENGLANTI.equals(preferoitukielikoodi)) {
      List<Yhteystieto> englanninkielisetYhteystiedot =
          yhteystiedot.stream().filter(englanninkieliset()).collect(Collectors.toList());
      List<Yhteystieto> englanninkielisetNumerot =
          puhelinnumerot.stream().filter(englanninkieliset()).collect(Collectors.toList());
      List<Yhteystieto> englanninkielisetEmailit =
          emailit.stream().filter(englanninkieliset()).collect(Collectors.toList());
      ...
      List<Yhteystieto> postit =
          englanninkielisetYhteystiedot.stream()
              .filter(ulkomainenPostiTyyppia())
              .collect(Collectors.toList());
    ...
    if (KieliUtil.RUOTSI.equals(preferoitukielikoodi)) {
      List<Yhteystieto> ruotsinkielisetYhteystiedot =
          yhteystiedot.stream().filter(ruotsinkieliset()).collect(Collectors.toList());

      List<Yhteystieto> postit =
          ruotsinkielisetYhteystiedot.stream().filter(postiTyyppia()).collect(Collectors.toList());

      List<Yhteystieto> ruotsinkielisetNumerot =
          puhelinnumerot.stream().filter(ruotsinkieliset()).collect(Collectors.toList());
      List<Yhteystieto> ruotsinkielisetEmailit =
          emailit.stream().filter(ruotsinkieliset()).collect(Collectors.toList());
    ...
    List<Yhteystieto> suomenkielisetNumerot =
        puhelinnumerot.stream().filter(suomenkieliset()).collect(Collectors.toList());
    List<Yhteystieto> suomenkielisetEmailit =
        emailit.stream().filter(suomenkieliset()).collect(Collectors.toList());
    ...
    List<Yhteystieto> suomenkielisetYhteystiedot =
        yhteystiedot.stream().filter(suomenkieliset()).collect(Collectors.toList());
    List<Yhteystieto> suomenkielisetPostit =
        suomenkielisetYhteystiedot.stream().filter(postiTyyppia()).collect(Collectors.toList());
    ...
    List<Yhteystieto> postit =
        yhteystiedot.stream().filter(postiTyyppia()).collect(Collectors.toList());
    if (!postit.isEmpty()) {
      return osoiteKomponentti.haeOsoiteYhteystiedoista(
          postit.iterator().next(),
          KieliType.FI,
  ...
  private static Predicate<Yhteystieto> puhelinnumero() {
    return yhteystiedot ->
        "puhelin".equals(yhteystiedot.getTyyppi())
