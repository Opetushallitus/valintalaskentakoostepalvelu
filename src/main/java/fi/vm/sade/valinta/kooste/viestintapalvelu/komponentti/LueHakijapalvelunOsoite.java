...
public class LueHakijapalvelunOsoite {
    ...
    public static final Set<String> kiinnostavatYhteystiedot = Collections.unmodifiableSet(Sets.newHashSet(Arrays.asList(KAYNTI_TYYPPI, POSTI_TYYPPI, ULKOMAINEN_POSTI_TYYPPI)));
     ...
     * osoitteen.
     */
    public static Osoite lueHakijapalvelunOsoite(HaeOsoiteKomponentti osoiteKomponentti, String preferoitukielikoodi, Organisaatio organisaatio, Teksti organisaationimi) {
        ...
        if (yhteystiedot.isEmpty()) {
            LOG.warn("Organisaatiolla {} ei ole tyyppia posti tai kaynti olevaa hakijapalvelun osoitetta!", organisaatio.getOid());
            return null;
        }
        String numero = null;
        String email = null;
        ...
        if (KieliUtil.ENGLANTI.equals(preferoitukielikoodi)) {
            ...
            if (!postit.isEmpty()) {
                return osoiteKomponentti.haeOsoiteYhteystiedoista(postit.iterator().next(),
        ...
        if (!suomenkielisetPostit.isEmpty()) {
            return osoiteKomponentti.haeOsoiteYhteystiedoista(suomenkielisetPostit.iterator().next(),
                    KieliType.FI, organisaatio.getMetadata().getHakutoimistonNimi().get("kieli_fi#1"), email, numero);
        }
        ...
        if (!yhteystiedot.isEmpty()) {
            return osoiteKomponentti.haeOsoiteYhteystiedoista(yhteystiedot.iterator().next(),
                    KieliType.FI, organisaatio.getMetadata().getHakutoimistonNimi().get("kieli_fi#1"), email, numero);
        }
        return null;
    }
    ...
    private static Predicate<Yhteystieto> suomenkieliset() {
        return yhteystiedot -> KieliUtil.SUOMI.equals(KieliUtil.normalisoiKielikoodi(TarjontaUriToKoodistoUtil.cleanUri(yhteystiedot.getKieli())));
    }

    private static Predicate<Yhteystieto> ruotsinkieliset() {
        return yhteystiedot -> KieliUtil.RUOTSI.equals(KieliUtil.normalisoiKielikoodi(TarjontaUriToKoodistoUtil.cleanUri(yhteystiedot.getKieli())));
    }

    private static Predicate<Yhteystieto> englanninkieliset() {
        return yhteystiedot -> KieliUtil.ENGLANTI.equals(KieliUtil.normalisoiKielikoodi(TarjontaUriToKoodistoUtil.cleanUri(yhteystiedot.getKieli())));
    }
