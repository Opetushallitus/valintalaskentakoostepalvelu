...
public class ValidoiTilatUtil {
  private static final Set<HakemuksenTila> HYVAKSYTTYNA =
      Sets.newEnumSet(
          Arrays.asList(HakemuksenTila.HYVAKSYTTY, HakemuksenTila.VARASIJALTA_HYVAKSYTTY),
          HakemuksenTila.class);

  private static final Set<HakemuksenTila> HYLATTY_TAI_VARALLA =
      Sets.newEnumSet(
          Arrays.asList(HakemuksenTila.HYLATTY, HakemuksenTila.VARALLA), HakemuksenTila.class);

  private static final Set<HakemuksenTila> PERUNEENA =
      Sets.newEnumSet(
          Arrays.asList(
              HakemuksenTila.PERUNUT, HakemuksenTila.PERUUTETTU, HakemuksenTila.PERUUNTUNUT),
          HakemuksenTila.class);

  private static final Set<ValintatuloksenTila> VASTAANOTTANEENA =
      Sets.newEnumSet(
          Arrays.asList(
              EI_VASTAANOTETTU_MAARA_AIKANA, EHDOLLISESTI_VASTAANOTTANUT, VASTAANOTTANUT_SITOVASTI),
  ...
  public static String validoi(
    ...
    if (hakemuksenTila == null || valintatuloksenTila == null || ilmoittautumisTila == null) {
      return virheellinenTilaYhdistelma(
              new StringBuilder("Tila ei saa olla tyhjä. "),
              hakemuksenTila,
              valintatuloksenTila,
              ilmoittautumisTila)
          .toString();
    ...
    if (!EI_ILMOITTAUTUMISTA.contains(ilmoittautumisTila)) {
      if (HYVAKSYTTYNA.contains(hakemuksenTila)
          && VASTAANOTTANEENA_TAI_PERUNEENA.contains(valintatuloksenTila)) {
        return null; // OK
      } else {
        return virheellinenTilaYhdistelma(
                new StringBuilder(
    ...
    if ((HakemuksenTila.PERUNUT.equals(hakemuksenTila)
            && ValintatuloksenTila.PERUNUT.equals(valintatuloksenTila))
    ...
    if (VASTAANOTTANEENA_TAI_PERUNEENA_EI_MYOHASTYNYT.contains(valintatuloksenTila)) {
      if (HYVAKSYTTYNA.contains(hakemuksenTila)) {
        ...
        return virheellinenTilaYhdistelma(
                new StringBuilder(
                    "Vastaanottaneen tai peruneen hakijan tulisi olla hyväksyttynä. "),
                hakemuksenTila,
                valintatuloksenTila,
                ilmoittautumisTila)
            .toString();
    ...
    if (HYLATTY_TAI_VARALLA.contains(hakemuksenTila)) {
      if (!VASTAANOTTANEENA.contains(valintatuloksenTila)
          && EI_ILMOITTAUTUMISTA.contains(ilmoittautumisTila)) {
        return null; // OK
      } else {
        return virheellinenTilaYhdistelma(
                new StringBuilder(
    ...
    if (PERUNEENA.contains(hakemuksenTila)) {
      if (KESKEN_TAI_PERUNUT_VASTAANOTTAJA.contains(valintatuloksenTila)) {
        ...
        return virheellinenTilaYhdistelma(
                new StringBuilder("Peruneella vastaanottajalla ei voi olla vastaanottotilaa. "),
                hakemuksenTila,
                valintatuloksenTila,
                ilmoittautumisTila)
            .toString();
  ...
  private static StringBuilder virheellinenTilaYhdistelma(
      StringBuilder sb,
      HakemuksenTila hakemuksenTila,
      ValintatuloksenTila valintatuloksenTila,
      IlmoittautumisTila ilmoittautumisTila) {
    sb.append(toString(hakemuksenTila))
        .append(", ")
        .append(toString(valintatuloksenTila))
        .append(" ja ")
        .append(toString(ilmoittautumisTila))
        .append(" on virheellinen tilayhdistelmä");
    return sb;
  }
