...
public class AmmatillisenKielikoeMigrationPistesyottoServiceV2 {
  ...
  @Autowired
  public AmmatillisenKielikoeMigrationPistesyottoServiceV2(
  ...
  public SingleSaveResult save(ValintakoeOsallistuminenDTO valintakoeOsallistuminen) {
    List<HakutoiveDTO> hakutoiveetWithKielikoeResults =
        valintakoeOsallistuminen.getHakutoiveet().stream()
            .filter(containsKielikoeResult())
            .collect(Collectors.toList());
    if (hakutoiveetWithKielikoeResults.size() != 1) {
      throw new IllegalStateException(
          String.format(
              "Odotettiin täsämälleen yhtä toivetta, jossa on kielikokeeseen osallistuminen, "
                  + "mutta löytyi %s kpl: %s",
              hakutoiveetWithKielikoeResults.size(), hakutoiveetWithKielikoeResults));
    }

    HakutoiveDTO toive = hakutoiveetWithKielikoeResults.get(0);
    String hakukohdeOid = toive.getHakukohdeOid();
    ResultV1RDTO<HakukohdeValintaperusteetV1RDTO> hakukohdeSearchResult =
        hakukohdeResource.findValintaperusteetByOid(hakukohdeOid);
    if (hakukohdeSearchResult.hasErrors()) {
      throw new RuntimeException(
          String.format(
              "Ongelma haettaessa hakukohteen %s tietoja tarjonnasta: %s",
              hakukohdeOid, hakukohdeSearchResult.getErrors()));
    }
  ...
  private Predicate<HakutoiveDTO> containsKielikoeResult() {
    return h ->
        h.getValinnanVaiheet().stream()
            .anyMatch(
                vaihe -> vaihe.getValintakokeet().stream().anyMatch(isKielikoeParticipation()));
  }

  private Predicate<ValintakoeDTO> isKielikoeParticipation() {
    return koe ->
        Osallistuminen.OSALLISTUU.equals(koe.getOsallistuminenTulos().getOsallistuminen())
