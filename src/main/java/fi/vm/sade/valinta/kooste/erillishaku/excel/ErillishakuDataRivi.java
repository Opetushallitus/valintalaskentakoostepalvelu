...
public class ErillishakuDataRivi extends DataRivi {
  ...
  public boolean validoi(Rivi rivi) throws ExcelValidointiPoikkeus {
    ...
    if (isDataRow(rivi, sukunimi, etunimi, oid)) {
      kuuntelija.erillishakuRiviTapahtuma(new ErillishakuRiviBuilder().sukunimi(sukunimi).etunimi(etunimi)
          .henkilotunnus(henkilotunnus).sahkoposti(sahkoposti).syntymaAika(syntymaAika).sukupuoli(sukupuoli)
          .personOid(oid).aidinkieli(aidinkieli).hakemuksenTila(hakemuksenTila)
          ...
          .hyvaksymiskirjeLahetetty(hyvaksymiskirjeLahetetty).vastaanottoTila(vastaanottoTila)
          .ilmoittautumisTila(ilmoittautumisTila).julkaistaankoTiedot(julkaistaankoTiedot)
          .poistetaankoRivi(false).asiointikieli(asiointikieli).puhelinnumero(puhelinnumero).osoite(osoite)
          .postinumero(postinumero).postitoimipaikka(postitoimipaikka).asuinmaa(asuinmaa)
          .kansalaisuus(kansalaisuus).kotikunta(kotikunta).toisenAsteenSuoritus(toisenAsteenSuoritus)
          .toisenAsteenSuoritusmaa(toisenAsteenSuoritusmaa).maksuvelvollisuus(maksuvelvollisuus)
          .maksuntila(maksuntila).build());
    }
    return true;
  }
  ...
  private boolean isDataRow(Rivi rivi, String sukunimi, String etunimi, String oid) {
    return !rivi.isTyhja() && rivi.getSolut().size() >= 14 // Copy-paste easily creates extra columns for excel doc
  ...
  static final Collection<String> SUKUPUOLEN_ARVOT = Arrays.stream(Sukupuoli.values()).map(Object::toString)
      .collect(Collectors.toList());
  private static final Collection<String> HAKEMUKSENTILA_ARVOT = Stream
      .concat(Stream.of("KESKEN"), Arrays.stream(HakemuksenTila.values()).map(Enum::toString))
      .collect(Collectors.toList());
  private static final Collection<String> HAKEMUKSENTILA_ARVOT_TOINEN_ASTE = Stream
      .concat(Stream.of("KESKEN"), Arrays.stream(HakemuksenTila.values()).map(Enum::toString))
      .collect(Collectors.toList());
  private static final Collection<String> HAKEMUKSENTILA_ARVOT_KK = Stream
      .concat(Stream.of("KESKEN"),
          Arrays.stream(HakemuksenTila.values())
              .filter(t -> !HakemuksenTila.HARKINNANVARAISESTI_HYVAKSYTTY.equals(t)).map(Enum::toString))
      .collect(Collectors.toList());
  private static final Collection<String> VASTAANOTTOTILA_ARVOT = Arrays.stream(ValintatuloksenTila.values())
      .map(Enum::toString).collect(Collectors.toList());
  private static final Collection<String> VASTAANOTTOTILA_ARVOT_KK = Stream.of(
      // KORKEAKOULUJEN VALINTATULOKSEN TILAT
      ValintatuloksenTila.EI_VASTAANOTETTU_MAARA_AIKANA, ValintatuloksenTila.PERUNUT,
      ValintatuloksenTila.PERUUTETTU, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI, ValintatuloksenTila.KESKEN
  //
  ).map(Enum::toString).collect(Collectors.toList());
  private static final Collection<String> VASTAANOTTOTILA_ARVOT_TOINEN_ASTE = Stream.of(
      // TOISEN ASTEEN VALINTATULOKSEN TILAT
      ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI, ValintatuloksenTila.EI_VASTAANOTETTU_MAARA_AIKANA,
      ValintatuloksenTila.PERUNUT, ValintatuloksenTila.KESKEN
  //
  ).map(Enum::toString).collect(Collectors.toList());
  private static final Collection<String> ILMOITTAUTUMISTILA_ARVOT = Arrays.stream(IlmoittautumisTila.values())
      .map(Enum::toString).collect(Collectors.toList());
  ...
  private static final Collection<String> JULKAISU_LUPA_ARVOT = Arrays.asList(LUPA_JULKAISUUN, StringUtils.EMPTY,
      EI_LUPAA_JULKAISUUN);
  ...
  public static MonivalintaArvo hakemuksenTila(Hakutyyppi hakutyyppi, String arvo) {
    return getMonivalintaArvo(hakutyyppi, arvo, HAKEMUKSENTILA_ARVOT_TOINEN_ASTE, HAKEMUKSENTILA_ARVOT_KK,
        HAKEMUKSENTILA_ARVOT);
  }

  static MonivalintaArvo vastaanottoTila(Hakutyyppi hakutyyppi, String arvo) {
    return getMonivalintaArvo(hakutyyppi, arvo, VASTAANOTTOTILA_ARVOT_TOINEN_ASTE, VASTAANOTTOTILA_ARVOT_KK,
        VASTAANOTTOTILA_ARVOT);
  }

  private static MonivalintaArvo getMonivalintaArvo(Hakutyyppi hakutyyppi, String arvo, Collection<String> toinenAste,
      Collection<String> kk, Collection<String> other) {
  ...
  private static final Collection<String> MAKSUVELVOLLISUUS_ARVOT = Arrays.asList(Maksuvelvollisuus.NOT_CHECKED,
      Maksuvelvollisuus.REQUIRED, Maksuvelvollisuus.NOT_REQUIRED);
  private static final Collection<String> MAKSUNTILA_ARVOT = Arrays.asList("", Maksuntila.MAKSAMATTA.toString(),
      Maksuntila.MAKSETTU.toString(), Maksuntila.VAPAUTETTU.toString());
  ...
  public static MonivalintaArvo ehdollisenHyvaksymisenEhtoKoodi(String ehdollisenHyvaksymisenEhtoKoodi,
    ...
    Map<String, Koodi> hyvaksymisenEhdot = koodistoCachedAsyncResource
        .haeKoodisto(KoodistoCachedAsyncResource.HYVAKSYNNAN_EHDOT);
    ...
    for (Koodi koodi : hyvaksymisenEhdot.values()) {
      ehdollisenHyvaksymisenEhdot
          .add(koodi.getKoodiArvo() + " " + KoodistoCachedAsyncResource.haeKoodistaArvo(koodi, "FI", null));
    }
    return new MonivalintaArvo(ehdollisenHyvaksymisenEhtoKoodi, ehdollisenHyvaksymisenEhdot);
  }

  public static MonivalintaArvo aidinkieli(String aidinkieli,
      KoodistoCachedAsyncResource koodistoCachedAsyncResource) {
