...
public class RetryUtil {
    ...
    public static <O> CompletableFuture<O> executeWithRetry(
            Supplier<CompletableFuture<O>> action, String tunniste, int retries, int secondsToWaitMultiplier) {
        return action
                .get()
                .handleAsync(
                        (r, t) -> {
                        ...
                        (r, t) -> {
                            if (t != null) {
                                ...
                                return CompletableFuture.supplyAsync(() -> "OK", delayedExecutor)
                                        .thenComposeAsync(
                                                x ->
                                                        retry(action, throwable, retry + 1, maxRetries, secondsToWaitMultiplier, tunniste));
                            }
                            return CompletableFuture.completedFuture(r);
                        })
                .thenCompose(java.util.function.Function.identity());
