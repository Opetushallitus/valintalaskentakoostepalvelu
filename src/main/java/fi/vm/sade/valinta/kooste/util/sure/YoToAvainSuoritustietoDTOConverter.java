...
public class YoToAvainSuoritustietoDTOConverter {
  ...
  private static final Set<String> OSAKOETUNNUS_WHITELIST = Collections.unmodifiableSet(
      Sets.newHashSet(Arrays.asList("01", "02", "03", "04", "05", "06", "07", "08", "09", "10")));
  ...
  private static final Function<Arvosana, Map<String, String>> mapArvosana = a -> {
    ...
    ofNullable(a.getArvio().getPisteet()).ifPresent(pisteet -> {
  ...
  public static List<AvainMetatiedotDTO> convert(Oppija oppija) {
    ...
    List<SuoritusJaArvosanat> yoSuoritukset = oppija.getSuoritukset().stream()
    ...
    List<Arvosana> yoArvosanat = yoSuoritukset.iterator().next().getArvosanat().stream().flatMap(a -> normalisoi(a))
        .collect(Collectors.toList());
    Map<String, List<Arvosana>> arvosanaGrouping = yoArvosanat.stream().collect(Collectors
        .groupingBy(a -> ((Arvosana) a).getAine(), Collectors.mapping(identity(), Collectors.toList())));
    return Stream.of(
        // arvosanat
        arvosanaGrouping.entrySet().stream()
            .map(a -> new AvainMetatiedotDTO(a.getKey(),
                a.getValue().stream().map(mapArvosana).collect(Collectors.toList()))),
        // ainereaali
        arvosanaGroup("AINEREAALI",
            yoArvosanat.stream()
                .filter(find("UE", "UO", "ET", "FF", "PS", "HI", "FY", "KE", "BI", "GE", "TE", "YH"))),
        arvosanaGroup("REAALI", yoArvosanat.stream().filter(find("RR", "RO", "RY"))),
        arvosanaGroup("PITKA_KIELI",
            yoArvosanat.stream().filter(find("EA", "FA", "GA", "HA", "PA", "SA", "TA", "VA", "S9"))),
        arvosanaGroup("KESKIPITKA_KIELI",
            yoArvosanat.stream().filter(find("EB", "FB", "GB", "HB", "PB", "SB", "TB", "VB"))),
        arvosanaGroup("LYHYT_KIELI",
            yoArvosanat.stream().filter(find("EC", "FC", "GC", "L1", "PC", "SC", "TC", "VC", "KC", "L7"))),
        arvosanaGroup("AIDINKIELI", yoArvosanat.stream().filter(find("O", "A", "I", "W", "Z", "O5", "A5"))))
  ...
  private static Stream<AvainMetatiedotDTO> arvosanaGroup(String group, Stream<Arvosana> arvosanat) {
    ...
    if (ar.isEmpty()) {
      ...
      return Stream.of(new AvainMetatiedotDTO(group,
          ar.stream().map(mapArvosanaWithLisatieto).collect(Collectors.toList())));
    }
  }
  ...
  private static Stream<Arvosana> normalisoi(Arvosana a) {
    if (OSAKOE_ASTEIKKO.equals(a.getArvio().getAsteikko())) {
      ...
      return Stream.of(new Arvosana(a.getId(), a.getSuoritus(), osakoe, a.getValinnainen(), a.getMyonnetty(),
          a.getSource(), oldExamRole(a), a.getArvio(), a.getLisatieto()));
    } else {
      return Stream.of(new Arvosana(a.getId(), a.getSuoritus(), aineMapper(a), a.getValinnainen(),
          a.getMyonnetty(), a.getSource(), oldExamRole(a), a.getArvio(), a.getLisatieto()));
    }
  }
  ...
  private static String aineMapper(Arvosana a) {
    return a.getKoetunnus().map(YoToAvainSuoritustietoDTOConverter::koetunnusMapper).orElseGet(() -> {
      ...
      LOG.warn("No koetunnus in YO arvosana: mapped aine '" + a.getAine() + "' and lisatieto '" + a.getLisatieto()
          + "' to aine " + aine);
      return aine;
    });
  }

  private static ImmutableMap<String, ImmutableMap<String, String>> NEW_EXAM_ROLE_TO_OLD_EXAM_ROLE = ImmutableMap.of(
      "mother-tongue", ImmutableMap.<String, String>builder().put("A", "11").put("Z", "11").put("O", "11")
          .put("W", "11").put("I", "11").put("J", "13").put("A5", "14").put("O5", "14").build(),
      "mandatory-subject",
      ImmutableMap.<String, String>builder().put("CA", "21").put("S2", "21").put("T1", "21").put("E2", "21")
          .put("P1", "21").put("F1", "21").put("CB", "21").put("V2", "21").put("S1", "21").put("P2", "21")
          .put("V1", "21").put("G2", "21").put("F2", "21").put("T2", "21").put("E1", "21").put("H2", "21")
          .put("BB", "21")
          // .put("RR","21")
          // .put("VC","21")
          // .put("PA","21")
          ...
          .put("O", "22").put("W", "22").put("Z", "22").put("A", "22")
          /* END OF 22 */
          .put("PC", "31").put("DC", "31").put("TC", "31").put("SC", "31").put("SA", "31").put("BA", "31")
          // .put("BB","31")
          .put("FC", "31").put("EA", "31").put("PA", "31").put("L1", "31").put("VC", "31").put("CC", "31")
          .put("EB", "31").put("EC", "31").put("S9", "31").put("L7", "31").put("VA", "31").put("FA", "31")
          /* END OF 31 */

          .put("UE", "41").put("RY", "41").put("HI", "41").put("GE", "41").put("PS", "41").put("YH", "41")
          .put("FY", "41").put("RO", "41").put("TE", "41").put("BI", "41").put("KE", "41").put("FF", "41")
          .put("UO", "41").put("ET", "41").put("RR", "41").put("M", "41").put("N", "41").build(),
      "optional-subject", ImmutableMap.<String, String>builder().put("A", "60").put("O", "60").put("Z", "60")
          /* END OF 60 */
          .put("L7", "61").put("VA", "61").put("IC", "61").put("L1", "61").put("VC", "61").put("EB", "61")
          .put("PC", "61").put("DC", "61").put("SA", "61").put("S9", "61").put("EC", "61").put("FA", "61")
          .put("KC", "61").put("TC", "61").put("GC", "61").put("QC", "61").put("P2", "61").put("EA", "61")
          .put("FC", "61").put("PA", "61").put("SC", "61")
          /* END OF 61 */
          .put("CA", "62").put("BB", "62").put("A5", "62").put("BA", "62").put("CB", "62")
          /* END OF 71 */
          .put("RR", "71").put("PS", "71").put("UE", "71").put("GE", "71").put("RY", "71").put("KE", "71")
          .put("FF", "71").put("YH", "71").put("HI", "71").put("ET", "71").put("TE", "71").put("RO", "71")
          .put("FY", "71").put("UO", "71").put("BI", "71")
          /* END OF 71 */
          .put("M", "81").put("N", "81")
          /* END OF 81 */
          .build());
  ...
  private static Optional<Map<String, String>> oldExamRole(String examId, String newExamRole) {
    return Optional
        .ofNullable(NEW_EXAM_ROLE_TO_OLD_EXAM_ROLE.getOrDefault(newExamRole, ImmutableMap.of()).get(examId))
        .map(oldExamRole -> ImmutableMap.of("aineyhdistelmarooli", oldExamRole, "koetunnus", examId));
  }
