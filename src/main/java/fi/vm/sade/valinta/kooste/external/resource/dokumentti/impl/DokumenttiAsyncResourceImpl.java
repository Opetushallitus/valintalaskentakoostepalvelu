...
public class DokumenttiAsyncResourceImpl implements DokumenttiAsyncResource {
  ...
  public CompletableFuture<Void> uudelleenNimea(String dokumenttiId, String filename) {
    final CompletableFuture<Collection<ObjectMetadata>> objectsFuture =
        dokumenttipalvelu.findAsync(Collections.singleton(dokumenttiId));
    return objectsFuture.thenComposeAsync(
        objects -> {
          if (objects.isEmpty()) {
            return CompletableFuture.failedFuture(
                new RuntimeException("Dokumenttia ei löytynyt dokumenttiId:llä " + dokumenttiId));
          }
          return dokumenttipalvelu.renameAsync(objects.stream().findFirst().get().key, filename);
        });
  }

  @Override
  public Observable<ResponseEntity<Object>> tallenna(
      String id,
      String filename,
      Long expirationDate,
      List<String> tags,
